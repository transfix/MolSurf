...
public:
  ...
  int m_ElementIndex;  // Used to define the atom type. See
                       // PDBParser::elementTable for details.
  unsigned int m_UserDefinedColor;
  char m_Chain; // Chain ID (from PDB)

  bool m_Backbone;       // If true, the atom is a member of the macromolecule's
                         // backbone.
  bool m_IsBoundaryAtom; // If true, atom contributes to the molecular surface

  bool m_donorOrAcceptor; // false = not a hbond donor/accpetor, true = is a
                          // donor/acceptor
  int m_internalHbond;    // 1 = external hbond, 2 = internal, 3 = both internal
                          // and external
