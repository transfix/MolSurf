...
template <class T> class StreamC2CBuffer : public C2CBuffer<T> {
...
template <class T> StreamC2CBuffer<T>::~StreamC2CBuffer() { delete m_io; }

template <class T> inline void StreamC2CBuffer<T>::readHeader() {
...
template <class T>
void StreamC2CBuffer<T>::getDimension(int dim[3], float orig[3],
                                      float span[3]) {
  for (int i = 0; i < 3; i++) {
    dim[i] = m_dim[i];
    orig[i] = m_orig[i];
...
template <class T> int StreamC2CBuffer<T>::getLayer(Cell *&cells) {
  int nc;
  nc = m_io->read32();
#ifdef _DEBUG
