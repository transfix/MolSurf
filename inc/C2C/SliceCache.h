...
/*
  ...
  Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA
*/
// slicecache.h: a cache class where we store slices.
// It's implemented using a FIFO algorithm.
// We use a mutex with slice cache to manage synchronization of accessing cache
...
template <class T> class SliceCache {
public:
  ...
  // add slice to cache if there is enough space. return true if the slice is
  // added
...
template <class T> SliceCache<T>::~SliceCache() {
#ifdef MUL_THREAD
  lock->acquire();
#endif
...
template <class T> bool SliceCache<T>::contains(int key) {
#ifdef MUL_THREAD
  lock->acquire();
#endif
...
template <class T> int SliceCache<T>::remove(int key) {
...
template <class T> int SliceCache<T>::remove_first() {
#ifdef MUL_THREAD
  lock->acquire();
#endif
  if (keys.size() == 0) {
...
template <class T> bool SliceCache<T>::enoughSpace(int nbytes) {
#ifdef MUL_THREAD
  lock->acquire();
#endif
  bool enough;
  ...
  if (size + nbytes <= cap || keys.size() == 0) {
    enough = true;
  } else {
    enough = false;
  }
...
template <class T> int SliceCache<T>::add(int key, Slice<T> *slice) {
...
template <class T> bool SliceCache<T>::tryAdd(int key, Slice<T> *slice) {
  if (!enoughSpace(slice->size())) {
    return false;
  }
  add(key, slice);
  return true;
...
template <class T> Slice<T> *SliceCache<T>::get(int key) {
