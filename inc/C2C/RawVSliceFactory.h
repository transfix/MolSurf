...
#include <C2C/RawVReader.h>
#include <C2C/SliceCache.h>
#include <C2C/SliceFactory.h>

template <class T> class RawVSliceFactory : virtual public SliceFactory<T> {
public:
...
template <class T> RawVSliceFactory<T>::RawVSliceFactory(const char *rawfile) {
...
template <class T> RawVSliceFactory<T>::~RawVSliceFactory() {
  delete cache;
  delete reader;
  m_io->close();
  delete m_io;
...
template <class T> Slice<T> *RawVSliceFactory<T>::getNextSlice() {
...
template <class T> void RawVSliceFactory<T>::readRawVHeader() {
  int dummy, i;
...
template <class T> void RawVSliceFactory<T>::loadInitPages() {
