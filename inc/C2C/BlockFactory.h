...
/*
  ...
  Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA
*/
// This class reads slices from an memory resident block
...
template <class T> class BlockFactory : virtual public SliceFactory<T> {
public:
...
template <class T> BlockFactory<T>::~BlockFactory() {}

template <class T> Slice<T> *BlockFactory<T>::getNextSlice() {
  Slice<T> *slc;
  int off;
  ...
  if (m_red && m_green && m_blue)
    slc = new Slice<T>(&m_data[off], &m_red[off], &m_green[off], &m_blue[off],
                       m_dim);
  else {
