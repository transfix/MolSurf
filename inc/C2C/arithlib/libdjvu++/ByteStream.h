...
/** @name ByteStream.h
    ...
    @version
    #$Id: ByteStream.h,v 1.1.1.1 2008/06/06 19:42:10 wzhao Exp $# */
//@{
...
#include <C2C/arithlib/libdjvu++/GContainer.h>
...
public:
  ...
  /** Sets the current position for reading or writing the ByteStream.  Class
      ...
      of the file. It is then advisable to provide a negative value for
     #offset#. \end{description} Results are undefined whenever the new position
     is greater than the total size of the ByteStream. Exception
     \Ref{GException} is thrown with a plain text error message whenever an
     error occurs. */
...
inline int MemoryByteStream::size() const { return bsize; }

inline char &MemoryByteStream::operator[](int n) {
