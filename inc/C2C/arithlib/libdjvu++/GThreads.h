...
/** @name GThreads.h
    ...
    @version
    #$Id: GThreads.h,v 1.1.1.1 2008/06/06 19:42:10 wzhao Exp $# */
//@{
...
inline GCriticalSection::GCriticalSection() : ok(FALSE) {
...
inline GEvent::GEvent() { hev = CreateEvent(NULL, FALSE, FALSE, NULL); }

inline GEvent::~GEvent() { CloseHandle(hev); }

inline void GEvent::set() { SetEvent(hev); }

inline void GEvent::wait() { WaitForSingleObject(hev, INFINITE); }

inline void GEvent::wait(int timeout) { WaitForSingleObject(hev, timeout); }
...
inline GCriticalSectionLock::GCriticalSectionLock(GCriticalSection *gsec)
    : gsec(gsec) {
  gsec->lock();
}

inline GCriticalSectionLock::~GCriticalSectionLock() { gsec->unlock(); }
