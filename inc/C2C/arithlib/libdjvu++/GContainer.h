...
/** @name GContainer.h
    ...
    @version
    #$Id: GContainer.h,v 1.1.1.1 2008/06/06 19:42:10 wzhao Exp $# */
//@{
...
public:
  ...
  /** Moves position #pos# to the next element in the container.
      ...
      ({\em b}) the input value of #pos# is not a valid position for this
     container. This function is called by #GPosition::operator++#. */
...
public:
  ...
  /** Extends the subscript range so that is contains #n#.
     ...
     array element: \begin{verbatim} int lineno=1; GArray<GString> a; while (!
...
/** Doubly linked list.
    ...
    of type #TYPE#.  Member functions are provided to search the list for an
   element, to insert or delete elements at specified positions.
...
template <class KTYPE, class VTYPE> class GMap : public GContainer<VTYPE> {
public:
  // CONSTRUCTORS
...
static inline unsigned int hash(const unsigned int &x) { return x; }

/** Hashing function (int). */
static inline unsigned int hash(const int &x) { return (unsigned int)x; }

/** Hashing function (long). */
static inline unsigned int hash(const long &x) { return (unsigned int)x; }

/** Hashing function (unsigned long). */
static inline unsigned int hash(const unsigned long &x) {
  return (unsigned int)x;
}

/** Hashing function (float). */
static inline unsigned int hash(const float &x) {
...
static inline unsigned int hash(const double &x) {
  // optimizer will get rid of unnecessary code
  unsigned int *addr = (unsigned int *)&x;
...
inline GPosition::GPosition() : cont(0), pi(0), pv(0) {}

inline GPosition::GPosition(const GContainerBase &gc) : cont(0), pi(0), pv(0) {
  *this = gc.firstpos();
}

inline GPosition::operator int() { return cont != 0; }

inline int GPosition::operator!() { return cont == 0; }

inline GPosition &GPosition::operator++() {
  if (cont) {
    cont->nextpos(*this);
  }
  return *this;
...
template <class TYPE> TYPE *GContainer<TYPE>::next(GPosition &pos) {
  TYPE *ret = get(pos);
  nextpos(pos);
  return ret;
}

template <class TYPE> TYPE *GContainer<TYPE>::prev(GPosition &pos) {
  TYPE *ret = get(pos);
  prevpos(pos);
  return ret;
}

template <class TYPE> const TYPE *GContainer<TYPE>::next(GPosition &pos) const {
  return ((GContainer<TYPE> *)this)->next(pos);
}

template <class TYPE> const TYPE *GContainer<TYPE>::prev(GPosition &pos) const {
...
template <class TYPE> GArray<TYPE>::~GArray() {
  for (int i = lobound; i <= hibound; i++) {
...
template <class TYPE> inline int GArray<TYPE>::size() const {
  return hibound - lobound + 1;
}

template <class TYPE> GPosition GArray<TYPE>::firstpos() const {
  GPosition pos;
  pos.cont = this;
  pos.pi = lobound;
  if (hibound < lobound) {
...
template <class TYPE> GPosition GArray<TYPE>::lastpos() const {
  GPosition pos;
  pos.cont = this;
  pos.pi = hibound;
  if (hibound < lobound) {
...
template <class TYPE> void GArray<TYPE>::nextpos(GPosition &pos) const {
  if (pos.cont == this && ++pos.pi <= hibound) {
    return;
  }
  pos.cont = 0;
}

template <class TYPE> void GArray<TYPE>::prevpos(GPosition &pos) const {
  if (pos.cont == this && --pos.pi >= lobound) {
    return;
  }
  pos.cont = 0;
}

template <class TYPE> inline TYPE *GArray<TYPE>::get(const GPosition &pos) {
...
template <class TYPE>
inline const TYPE *GArray<TYPE>::get(const GPosition &pos) const {
  return ((GArray<TYPE> *)(this))->get(pos);
}

template <class TYPE> inline GArray<TYPE>::operator TYPE *() {
  return &data[-minlo];
}

template <class TYPE> inline GArray<TYPE>::operator const TYPE *() const {
  return &data[-minlo];
}

template <class TYPE> inline TYPE &GArray<TYPE>::operator[](int n) {
...
template <class TYPE> inline const TYPE &GArray<TYPE>::operator[](int n) const {
  return ((GArray<TYPE> *)(this))->operator[](n);
}

template <class TYPE> inline TYPE &GArray<TYPE>::operator[](GPosition pos) {
  if (!(pos.cont == this && pos.pi >= lobound && pos.pi <= hibound)) {
    THROW("Illegal GArray subscript");
  }
  return data[pos.pi - minlo];
...
template <class TYPE>
inline const TYPE &GArray<TYPE>::operator[](GPosition pos) const {
  return ((GArray<TYPE> *)(this))->operator[](pos);
}

template <class TYPE> inline int GArray<TYPE>::lbound() const {
  return lobound;
}

template <class TYPE> inline int GArray<TYPE>::hbound() const {
  return hibound;
}

template <class TYPE> inline void GArray<TYPE>::empty() { resize(0, -1); }

template <class TYPE> inline void GArray<TYPE>::resize(int hi) {
  resize(0, hi);
}

template <class TYPE> inline void GArray<TYPE>::touch(int n) {
  if (hibound < lobound) {
    resize(n, n);
  } else {
    int nlo = lobound;
...
template <class TYPE> void GArray<TYPE>::resize(int lo, int hi) {
  int i;
  int nsize = hi - lo + 1;
  // Validation
  if (nsize < 0) {
...
template <class TYPE> void GArray<TYPE>::shift(int disp) {
  lobound += disp;
  hibound += disp;
  minlo += disp;
  maxhi += disp;
...
template <class TYPE> void GArray<TYPE>::del(int n, unsigned int howmany) {
  if (howmany == 0) {
    return;
  }
...
template <class TYPE>
void GArray<TYPE>::ins(int n, const TYPE &val, unsigned int howmany) {
  int i;
...
template <class TYPE>
GArray<TYPE> &GArray<TYPE>::operator=(const GArray<TYPE> &ga) {
  if (&ga == this) {
    return *this;
  }
  empty();
...
template <class TYPE> void GSArray<TYPE>::sort() {
  sort(this->lobound, this->hibound);
}

template <class TYPE> void GSArray<TYPE>::sort(int lo, int hi) {
  if (hi <= lo) {
    return;
  }
...
template <class TYPE> GList<TYPE>::GList() : nelem(0) {
  head.next = &head;
  head.prev = &head;
}

template <class TYPE>
GList<TYPE>::GList(const GContainer<TYPE> &gc) : nelem(0) {
  head.next = &head;
  head.prev = &head;
...
template <class TYPE> GList<TYPE>::GList(const GList<TYPE> &gc) : nelem(0) {
  head.next = &head;
  head.prev = &head;
  GPosition pos(gc);
  const TYPE *pdata;
...
template <class TYPE> GList<TYPE>::~GList() {
...
template <class TYPE> inline int GList<TYPE>::size() const { return nelem; }

template <class TYPE> GPosition GList<TYPE>::firstpos() const {
  GPosition pos;
...
template <class TYPE> GPosition GList<TYPE>::lastpos() const {
  GPosition pos;
...
template <class TYPE> void GList<TYPE>::nextpos(GPosition &pos) const {
  if (pos.cont == this) {
...
template <class TYPE> void GList<TYPE>::prevpos(GPosition &pos) const {
  if (pos.cont == this) {
...
template <class TYPE> inline TYPE *GList<TYPE>::get(const GPosition &pos) {
  if (pos.cont != this) {
    return 0;
  }
...
template <class TYPE>
inline const TYPE *GList<TYPE>::get(const GPosition &pos) const {
  return ((GList<TYPE> *)this)->get(pos);
}

template <class TYPE> inline int GList<TYPE>::isempty() const { return !nelem; }

template <class TYPE> inline TYPE &GList<TYPE>::operator[](GPosition pos) {
  TYPE *ref = get(pos);
  if (!ref) {
...
template <class TYPE>
inline const TYPE &GList<TYPE>::operator[](GPosition pos) const {
  return ((GList<TYPE> *)(this))->operator[](pos);
}

template <class TYPE> int GList<TYPE>::operator==(const GList<TYPE> &l2) const {
  if (nelem != l2.nelem()) {
    return 0;
  }
  GListNode *p = head.next;
...
template <class TYPE>
int GList<TYPE>::nth(unsigned int n, GPosition &pos) const {
  if ((int)n >= nelem) {
    return 0;
  }
  GListNode *p = head.next;
...
template <class TYPE> int GList<TYPE>::contains(const TYPE &elt) const {
  GPosition pos;
  return search(elt, pos);
}

template <class TYPE>
int GList<TYPE>::search(const TYPE &elt, GPosition &pos) const {
...
template <class TYPE> void GList<TYPE>::empty() {
  GListNode *node = head.next;
  while (node != &head) {
    GListNode *next = node->next;
    delete node;
...
template <class TYPE> void GList<TYPE>::append(const TYPE &elt) {
...
template <class TYPE> void GList<TYPE>::prepend(const TYPE &elt) {
  GListNode *node = new GListNode(elt);
  node->next = head.next;
  node->prev = &head;
  node->next->prev = node;
...
template <class TYPE>
GList<TYPE> &GList<TYPE>::operator=(const GList<TYPE> &gl) {
  if (&gl == this) {
    return *this;
  }
  empty();
...
template <class TYPE>
void GList<TYPE>::insert_after(GPosition pos, const TYPE &elt) {
  GListNode *ref = (GListNode *)(pos.pv);
...
template <class TYPE>
void GList<TYPE>::insert_before(GPosition pos, const TYPE &elt) {
  GListNode *ref = (GListNode *)(pos.pv);
  if (pos.cont != this || ref == &head) {
    THROW("Illegal GList subscript");
  }
...
template <class TYPE> void GList<TYPE>::del(GPosition &pos) {
  GListNode *ref = (GListNode *)(pos.pv);
  if (pos.cont == this && ref != &head) {
...
GMap<KTYPE, VTYPE>::GMap() : nbuckets(17), nelems(0) {
...
template <class KTYPE, class VTYPE> GMap<KTYPE, VTYPE>::~GMap() {
  empty();
  delete[] table;
}

template <class KTYPE, class VTYPE>
inline int GMap<KTYPE, VTYPE>::size() const {
  return nelems;
}

template <class KTYPE, class VTYPE>
GPosition GMap<KTYPE, VTYPE>::firstpos() const {
  GPosition pos;
  pos.pi = 0;
  pos.pv = table[0];
...
template <class KTYPE, class VTYPE>
GPosition GMap<KTYPE, VTYPE>::lastpos() const {
...
template <class KTYPE, class VTYPE>
void GMap<KTYPE, VTYPE>::nextpos(GPosition &pos) const {
  if (pos.cont == this) {
...
template <class KTYPE, class VTYPE>
void GMap<KTYPE, VTYPE>::prevpos(GPosition &pos) const {
  THROW("GMap does not support backward iteration");
}

template <class KTYPE, class VTYPE>
inline VTYPE *GMap<KTYPE, VTYPE>::get(const GPosition &pos) {
  GMapNode *n = (GMapNode *)(pos.pv);
...
template <class KTYPE, class VTYPE>
inline const VTYPE *GMap<KTYPE, VTYPE>::get(const GPosition &pos) const {
  return ((GMap<KTYPE, VTYPE> *)this)->get(pos);
}

template <class KTYPE, class VTYPE>
inline const KTYPE &GMap<KTYPE, VTYPE>::key(const GPosition &pos) const {
  GMapNode *n = (GMapNode *)(pos.pv);
  if (!(pos.cont == this && n != 0)) {
...
template <class KTYPE, class VTYPE>
inline int GMap<KTYPE, VTYPE>::isempty() const {
  return !nelems;
}

template <class KTYPE, class VTYPE>
int GMap<KTYPE, VTYPE>::contains(const KTYPE &key, GPosition &pos) const {
...
template <class KTYPE, class VTYPE>
int GMap<KTYPE, VTYPE>::contains(const KTYPE &key) const {
  GMapNode *n = (GMapNode *)search(key, 0);
  return (n != 0);
}

template <class KTYPE, class VTYPE> void GMap<KTYPE, VTYPE>::empty() {
  GMapNode *p;
  nelems = 0;
  for (int i = 0; i < nbuckets; i++)
    while ((p = table[i])) {
...
template <class KTYPE, class VTYPE>
inline VTYPE &GMap<KTYPE, VTYPE>::operator[](const GPosition &pos) {
  VTYPE *ret = get(pos);
  if (!ret) {
    THROW("Illegal GMap subscript");
  }
...
template <class KTYPE, class VTYPE>
const VTYPE &GMap<KTYPE, VTYPE>::operator[](const GPosition &pos) const {
  return ((GMap<KTYPE, VTYPE> *)this)->operator[](pos);
}

template <class KTYPE, class VTYPE>
VTYPE &GMap<KTYPE, VTYPE>::operator[](const KTYPE &key) {
  GMapNode *n = (GMapNode *)search_create(key, 0);
  return n->val;
}

template <class KTYPE, class VTYPE>
const VTYPE &GMap<KTYPE, VTYPE>::operator[](const KTYPE &key) const {
  GMapNode *n = (GMapNode *)search(key, 0);
  if (!n) {
    THROW("Cannot create key into const GMap");
  }
  return n->val;
}

template <class KTYPE, class VTYPE>
void GMap<KTYPE, VTYPE>::del(GPosition &pos) {
  GMapNode *n = (GMapNode *)(pos.pv);
  if (pos.cont == this && n) {
    if (n->next) {
...
template <class KTYPE, class VTYPE>
void GMap<KTYPE, VTYPE>::del(const KTYPE &key) {
  GMapNode *n = (GMapNode *)search(key, 0);
  if (n) {
    if (n->next) {
      n->next->pprev = n->pprev;
...
template <class KTYPE, class VTYPE>
void GMap<KTYPE, VTYPE>::rehash(int newsize) {
  typedef GMapNode *PGMN;
...
template <class KTYPE, class VTYPE>
void *GMap<KTYPE, VTYPE>::search(const KTYPE &key, int *pbucket) const {
...
template <class KTYPE, class VTYPE>
void *GMap<KTYPE, VTYPE>::search_create(const KTYPE &key, int *pbucket) {
  unsigned int hashcode = hash(key);
  int bucket = hashcode % nbuckets;
  GMapNode *n;
  for (n = table[bucket]; n; n = n->next)
