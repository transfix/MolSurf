...
/** @name GString.h
    ...
    @version
    #$Id: GString.h,v 1.1.1.1 2008/06/06 19:42:10 wzhao Exp $# */
//@{
...
public:
  ...
  // -- COMPARISONS
  /** String comparison. Returns true if and only if character strings #s1# and
     #s2# are equal (as with #strcmp#.) */
  ...
  /** String comparison. Returns true if and only if character strings #s1# and
     #s2# are not equal (as with #strcmp#.) */
  friend int operator!=(const GString &s1, const GString &s2) {
    return strcmp(s1, s2) != 0;
  }
  friend int operator!=(const GString &s1, const char *s2) {
...
inline GString::GString(GStringRep *rep) : GP<GStringRep>(rep) {}

inline GString::GString(const GString &gs) : GP<GStringRep>(gs) {}

inline GString &GString::operator=(GStringRep *rep) {
  GP<GStringRep>::operator=(rep);
  return *this;
}

inline GString &GString::operator=(const GString &gs) {
  GP<GStringRep>::operator=(gs);
  return *this;
}

inline GString GString::substr(int from, unsigned int len) const {
  return GString((GString &)*this, from, len);
}

inline char GString::operator[](int n) const {
  if (n < 0) {
    n += (*this)->size;
  }
...
inline GString &GString::operator+=(char ch) {
  setat((*this)->size, ch);
  return *this;
}
