...
public:
  ...
  SimpleVolumeData *createDepthColoredVolume(double *colorMap,
                                             int colorMapSize);
  ...
  static bool getMinMax(unsigned char *data, int width, int height, int depth,
                        unsigned char *minVal, unsigned char *maxVal);
  static bool getMinMax(unsigned short *data, int width, int height, int depth,
                        unsigned short *minVal, unsigned short *maxVal);
  static bool getMinMax(unsigned long *data, int width, int height, int depth,
                        unsigned long *minVal, unsigned long *maxVal);
  static bool getMinMax(float *data, int width, int height, int depth,
                        float *minVal, float *maxVal);
  static bool getMinMax(double *data, int width, int height, int depth,
                        double *minVal, double *maxVal);
  static bool getUnsignedCharData(void *data1, int type, unsigned char *data,
                                  int width, int height, int depth);
  static bool getNormalizedUnsignedCharData(void *data1, int type,
                                            unsigned char *data, int width,
                                            int height, int depth);
  static bool getUnsignedCharData(void *data1, int type1, void *data2,
                                  int type2, void *data3, int type3,
                                  void *data4, int type4, unsigned char *data,
                                  int width, int height, int depth);
  bool getFloatData(float **fVolume, unsigned int variable);
  bool getFloatData(float **fVolume);
  bool addVolume(SimpleVolumeData *sData, double scale, double sum);

protected:
  void setDefaults();
  double getDistanceOfVoxel(int i, int j, int k, int width, int height,
                            int depth);
