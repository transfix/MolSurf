...
{
	public:
		OTVertex(float a, float b, float c){xyz[0] = a; xyz[1] = b; xyz[2] = c; value = 0.0; valueReady = false;}
		~OTVertex();
...
class OTCell 
{
	public:
		...
		OTCell(){isLeaf = true; splitRequired = false; for(int i=0; i<8; i++) {vertices[i] = -1; children[i] = -1; span = 0.0;} }
		OTCell(int* vs, float a, float b, float c, float sp, int d) { isLeaf = true; splitRequired = false; xyz[0] = a; xyz[1] = b; xyz[2] = c; span = sp; depth = d; for(int i=0; i<8; i++) {vertices[i] =vs[i]; children[i] = -1;} }
};

class MeshVertex
{
	public:
		MeshVertex(float x, float y, float z){xyz[0] = x; xyz[1] = y; xyz[2] = z;}
		float xyz[3];
};
...
{
	public:
		MeshSimplex(float a, float b, float c){verts[0] = a; verts[1] = b; verts[2] = c;}
		int verts[3];
};
...
//		int depth;
	...
	public:
		Octree(std::string pqrFN,  std::string rawFN, int minDepth, int maxDepth, float blobbyness, float isovalue);
};
