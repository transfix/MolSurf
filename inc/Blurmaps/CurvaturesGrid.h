...
public:
  ...
  bool getCurvatures(double *HandK, double *normals, double *k1Vec,
                     double *k2Vec, PDBParser::GroupOfAtoms *molecule,
                     int numberOfPoints, float *points, double blobbyness,
                     int numberOfGridDivisions, double maxFunctionError,
                     int radiusType, unsigned int level);
  bool write(int numberOfPoints, double *HandK, double *normals, double *k1Vec,
             double *k2Vec, const char *filename);

protected:
  void createGrid(PDBParser::GroupOfAtoms *molecule, int numberOfPoints,
                  float *points, double blobbyness, int numberOfGridDivisions,
                  double maxFunctionError, int radiusType, unsigned int level);
  void fillAtomsRecursively(PDBParser::GroupOfAtoms *molecule,
                            double overlapExtent, int radiusType,
                            unsigned int level);
  void getMinMax(PDBParser::GroupOfAtoms *molecule, int numberOfPoints,
                 float *points);
