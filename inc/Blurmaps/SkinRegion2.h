...
public:
  ...
  bool getSkinRegion(PDBParser::GroupOfAtoms *molecule, int dim1, int dim2,
                     int dim3, double probeRadius,
                     PDBParser::GroupOfAtoms::RADIUS_TYPE radiusType,
                     SimpleVolumeData *vol, int depth, const char *cmapFile = 0,
                     bool withColor = false,
                     PDBParser::GroupOfAtoms::GOA_TYPE colorLevel =
                         PDBParser::GroupOfAtoms::ATOM);

protected:
  bool getColoredAtomList(PDBParser::GroupOfAtoms *molecule,
                          const char *cmapFile,
                          PDBParser::GroupOfAtoms::GOA_TYPE colorLevel);
  ...
  double inline getDist(double x1, double y1, double z1, double x2, double y2,
                        double z2);
  ...
  void getSphereRadiusCenter(PDBParser::Atom *at, double *radius,
                             double *center);
  double getDistToSas(int vertx, int verty, int vertz, int neighborx,
                      int neighborj, int neighbork, unsigned char *red,
                      unsigned char *green, unsigned char *blue, bool color);
  double getDistToSas2(int vertx, int verty, int vertz, int neighborx,
                       int neighbory, int neighborz);
  void getColors(PDBParser::Atom *atom, unsigned char *red,
  ...
  ////   Also, one of the boolean arrays is smaller than this ( contains cells,
  /// not vertices! )
