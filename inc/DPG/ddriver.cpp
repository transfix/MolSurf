...
int main(int argc, char **argv) {
  ...
  while (index != -1) {
    ...
    case '3': { /* Insert element */
      ...
      if (DETAIL > 1) {
        if (found)
          ...
          printf("%d already exists in the hash\n", key);
      }
      break;
    }
    case '4': { /* Lookup element */
      ...
      if (DETAIL >= 1) {
        if (found)
          printf("%d found in hash\n", key);
        else
          printf("%d not found in hash\n", key);
      }
      break;
    }
    case '5': { /* Delete element */
      ...
      if (DETAIL >= 1) {
        if (found)
          printf("%d deleted from hash\n", key);
        else
          printf("%d not found to be deleted\n", key);
      }
      break;
    }
