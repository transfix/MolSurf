...
public:
  ...
  bool operator==(const tuple<T> &that) const { return (id == that.id); }
  bool operator<(const tuple<T> &that) const { return id < that.id; }
  bool operator>(const tuple<T> &that) const { return id > that.id; }
  ...
  bool operator!=(const tuple<T> &that) const { return !(operator==(that)); }
...
template <typename T> ostream &operator<<(ostream &ost, const tuple<T> &p) {
  string end = "";
  ost << p.id << " " << p.ptr;
  return ost << end;
}
...
template <class T> class IntegerRange {
  ...
  void printReps() {
    ...
    for (int i = 0; i < num_clusters; i++)
      cout << "Representative for tree # " << i + 1 << " is " << repMap[i]
           << endl;
...
public:
  ...
  void remove(int k) {
    ...
    /*
    If removing k from BST[next_clus] makes it empty, do the following and
    return:
