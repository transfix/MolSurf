...
private:
  ...
  void mergeSortedLists(vector<Comparable> &t1, vector<Comparable> &t2,
  ...
  /*traverses the tree rooted at 't' in inorder and stores the sorted list in
   * the given vector*/
  void Inorder(vector<Comparable> &inorder_vector,
...
public:
  ...
  BinarySearchTree(const BinarySearchTree<Comparable> &rhs)
      : num_nodes(rhs.num_nodes), root(NULL),
  ...
  /*Merge two binary search tree and merged tree is stored in this; the other
   * tree remains unaltered*/
