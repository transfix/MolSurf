...
/* $Id: fftw3.h,v 1.1 2008/06/13 20:25:53 albertc Exp $ */
...
#define FFTW_DEFINE_API(X, R, C)                                               \
  ...
  X(plan)                                                                      \
  X(plan_dft_3d)                                                               \
  (int nx, int ny, int nz, C *in, C *out, int sign, unsigned flags);           \
                                                                               \
  X(plan)                                                                      \
  X(plan_many_dft)                                                             \
  (int rank, const int *n, int howmany, C *in, const int *inembed,             \
   int istride, int idist, C *out, const int *onembed, int ostride, int odist, \
  ...
  X(plan_many_dft_c2r)                                                         \
  (int rank, const int *n, int howmany, C *in, const int *inembed,             \
   int istride, int idist, R *out, const int *onembed, int ostride, int odist, \
   unsigned flags);                                                            \
                                                                               \
  X(plan)                                                                      \
  ...
  X(plan)                                                                      \
  X(plan_r2r)                                                                  \
  (int rank, const int *n, R *in, R *out, const X(r2r_kind) * kind,            \
   unsigned flags);                                                            \
                                                                               \
  X(plan)                                                                      \
  ...
  X(plan)                                                                      \
  X(plan_r2r_2d)                                                               \
  (int nx, int ny, R *in, R *out, X(r2r_kind) kindx, X(r2r_kind) kindy,        \
   unsigned flags);                                                            \
  X(plan)                                                                      \
  X(plan_r2r_3d)                                                               \
  (int nx, int ny, int nz, R *in, R *out, X(r2r_kind) kindx,                   \
   X(r2r_kind) kindy, X(r2r_kind) kindz, unsigned flags);                      \
                                                                               \
  X(plan)                                                                      \
  X(plan_guru_r2r)                                                             \
  (int rank, const X(iodim) * dims, int howmany_rank,                          \
   const X(iodim) * howmany_dims, R * in, R * out, const X(r2r_kind) * kind,   \
   unsigned flags);                                                            \
