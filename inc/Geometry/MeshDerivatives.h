...
public:
  MeshDerivatives(unsigned int numTris, unsigned int numTriVerts,
                  unsigned int *tris, float *triVerts, float *m_TriVertNormals,
                  float *triMeanCurv, float *triGaussianCurv, float *k1,
                  float *k2);
...
protected:
  ...
  bool findVerts(unsigned int triIndex, unsigned int vertIndex,
                 unsigned int *vleft, unsigned int *vright);
  double areaOfInfinitesimalTriangle(int triangleIdx, unsigned int vertIndex,
                                     unsigned int vleft, unsigned int vright);
  double getEdgeLength(unsigned int v1, unsigned int v2);
  double voronoiArea(unsigned int vertIndex, unsigned int vleft,
  ...
  double getTriangleArea(unsigned int vertIndex, unsigned int vleft,
                         unsigned int vright);
  bool computeAngles();
  void getNeighbors(unsigned int vertIndex,
                    vector<unsigned int> *neighboringVertices);
  bool getLaplaceBeltramiOperator(double *Kx, double *Ky, double *Kz,
                                  unsigned int vertIndex, double A_mixed);
  void unionVertex(unsigned int v, vector<unsigned int> *neighboringVertices);
  void getNeighborsAngles(unsigned int x_i, unsigned int x_j, double *alpha,
