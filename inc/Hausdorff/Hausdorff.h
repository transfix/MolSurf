...
/**
Class for computing the Hausdorff distance and the average distance between two point clouds.

We assume that the inputs are meshes in the form of RAW or RAWN file format. The distance is computed based on the vertices; simplices are ignored.
...
3. We define hausdorffDist(X,Y) = max(oneWayHausdorffDist(X,Y), oneWayHausdorffDist(Y,X))
4. We define oneWayAverageDist(X,Y) = avg_{x \in X} minDist(x,Y)
4. We define averageDist(X,Y) = avg(oneWayAverageDist(X,Y), oneWayAverageDist(Y,X))
