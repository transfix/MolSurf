...
public:
  ...
  double thetaA; // angle subtended by the cone from center of atom A onto the
                 // circle
  double thetaB; // angle subtended by the cone from center of atom B onto the
                 // circle
};
...
public:
  ...
  Atom() {
    copyDisabled = false;
    pairIntersections.clear();
    tripleIntersections.clear();
  }

  double dist(Atom *a) {
    return sqrt((xyz[0] - a->xyz[0]) * (xyz[0] - a->xyz[0]) +
                (xyz[1] - a->xyz[1]) * (xyz[1] - a->xyz[1]) +
                (xyz[2] - a->xyz[2]) * (xyz[2] - a->xyz[2]));
  }
  double pointDist(double x, double y, double z, double R) {
    return ((xyz[0] - x) * (xyz[0] - x) + (xyz[1] - y) * (xyz[1] - y) +
            (xyz[2] - z) * (xyz[2] - z) - (r + R) * (r + R));
  }
  bool intersect(Atom *a, double solventRad) {
    return dist(a) < (r + a->r + 2 * solventRad);
  }
...
public:
  ...
  void getInterfaceArea(OTNode *otherNode, double *trans, double interfaceWidth,
                        double &areaInner, double &areaOuter, double isoValue);
...
public:
  ...
  double getInterfaceArea(Octree *otherTree, double *trans,
                          double interfaceWidth);
  double getInterfaceAreaUnderRoot(OTNode *root, Octree *otherTree,
                                   double *trans, double interfaceWidth);

  void GetAffectedCells(Atom *a, std::vector<OTNode *> &intersectedCells);
  void LocateAffectedCells(OTNode *nd, Atom *a,
                           std::vector<OTNode *> &intersectedCells);
  ...
  bool getPairwiseIntersection(int atomA, int atomB, double x, double y,
                               double z, double solventRad,
                               PairIntersection *PI);
  bool getSDFForIntersectingSpheres(PairIntersection *PI, double x, double y,
                                    double z, double solventRad, double &sdf);

  bool computeTripleIntersection(int atomA, int atomB, int atomC, double x,
                                 double y, double z, double solventRad,
                                 TripleIntersection *TI);
  bool getSDFForTripleIntersect(TripleIntersection *TI, double x, double y,
                                double z, double &sdf);
