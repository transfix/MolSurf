...
*/
#if !defined(                                                                  \
    AFX_MULTICONTOUR_H__9DC23294_BACA_494B_AF88_C41B94D54D4F__INCLUDED_)
...
private:
  ...
  public:
    MultiContourNode(int ID, float isovalue, float R, float G, float B,
                     bool wire, MultiContourNode *next)
        : m_Next(next) {
      m_Contour.setID(ID);
    ...
    virtual ~MultiContourNode() { delete m_Next; };
