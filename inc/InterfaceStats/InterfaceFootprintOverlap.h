...
public:
  InterfaceFootprintOverlap(string SurfaceFileName1, string SurfaceFileName2,
                            string SurfaceFileName3, double interfaceWidth,
                            bool verbose = false);
  ...
  // if genSurfMode = true, then it prepares new raw files where the interfaces
  // and overlaps are highlighted. Otherwise simply computes the areas. poseId =
  // id/rank of the pose corresponding to the transformation (it is only used to
  // name a file, use dummy value if the rank is not available)
  ...
  double getAreaOfReceptorInterfaceWithStaticLigand() {
    return staticInterfaceComputed ? areaOfReceptorInterfaceWithLigand1 : 0.0;
  }
  double getAreaOfReceptorInterfaceWithMovingLigand() {
    return movingInterfaceComputed ? areaOfReceptorInterfaceWithLigand2 : 0.0;
  }
  double getAreaOfStaticLigandInterface() {
    return staticInterfaceComputed ? areaOfLigandInterface1 : 0.0;
  }
  double getAreaOfMovingLigandInterface() {
    return movingInterfaceComputed ? areaOfLigandInterface2 : 0.0;
  }
  double getAreaOfOverlap() {
    return movingInterfaceComputed ? areaOfOverlap : 0.0;
  }
