...
public:
  ...
  MultiBodyGraph(ScoreWeight *nodeWeights, ScoreWeight *edgeWeights,
                 ScoreComparator *nodeComp, ScoreComparator *edgeComp,
                 bool verbose = false);
  ~MultiBodyGraph();
...
public:
  ...
  bool operator()(const MultiBodyNode *lhs, const MultiBodyNode *rhs) const {
    if (verbose)
      std::cout << "Comparing nodes " << lhs->getId() << " and " << rhs->getId()
                << std::endl;
