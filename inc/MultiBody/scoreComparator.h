...
enum { WEIGHTEDSUM, MAJORITY, LEXICOGRAPHIC, GROUPWEIGHTEDSUM, HIERARCHICAL };
...
class HierarchicalWeight
{
        ...
        public:
                HierarchicalWeight(ScoreWeight* w) {weightsForThisLayer = w;
isLeaf = true;}

                ~HierarchicalWeight() {weightsForNextLayer.clear();}

                void setWeightsForThisLayer(ScoreWeight* w) {
weightsForThisLayer = w; }

                void addWeightsForNextLayer(HierarchicalWeight* w) {
weightsForNextLayer.push_back(w); isLeaf = false;}
                ...
                ScoreWeight* getWeightForThisLayer() { return
weightsForThisLayer; }
}
*/
...
public:
  ...
  int compare(const Score *S1,
              const Score *S2) const; // 0 = equal, -1 = S1 < S2, +1 = S1 > S2

  bool operator()(const Score *lhs, const Score *rhs)
      const // lets us use the comparator in sorted data structures like sets,
            // priority queues etc.
  {
