...
*/
// Commonly used geometric primitives
...
struct Point3f {
  ...
  const float operator[](int i) const { return pnt[i]; }

  float &operator[](int i) { return pnt[i]; }
  ...
  friend bool operator==(const Point3f &p1, const Point3f &p2) {
    return (p1.pnt[0] == p2.pnt[0] && p1.pnt[1] == p2.pnt[1] &&
            p1.pnt[2] == p2.pnt[2]);
  }

  friend const Point3f operator-(const Point3f p1, const Point3f p2) {
    return Point3f(p1.pnt[0] - p2.pnt[0], p1.pnt[1] - p2.pnt[1],
                   p1.pnt[2] - p2.pnt[2]);
  }
...
struct Point4f {
  ...
  bool operator==(const Point4f &p) const {
    return (pnt[0] == p.pnt[0] && pnt[1] == p.pnt[1] && pnt[2] == p.pnt[2] &&
            pnt[3] == p.pnt[3]);
  }
};
...
struct TriId3i {
  ...
  const unsigned int operator[](int i) const { return id[i]; }

  unsigned int &operator[](int i) { return id[i]; }
};
...
struct BoundingBox2 {
  ...
  BoundingBox2(float lx = -1, float ly = -1, float lz = -1, float ux = 1,
               float uy = 1, float uz = 1)
      : lower(lx, ly, lz), upper(ux, uy, uz) {}

  BoundingBox2 &operator=(const BoundingBox2 &box) {
    if (this == &box) {
      return *this;
    }
    lower = box.lower;
