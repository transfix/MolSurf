...
#include <SignDistanceFunction_v2/cubes.h>

// Reg3Data: A class representing a scalar field on a regular 3D grid.
// This class is based on many previous versions of regular 3D data class.
template <class T> class Reg3Data {
public:
  ...
  Reg3Data(int dim[3], T *array) {
    ...
    p_grad = NULL;
    if (array) {
      memcpy(p_data, array, sizeof(T) * dim[0] * dim[1] * dim[2]);
    }
    init();
  }
  ...
  int getNVerts() const { return m_dim[0] * m_dim[1] * m_dim[2]; }
  ...
  float operator[](int nv) const { return getValue(nv); }
  ...
  bool isBoundary(int v) {
    int x, y, z;
    vert2index(v, x, y, z);
    if ((x <= 0 || x >= m_dim[0] - 1) || (y <= 0 || y >= m_dim[1] - 1) ||
        (z <= 0 || z >= m_dim[2] - 1)) {
      return true;
    }
    return false;
  ...
  int index2vert(int i, int j, int k) const {
    if (((i < 0) || (i >= m_dim[0])) || ((j < 0) || (j >= m_dim[1])) ||
        ((k < 0) || (k >= m_dim[2]))) {
      return -1;
    }
  ...
  int index2cell(int i, int j, int k) const {
    if (((i < 0) || (i >= m_dim[0] - 1)) || ((j < 0) || (j >= m_dim[1] - 1)) ||
        ((k < 0) || (k >= m_dim[2] - 1))) {

      cout << i << " " << j << " " << k << endl;
      cout << m_dim[0] << " " << m_dim[1] << " " << m_dim[2] << " " << endl;
      return -1;
    }
...
private:
  bool m_dirty;
  bool hasGradient() const { return (p_grad != NULL); }
};

template <class T> void Reg3Data<T>::init() {
  m_fmin = m_fmax = p_data[0];
  int i, n = m_dim[0] * m_dim[1] * m_dim[2];
  for (i = 1; i < n; i++) {
    if (p_data[i] < m_fmin) {
      m_fmin = p_data[i];
    } else if (p_data[i] > m_fmax) {
      m_fmax = p_data[i];
    }
  }
  m_dirty = false;
...
template <class T> void Reg3Data<T>::getVertGrad(int v, float grad[3]) const {
  assert(hasGradient());
...
template <class T> int Reg3Data<T>::getNeighborVtx(int vtx, int *neighbor_vtx) {
  int i, j, k, idx;
  int nbr_idx = 0;
  int nidx = 0;
  int nbr_vtx[100];
...
template <class T> void Reg3Data<T>::calcGradient(int mode) {
  if (hasGradient()) {
    return;
  }
      ...
      for (j = 0; j < m_dim[1]; j++) {
        for (i = 0; i < m_dim[0]; i++) {
          ...
          if (j == 0) {
            ...
            p_grad[vid][1] =
                (getValue(vid + m_dim[0]) - getValue(vid - m_dim[0])) / 2;
          }
          if (k == 0) {
            p_grad[vid][2] =
                getValue(vid + m_dim[0] * m_dim[1]) - getValue(vid);
          } else if (k == m_dim[2] - 1) {
            p_grad[vid][2] =
                getValue(vid) - getValue(vid - m_dim[1] * m_dim[0]);
          } else {
            p_grad[vid][2] = (getValue(vid + m_dim[1] * m_dim[0]) -
                              getValue(vid - m_dim[1] * m_dim[0])) /
                             2;
          }
