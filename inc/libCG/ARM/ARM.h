...
private:
  ...
  std::vector<PDBParser::Atom *> arm[2]; // the atomic level and the first level
                                         // of adaptive resolution model
...
public:
  ...
  static void getAtomList(std::vector<PDBParser::Atom *> &atomList,
                          const char *fname);
  ...
  void printAtomList(std::vector<PDBParser::Atom *> &atomList,
                     char *outfilename);
  void getOptimizedCG(std::vector<PDBParser::Atom *> &cg, int size,
