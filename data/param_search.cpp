...
void getAreaAndVolume(string fn, double b, double iso, double &area,
                      double &volume) {
  ostringstream cmd;
  cmd << "../MolSurf -getSurfaceUsingGaussianBlur " << " PDB/" << fn
      << ".pdb Temp/" << fn << "_gb.raw " << iso << " " << gridSize << " "
      << gridSize << " " << gridSize << " " << b << " 0 1 false";
...
void bestParams(string fn, double &b, double &iso) {
  ...
  // compute adaptive grid surface
  ostringstream cmd;
  cmd << "../MolSurf -getSurfaceUsingAdaptiveGrid PDB/" + fn + ".pdb Temp/" +
             fn + "_ag.raw "
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if ((ofl > 0.0 && ofs > 0.0) || (ofl < 0.0 && ofs < 0.0)) {
