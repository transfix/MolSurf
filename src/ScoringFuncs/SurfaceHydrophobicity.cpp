...
SurfaceHydrophobicity::SurfaceHydrophobicity(string pqrFileName, string quadFileName)
{
	dataReady = false;
...
double SurfaceHydrophobicity::getSurfaceHydrophobicity()
{
	...
	{
		ip = getHydrophobicity( atms[i]->getName(), atms[i]->getResidueName(), hydroIndex, nHydroIndex, true, true );
		...
		{
			totalSurfaceHydrophobicity += ((DPG::WeightedPoint*)quadsInRange[j])->getWeight() * ip;
		}
...
float SurfaceHydrophobicity::getHydrophobicity( const char *atomName, const char *residueName, int *index, int nIndex, bool useInterfacePropensity, bool perResidueHydrophobicity )
{
	...
	{ 
		...
		{
			int k = ( i == nIndex - 1 ) ? PDBParser::MAX_BIOCHEM_ELEMENTS : index[ i + 1 ];
...
void SurfaceHydrophobicity::preprocessElementInformationTable( int **index, int *nIndex )
{
	int c = 1;
	int n = sizeof( PDBParser::elementTable ) / sizeof( PDBParser::elementTable[ 0 ] );

	for ( int i = 1; i < n; i++ )
		if ( strcmp( PDBParser::elementTable[ i ].residueName, PDBParser::elementTable[ i - 1 ].residueName ) ) 
			c++;
...
void SurfaceHydrophobicity::computeXlateForPG( )
{
	...
	minXYZ = minX;
	if(minXYZ > minY) minXYZ = minY;
	if(minXYZ > minZ) minXYZ = minZ;
	...
	if(minXYZ < 0) xlatePG = -2.0 * minXYZ;
}
...
bool SurfaceHydrophobicity::readPQR(string pqrFileName)
{
	...
	PDBParser::FlattenGOA(molecule, atms, collectionData, 0, 0, 0, radiusType, PDBParser::ATOM_TYPE, false);
...
bool SurfaceHydrophobicity::readQuad(string quadFileName)
{
	...
	while ( fscanf( fp, (char *)"%lf %lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx, &ny, &nz, &w ) == 7 ) 
	{
