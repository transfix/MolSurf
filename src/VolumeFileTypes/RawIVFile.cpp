...
SimpleVolumeData *RawIVFile::loadFile(const string &fileName) {
  ...
  unsigned int dims[3];
  unsigned int temp_numverts,
      temp_numcells; // for large files, these are meaningless, and anyway not
                     // used.
  ...
  if (dataSize % testSize) {
    error("Error: rawiv file " + fileName +
          " header dimensions don't match file size");
  }
  ...
  unsigned char *data =
      (unsigned char *)malloc((Q_ULLONG)sizeof(unsigned char) * dataSize);
  // read the data
  ...
  if (isLittleEndian()) {
    ...
    case 1:
      break;
    case 2:
      swapByteOrder((unsigned short *)data,
                    (Q_ULLONG)dims[0] * dims[1] * dims[2]);
      break;
    case 4:
...
bool RawIVFile::checkType(const string &fileName) { return false; }

bool RawIVFile::saveFile(SimpleVolumeData *simpleVolumeData,
  ...
  // write the data
  fwrite(simpleVolumeData->getData(variable),
         simpleVolumeData->getDataSize(variable), 1, fp);
