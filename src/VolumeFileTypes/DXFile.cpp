...
void DXFile::tryToGetDimensions(bool *dimsSet, unsigned int *dims,
                                const char *line) {
  ...
  //  set dims to 'B', 'C', 'D'
  int dummy;
  if (sscanf(line, "object %d class gridpositions counts %d %d %d", &dummy,
             &(dims[0]), &(dims[1]),
...
void DXFile::tryToGetSpans(int *deltasFound, float *span, const char *line) {
  ...
  if (sscanf(line, "delta %lf %lf %lf", &(deltas[0]), &(deltas[1]),
             &(deltas[2])) != EOF) {
...
void DXFile::tryToGetDataHeader(bool *dataHeaderFound, unsigned int *dims,
                                const char *line) {
  ...
  char type[256];
  if (sscanf(line,
             "object %d class array type %s rank %d items %d data follows",
...
bool DXFile::checkType(const string &fileName) { return false; }

bool DXFile::saveFile(SimpleVolumeData *simpleVolumeData,
  ...
  if (simpleVolumeData->getType(variable) == SimpleVolumeData::ULONG ||
      simpleVolumeData->getType(variable) == SimpleVolumeData::DOUBLE) {
    return false;
  }
  ...
  fprintf(fp, "object 1 class gridpositions counts %d %d %d\n",
          simpleVolumeData->getWidth(), simpleVolumeData->getHeight(),
          simpleVolumeData->getDepth());
  fprintf(fp, "origin %e %e %e\n", simpleVolumeData->getMinX(),
          simpleVolumeData->getMinY(), simpleVolumeData->getMinZ());
  ...
  fprintf(fp, "object 2 class gridconnections counts %d %d %d\n",
          ...
          simpleVolumeData->getDepth());
  int dataIndex = 3;
  int size = simpleVolumeData->getWidth() * simpleVolumeData->getHeight() *
             simpleVolumeData->getDepth();
...
VolumeFileType *DXFile::getRepresentative() { return &ms_DXFileRepresentative; }
