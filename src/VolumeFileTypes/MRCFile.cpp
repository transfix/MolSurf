...
bool MRCFile::interpretNewHeader(MrcHeader &header, unsigned int fileSize,
  ...
  if (header.xlength <= 0.0 || header.ylength <= 0.0 || header.zlength <= 0.0 ||
      !finite(header.xlength) || !finite(header.ylength) ||
      !finite(header.zlength)) {
    // hmm, this is wierd
...
bool MRCFile::interpretOldHeader(MrcHeader &header, unsigned int fileSize,
                                 SimpleVolumeData *simpleVolumeData) {
  unsigned int dims[3];
  dims[0] = header.nx;
  dims[1] = header.ny;
...
bool MRCFile::checkHeader(MrcHeader &header, unsigned int fileSize) {
  ...
  // check the fileSize
  if (sizes[header.mode] * header.nx * header.ny * header.nz + 1024 !=
      fileSize) {
...
bool MRCFile::readHeader(FILE *fp, int fileSize, bool *swap,
                         SimpleVolumeData *simpleVolumeData) {
  MrcHeader header;
  freadSafely(&header, sizeof(MrcHeader), 1, fp);
...
SimpleVolumeData *MRCFile::loadFile(const string &fileName) {
  std::cout << "MRCFile loadFile is used" << std::endl;
  ...
  int numVerts = simpleVolumeData->getWidth() * simpleVolumeData->getHeight() *
                 simpleVolumeData->getDepth();
  int dataSize = simpleVolumeData->getTypeSize(0) * numVerts;
  if (dataSize < 1) {
    delete simpleVolumeData;
    simpleVolumeData = 0;
    return 0;
...
bool MRCFile::checkType(const string &fileName) { return false; }

bool MRCFile::fillHeader(MrcHeader &header,
                         SimpleVolumeData *simpleVolumeData) {
  ...
  if (!simpleVolumeData->getMinMax(0, &minVal, &maxVal)) {
    header.amin = 0.0; // minimum density value
    header.amax = 0.0; // maximum density value
  } else {
    header.amin = (float)(minVal);
    header.amax = (float)(maxVal);
  }
  // mean just put 0.0
...
bool MRCFile::saveFile(SimpleVolumeData *simpleVolumeData,
                       const string &fileName, unsigned int variable) {
