 ...
 * are as follows:
 * For the constraint a<=x[i]<=b: 4*(2*x[i]-(a+b))/(b-a)^2 if x[i] not in [a,
 *b], 0 otherwise For the constraint a<=x[i]: -1 if x[i]<=a, 0 otherwise For the
 *constraint x[i]<=b: 1 if b<=x[i], 0 otherwise
 *
...
static void LMBLEC_JACF(LM_REAL *p, LM_REAL *jac, int m, int n, void *adata) {
  ...
  for (i = nn, j = 0; i < n; ++i, ++j) {
    ...
    case __BC_INTERVAL__:
      ...
      tmp = ub[j] - lb[j];
      tmp =
          LM_CNST(4.0) * (LM_CNST(2.0) * p[j] - (lb[j] + ub[j])) / (tmp * tmp);
      jac[i * m + j] = w[j] * tmp;
      break;
 ...
 * measurements vector x under box & linear constraints. More precisely, given a
 * vector function  func : R^m --> R^n with n>=m, it finds p s.t. func(p) ~= x,
 * i.e. the squared second order (i.e. L2) norm of e=x-func(p) is minimized
 * under the constraints lb[i]<=p[i]<=ub[i] and A p=b; A is kxm, b kx1. Note
 * that this function DOES NOT check the satisfiability of the specified box and
 * linear equation constraints. If no lower bound constraint applies for p[i],
 * use -DBL_MAX/-FLT_MAX for lb[i]; If no upper bound constraint applies for
 * p[i], use DBL_MAX/FLT_MAX for ub[i].
 *
 * This function requires an analytic Jacobian. In case the latter is
 * unavailable, use LEVMAR_BLEC_DIF() bellow
 *
 * Returns the number of iterations (>=0) if successful, LM_ERROR if failed
 *
 * For more details on the algorithm implemented by this function, please refer
 * to the comments in the top of this file.
 *
 */
int LEVMAR_BLEC_DER(
     ...
     */
    LM_REAL *work,  /* working memory at least LM_BLEC_DER_WORKSZ() reals large,
                       allocated if NULL */
                  ...
                  */
{
  ...
  if (!lb && !ub) {
    fprintf(stderr,
            RCAT(LCAT(LEVMAR_BLEC_DER, "(): lower and upper bounds for box "
                                       "constraints cannot be both NULL, use "),
  ...
  if (x) { /* nonzero x */
    ...
    if (!data.x) {
      fprintf(stderr, LCAT(LEVMAR_BLEC_DER,
                           "(): memory allocation request #1 failed\n"));
      return LM_ERROR;
    }
  ...
  data.w =
      (LM_REAL *)malloc(m * sizeof(LM_REAL) +
                        m * sizeof(int)); /* should be arranged in that order
                                             for proper doubles alignment */
  if (!data.w) {
    fprintf(stderr,
            LCAT(LEVMAR_BLEC_DER, "(): memory allocation request #2 failed\n"));
    if (data.x)
      free(data.x);
    return LM_ERROR;
  }
  ...
  if (!info)
    info = locinfo; /* make sure that LEVMAR_LEC_DER() is called with non-null
                       info */
  ret = LEVMAR_LEC_DER(LMBLEC_FUNC, LMBLEC_JACF, p, data.x, m, n + m, A, b, k,
...
/* Similar to the LEVMAR_BLEC_DER() function above, except that the Jacobian is
...
int LEVMAR_BLEC_DIF(
     ...
     * solved, i.e. # attempts for reducing error
     */
    LM_REAL *work,  /* working memory at least LM_BLEC_DIF_WORKSZ() reals large,
                       allocated if NULL */
    LM_REAL *covar, /* O: Covariance matrix corresponding to LS solution; mxm.
                       Set to NULL if not needed. */
    void *adata) /* pointer to possibly additional data, passed uninterpreted to
                  ...
                  */
{
  ...
  if (!lb && !ub) {
    fprintf(stderr,
            RCAT(LCAT(LEVMAR_BLEC_DIF, "(): lower and upper bounds for box "
                                       "constraints cannot be both NULL, use "),
  ...
  if (!info)
    info = locinfo; /* make sure that LEVMAR_LEC_DIF() is called with non-null
                       info */
  ret = LEVMAR_LEC_DIF(LMBLEC_FUNC, p, data.x, m, n + m, A, b, k, itmax, opts,
                       info, work, covar, (void *)&data);
