...
#define GEMM                                                                   \
  LM_CAT_(LM_BLAS_PREFIX, LM_ADD_PREFIX(LM_CAT_(gemm, LM_BLAS_SUFFIX)))
#else
#define GEMM LM_ADD_PREFIX(LM_CAT_(gemm, LM_BLAS_SUFFIX))
#endif
/* C := alpha*op( A )*op( B ) + beta*C */
extern void GEMM(char *transa, char *transb, int *m, int *n, int *k,
                 LM_REAL *alpha, LM_REAL *a, int *lda, LM_REAL *b, int *ldb,
                 LM_REAL *beta, LM_REAL *c, int *ldc);
...
/* blocked multiplication of the transpose of the nxm matrix a with itself (i.e.
 * a^T a) using a block size of bsize. The product is returned in b. Since a^T a
 * is symmetric, its computation can be sped up by computing only its upper
 * triangular part and copying it to the lower part.
 *
...
void LEVMAR_TRANS_MAT_MAT_MULT(LM_REAL *a, LM_REAL *b, int n, int m) {
  ...
  /* Fool BLAS to compute a^T*a avoiding transposing a: a is equivalent to a^T
   * in column major, therefore BLAS computes a*a^T with a and a*a^T in column
   * major, which is equivalent to computing a^T*a in row major!
   */
...
void LEVMAR_FDIF_FORW_JAC_APPROX(
  ...
  for (j = 0; j < m; ++j) {
    ...
    d = LM_CNST(1.0) / d; /* invert so that divisions can be carried out faster
                             as multiplications */
    for (i = 0; i < n; ++i) {
...
void LEVMAR_FDIF_CENT_JAC_APPROX(
  ...
  for (j = 0; j < m; ++j) {
    ...
    d = LM_CNST(0.5) / d; /* invert so that divisions can be carried out faster
                             as multiplications */
    for (i = 0; i < n; ++i) {
 ...
 *
 *
 * func points to a function from R^m --> R^n: Given a p in R^m it yields hx in
 * R^n jacf points to a function implementing the Jacobian of func, whose
 * correctness is to be tested. Given a p in R^m, jacf computes into the nxm
 * matrix j the Jacobian of func at p. Note that row i of j corresponds to the
 * gradient of the i-th component of func, evaluated at p. p is an input array
 * of length m containing the point of evaluation. m is the number of variables
...
void LEVMAR_CHKJAC(void (*func)(LM_REAL *p, LM_REAL *hx, int m, int n,
                                void *adata),
  ...
  buf = (LM_REAL *)malloc((fvec_sz + fjac_sz + pp_sz + fvecp_sz) *
                          sizeof(LM_REAL));
  if (!buf) {
    fprintf(stderr,
            LCAT(LEVMAR_CHKJAC, "(): memory allocation request failed\n"));
    exit(1);
  }
  fvec = buf;
  fjac = fvec + fvec_sz;
...
static int LEVMAR_PSEUDOINVERSE(LM_REAL *A, LM_REAL *B, int m) {
  ...
  if (!buf) {
    fprintf(stderr,
            RCAT("memory allocation in ", LEVMAR_PSEUDOINVERSE) "() failed!\n");
    return 0; /* error */
  }
...
static int LEVMAR_LUINVERSE(LM_REAL *A, LM_REAL *B, int m) {
  ...
  if (!buf) {
    fprintf(stderr,
            RCAT("memory allocation in ", LEVMAR_LUINVERSE) "() failed!\n");
    return 0; /* error */
  }
 ...
 * squares fit. JtJ is the approximate Hessian at the solution (i.e. J^T*J,
 * where J is the Jacobian at the solution), sumsq is the sum of squared
 * residuals (i.e. goodnes of fit) at the solution, m is the number of
 * parameters (variables) and n the number of observations. JtJ can coincide
 * with C.
 *
...
#ifdef _MSC_VER
#pragma message(                                                               \
    "LAPACK not available, LU will be used for matrix inversion when computing the covariance; this might be unstable at times")
#else
 ...
 *  covar is the mxm covariance matrix of the best-fit parameters (see also
 * LEVMAR_COVAR()).
 *
...
LM_REAL LEVMAR_CORCOEF(LM_REAL *covar, int m, int i, int j) {
  return (LM_REAL)(covar[i * m + j] /
                   sqrt(covar[i * m + i] * covar[j * m + j]));
}

/* coefficient of determination.
 * see  http://en.wikipedia.org/wiki/Coefficient_of_determination
 */
LM_REAL LEVMAR_R2(void (*func)(LM_REAL *p, LM_REAL *hx, int m, int n,
                               void *adata),
                  LM_REAL *p, LM_REAL *x, int m, int n, void *adata) {
  register int i;
  register LM_REAL tmp;
  LM_REAL SSerr, // sum of squared errors, i.e. residual sum of squares \sum_i
                 // (x_i-hx_i)^2
  ...
  if ((hx = (LM_REAL *)malloc(n * sizeof(LM_REAL))) == NULL) {
    fprintf(stderr,
            RCAT("memory allocation request failed in ", LEVMAR_R2) "()\n");
    exit(1);
  }
...
/* compute the Cholesky decomposition of C in W, s.t. C=W^t W and W is upper
 * triangular */
int LEVMAR_CHOLESKY(LM_REAL *C, LM_REAL *W, int m) {
  ...
  if (info != 0) {
    if (info < 0) {
      fprintf(stderr,
              "LAPACK error: illegal value for argument %d of dpotf2 in %s\n",
              -info, LCAT(LEVMAR_CHOLESKY, "()"));
    } else {
      fprintf(stderr,
              "LAPACK error: the leading minor of order %d is not positive "
              "definite,\n%s()\n",
              info,
 ...
 * stalls and increase instruction-level parallelism; see
...
LM_REAL LEVMAR_L2NRMXMY(LM_REAL *e, LM_REAL *x, LM_REAL *y, int n) {
  ...
  /* unroll the loop in blocks of `blocksize'; looping downwards gains some more
   * speed */
  if (x) {
