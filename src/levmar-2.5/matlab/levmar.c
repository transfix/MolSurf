...
#ifdef _MSC_VER
#pragma message(                                                               \
    "LAPACK not available, certain functionalities cannot be compiled!")
#else
...
/* matlab matrices are in column-major, this routine converts them to row major
 * for levmar */
...
/* check the supplied matlab function and its Jacobian. Returns 1 on error, 0
 * otherwise */
static int checkFuncAndJacobian(double *p, int m, int n, int havejac,
                                struct mexdata *dat) {
  mxArray *lhs[1];
  register int i;
  int ret = 0;
  ...
  if (i) {
    ...
    fprintf(stderr,
            "levmar: '%s' should produce a real vector with %d elements (got "
            "%d).\n",
  ...
  if (havejac) {
    ...
    if (i) {
      ...
      fprintf(stderr,
              "levmar: '%s' should produce a real %dx%d matrix (got %dx%d).\n",
              dat->jacname, n, m, mxGetM(lhs[0]), mxGetN(lhs[0]));
      ret = 1;
    } else if (mxIsSparse(lhs[0])) {
      fprintf(stderr,
              "levmar: '%s' should produce a real dense matrix (got a sparse "
              "one).\n",
              dat->jacname);
      ret = 1;
    }
  ...
  mexSetTrapFlag(0); /* on error terminate the MEX-file and return control to
                        the MATLAB prompt */
...
/*
[ret, p, info, covar]=levmar_der (f, j, p0, x, itmax, opts, 'unc' ...) [ret, p,
info, covar]=levmar_bc  (f, j, p0, x, itmax, opts, 'bc',   lb, ub, ...) [ret, p,
info, covar]=levmar_lec (f, j, p0, x, itmax, opts, 'lec',          A, b, ...)
[ret, p, info, covar]=levmar_blec(f, j, p0, x, itmax, opts, 'blec', lb, ub, A,
b, wghts, ...)

[ret, p, info, covar]=levmar_bleic(f, j, p0, x, itmax, opts, 'bleic', lb, ub, A,
b, C, d, ...) [ret, p, info, covar]=levmar_blic (f, j, p0, x, itmax, opts,
'blic',  lb, ub,       C, d, ...) [ret, p, info, covar]=levmar_leic (f, j, p0,
x, itmax, opts, 'leic',          A, b, C, d, ...) [ret, p, info,
covar]=levmar_lic  (f, j, p0, x, itmax, opts, 'lic',                 C, d, ...)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *Prhs[]) {
  ...
  double opts[LM_OPTS_SZ] = {LM_INIT_MU, LM_STOP_THRESH, LM_STOP_THRESH,
                             LM_STOP_THRESH, LM_DIFF_DELTA};
  double info[LM_INFO_SZ];
  double *lb = NULL, *ub = NULL, *A = NULL, *b = NULL, *wghts = NULL, *C = NULL,
         *d = NULL, *covar = NULL;

  /* parse input args; start by checking their number */
  if ((nrhs < 5))
    matlabFmtdErrMsgTxt("levmar: at least 5 input arguments required (got %d).",
                        nrhs);
  if (nlhs > 4)
    matlabFmtdErrMsgTxt("levmar: too many output arguments (max. 4, got %d).",
                        nlhs);
  else if (nlhs < 2)
    matlabFmtdErrMsgTxt("levmar: too few output arguments (min. 2, got %d).",
                        nlhs);

  /* note that in order to accommodate optional args, prhs & nrhs are adjusted
   * accordingly below */
  ...
  if (mxGetM(prhs[0]) != 1)
    mexErrMsgTxt(
        "levmar: first argument must be a string (i.e. char row vector).");
  /* store supplied name */
  len = mxGetN(prhs[0]) + 1;
  ...
  if (mxIsChar(prhs[1]) == 1) {
    if (mxGetM(prhs[1]) != 1)
      mexErrMsgTxt(
          "levmar: second argument must be a string (i.e. row vector).");
    /* store supplied name */
    len = mxGetN(prhs[1]) + 1;
  ...
  */
  ...
  if (!mxIsDouble(prhs[3]) || mxIsComplex(prhs[3]) || mxGetM(prhs[3]) != 1 ||
      mxGetN(prhs[3]) != 1)
  ...
  if (nopts !=
      0) { /* if opts==[], nothing needs to be done and the defaults are used */
    if (nopts > LM_OPTS_SZ)
      matlabFmtdErrMsgTxt("levmar: opts must have at most %d elements, got %d.",
                          LM_OPTS_SZ, nopts);
    else if (nopts < ((havejac) ? LM_OPTS_SZ - 1 : LM_OPTS_SZ))
      matlabFmtdWarnMsgTxt("levmar: only the %d first elements of opts "
                           "specified, remaining set to defaults.",
                           nopts);
    for (i = 0; i < nopts; ++i)
      opts[i] = pdbl[i];
  }
  ...
  /** lb, ub **/
  if (nrhs >= 7 &&
      (mintype == MIN_CONSTRAINED_BC || mintype == MIN_CONSTRAINED_BLEC ||
       mintype == MIN_CONSTRAINED_BLIC || mintype == MIN_CONSTRAINED_BLEIC)) {
    ...
    if (mxIsDouble(prhs[5]) && !mxIsComplex(prhs[5]) &&
      ...
      if (mxIsDouble(prhs[6]) && !mxIsComplex(prhs[6]) &&
        ...
        if ((i = __MAX__(mxGetM(prhs[5]), mxGetN(prhs[5]))) != m)
          matlabFmtdErrMsgTxt("levmar: lb must have %d elements, got %d.", m,
                              i);
        if ((i = __MAX__(mxGetM(prhs[6]), mxGetN(prhs[6]))) != m)
          matlabFmtdErrMsgTxt("levmar: ub must have %d elements, got %d.", m,
                              i);
  ...
  /** A, b **/
  if (nrhs >= 7 &&
      (mintype == MIN_CONSTRAINED_LEC || mintype == MIN_CONSTRAINED_BLEC ||
       mintype == MIN_CONSTRAINED_LEIC || mintype == MIN_CONSTRAINED_BLEIC)) {
    /* check if the next two arguments are a real matrix and a real row or
     * column vector */
    if (mxIsDouble(prhs[5]) && !mxIsComplex(prhs[5]) && mxGetM(prhs[5]) >= 1 &&
        mxGetN(prhs[5]) >= 1) {
      if (mxIsDouble(prhs[6]) && !mxIsComplex(prhs[6]) &&
          (mxGetM(prhs[6]) == 1 || mxGetN(prhs[6]) == 1)) {
        if ((i = mxGetN(prhs[5])) != m)
          matlabFmtdErrMsgTxt("levmar: A must have %d columns, got %d.", m, i);
        if ((i = __MAX__(mxGetM(prhs[6]), mxGetN(prhs[6]))) !=
            (Arows = mxGetM(prhs[5])))
          matlabFmtdErrMsgTxt("levmar: b must have %d elements, got %d.", Arows,
                              i);
  ...
  if (nrhs >= 6 &&
      mintype == MIN_CONSTRAINED_BLEC) { /* only check if we have seen both box
                                            & linear constraints */
    if (mxIsDouble(prhs[5]) && !mxIsComplex(prhs[5]) &&
        (mxGetM(prhs[5]) == 1 || mxGetN(prhs[5]) == 1)) {
  ...
  /** C, d **/
  if (nrhs >= 7 &&
      (mintype == MIN_CONSTRAINED_BLEIC || mintype == MIN_CONSTRAINED_BLIC ||
       mintype == MIN_CONSTRAINED_LEIC || mintype == MIN_CONSTRAINED_LIC)) {
    /* check if the next two arguments are a real matrix and a real row or
     * column vector */
    if (mxIsDouble(prhs[5]) && !mxIsComplex(prhs[5]) && mxGetM(prhs[5]) >= 1 &&
        mxGetN(prhs[5]) >= 1) {
      if (mxIsDouble(prhs[6]) && !mxIsComplex(prhs[6]) &&
        ...
        if ((i = __MAX__(mxGetM(prhs[6]), mxGetN(prhs[6]))) !=
            (Crows = mxGetM(prhs[5])))
          matlabFmtdErrMsgTxt("levmar: d must have %d elements, got %d.", Crows,
                              i);
...
extraargs:
  ...
  for (i = 0; i < nextra; ++i)
    mdata.rhs[i + 1] =
        (mxArray *)prhs[nrhs - nextra + i]; /* discard 'const' modifier */
#ifdef DEBUG
  fflush(stderr);
  ...
  case MIN_UNCONSTRAINED: /* no constraints */
    if (havejac)
      status = dlevmar_der(func, jacfunc, p, x, m, n, itmax, opts, info, NULL,
                           covar, (void *)&mdata);
    else
      status = dlevmar_dif(func, p, x, m, n, itmax, opts, info, NULL, covar,
                           (void *)&mdata);
#ifdef DEBUG
    fflush(stderr);
  ...
  case MIN_CONSTRAINED_BC: /* box constraints */
    if (havejac)
      status = dlevmar_bc_der(func, jacfunc, p, x, m, n, lb, ub, itmax, opts,
                              info, NULL, covar, (void *)&mdata);
    else
      status = dlevmar_bc_dif(func, p, x, m, n, lb, ub, itmax, opts, info, NULL,
                              covar, (void *)&mdata);
#ifdef DEBUG
    fflush(stderr);
...
#ifdef HAVE_LAPACK
    if (havejac)
      status = dlevmar_lec_der(func, jacfunc, p, x, m, n, A, b, Arows, itmax,
                               opts, info, NULL, covar, (void *)&mdata);
    else
      status = dlevmar_lec_dif(func, p, x, m, n, A, b, Arows, itmax, opts, info,
                               NULL, covar, (void *)&mdata);
#else
    mexErrMsgTxt("levmar: no linear constraints support, HAVE_LAPACK was not "
                 "defined during MEX-file compilation.");
#endif /* HAVE_LAPACK */
...
#ifdef HAVE_LAPACK
    if (havejac)
      status = dlevmar_blec_der(func, jacfunc, p, x, m, n, lb, ub, A, b, Arows,
                                wghts, itmax, opts, info, NULL, covar,
                                (void *)&mdata);
    else
      status = dlevmar_blec_dif(func, p, x, m, n, lb, ub, A, b, Arows, wghts,
                                itmax, opts, info, NULL, covar, (void *)&mdata);
#else
    mexErrMsgTxt("levmar: no box & linear constraints support, HAVE_LAPACK was "
                 "not defined during MEX-file compilation.");
#endif /* HAVE_LAPACK */
...
#ifdef HAVE_LAPACK
    if (havejac)
      status = dlevmar_bleic_der(func, jacfunc, p, x, m, n, lb, ub, A, b, Arows,
                                 C, d, Crows, itmax, opts, info, NULL, covar,
                                 (void *)&mdata);
    else
      status =
          dlevmar_bleic_dif(func, p, x, m, n, lb, ub, A, b, Arows, C, d, Crows,
                            itmax, opts, info, NULL, covar, (void *)&mdata);
#else
    mexErrMsgTxt(
        "levmar: no box, linear equation & inequality constraints support, "
        "HAVE_LAPACK was not defined during MEX-file compilation.");
#endif /* HAVE_LAPACK */

#ifdef DEBUG
    fflush(stderr);
    fprintf(stderr,
            "LEVMAR: calling dlevmar_bleic_der()/dlevmar_bleic_dif()\n");
#endif /* DEBUG */
    break;
  case MIN_CONSTRAINED_BLIC: /* box, linear inequalities constraints */
#ifdef HAVE_LAPACK
    if (havejac)
      status = dlevmar_bleic_der(func, jacfunc, p, x, m, n, lb, ub, NULL, NULL,
                                 0, C, d, Crows, itmax, opts, info, NULL, covar,
                                 (void *)&mdata);
    else
      status = dlevmar_bleic_dif(func, p, x, m, n, lb, ub, NULL, NULL, 0, C, d,
                                 Crows, itmax, opts, info, NULL, covar,
                                 (void *)&mdata);
#else
    mexErrMsgTxt("levmar: no box & linear inequality constraints support, "
                 "HAVE_LAPACK was not defined during MEX-file compilation.");
#endif /* HAVE_LAPACK */
...
#ifdef HAVE_LAPACK
    if (havejac)
      status = dlevmar_bleic_der(func, jacfunc, p, x, m, n, NULL, NULL, A, b,
                                 Arows, C, d, Crows, itmax, opts, info, NULL,
                                 covar, (void *)&mdata);
    else
      status = dlevmar_bleic_dif(func, p, x, m, n, NULL, NULL, A, b, Arows, C,
                                 d, Crows, itmax, opts, info, NULL, covar,
                                 (void *)&mdata);
#else
    mexErrMsgTxt("levmar: no linear equation & inequality constraints support, "
                 "HAVE_LAPACK was not defined during MEX-file compilation.");
#endif /* HAVE_LAPACK */
...
#ifdef HAVE_LAPACK
    if (havejac)
      status = dlevmar_bleic_der(func, jacfunc, p, x, m, n, NULL, NULL, NULL,
                                 NULL, 0, C, d, Crows, itmax, opts, info, NULL,
                                 covar, (void *)&mdata);
    else
      status = dlevmar_bleic_dif(func, p, x, m, n, NULL, NULL, NULL, NULL, 0, C,
                                 d, Crows, itmax, opts, info, NULL, covar,
                                 (void *)&mdata);
#else
    mexErrMsgTxt("levmar: no linear equation & inequality constraints support, "
  ...
  /** popt **/
  plhs[1] = (mdata.isrow_p0 == 1) ? mxCreateDoubleMatrix(1, m, mxREAL)
  ...
  if (nlhs > 3) {
    ...
    pdbl = mxGetPr(plhs[3]);
    for (i = 0; i < m * m; ++i) /* covariance matrices are symmetric, thus no
                                   need to transpose! */
      pdbl[i] = covar[i];
  }
