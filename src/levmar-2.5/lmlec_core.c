...
extern int GEQP3(int *m, int *n, LM_REAL *a, int *lda, int *jpvt, LM_REAL *tau,
                 LM_REAL *work, int *lwork, int *info);

extern int ORGQR(int *m, int *n, int *k, LM_REAL *a, int *lda, LM_REAL *tau,
                 LM_REAL *work, int *lwork, int *info);

extern int TRTRI(char *uplo, char *diag, int *n, LM_REAL *a, int *lda,
                 int *info);
 ...
 * Two matrices Y and Z of dimensions nxm and nx(n-m) are computed from A^T so
 * that their columns are orthonormal and every x can be written as x=Y*b +
 * Z*x_z= c + Z*x_z, where c=Y*b is a fixed vector of dimension n and x_z is an
 ...
 * The function returns LM_ERROR in case of error, A's computed rank if
 * successful
 *
 */
static int LMLEC_ELIM(LM_REAL *A, LM_REAL *b, LM_REAL *c, LM_REAL *Y,
                      LM_REAL *Z, int m, int n) {
  ...
  if (m > n) {
    fprintf(stderr,
            RCAT("matrix of constraints cannot have more rows than columns in",
                 LMLEC_ELIM) "()!\n");
    return LM_ERROR;
  }
  ...
  // ORGQR((int *)&tm, (int *)&tm, (int *)&mintmn, NULL, (int *)&tm, NULL,
  // (LM_REAL *)&aux, &worksz, &info);
  GEQP3((int *)&tm, (int *)&tn, NULL, (int *)&tm, NULL, NULL, (LM_REAL *)&aux,
        (int *)&worksz, &info);
  worksz = (int)aux;
  ...
  tot_sz = (a_sz + tau_sz + r_sz + worksz + Y_sz) * sizeof(LM_REAL) +
           jpvt_sz * sizeof(int); /* should be arranged in that order for proper
                                     doubles alignment */
  buf = (LM_REAL *)malloc(tot_sz); /* allocate a "big" memory chunk at once */
  if (!buf) {
    fprintf(stderr,
            RCAT("Memory allocation request failed in ", LMLEC_ELIM) "()\n");
    return LM_ERROR;
  }
  ...
  GEQP3((int *)&tm, (int *)&tn, a, (int *)&tm, jpvt, tau, work, (int *)&worksz,
        &info);
  ...
  if (info != 0) {
    if (info < 0) {
      ...
      fprintf(stderr,
              RCAT(RCAT("unknown LAPACK error (%d) for ", GEQP3) " in ",
                   LMLEC_ELIM) "()\n",
              info);
    }
    free(buf);
  ...
  /* the upper triangular part of a now contains the upper triangle of the
   * unpermuted R */
  ...
  tmp = (tmp > LM_CNST(1E-12))
            ? tmp
            : LM_CNST(1E-12); // ensure that threshold is not too small
  ...
  if (rank < tn) {
    fprintf(stderr,
            RCAT("\nConstraints matrix in ",
                 LMLEC_ELIM) "() is not of full row rank (i.e. %d < %d)!\n"
                             "Make sure that you do not specify redundant or "
  ...
  if (info != 0) {
    if (info < 0) {
      ...
      fprintf(stderr,
              RCAT(RCAT("A(%d, %d) is exactly zero for ",
                        TRTRI) " (singular matrix) in ",
                   LMLEC_ELIM) "()\n",
              info, info);
    }
    free(buf);
  ...
  ORGQR((int *)&tm, (int *)&tm, (int *)&mintmn, a, (int *)&tm, tau, work,
        &worksz, &info);
  /* error checking */
  if (info != 0) {
    if (info < 0) {
      ...
      fprintf(stderr,
              RCAT(RCAT("unknown LAPACK error (%d) for ", ORGQR) " in ",
                   LMLEC_ELIM) "()\n",
              info);
    }
    free(buf);
 ...
 */
static void LMLEC_JACF(LM_REAL *pp, LM_REAL *jacjac, int mm, int n,
                       void *adata) {
  struct LMLEC_DATA *data = (struct LMLEC_DATA *)adata;
  int m;
  register int i, j, l;
  ...
  if (n * m <= __BLOCKSZ__SQ) { // this is a small problem
     ...
     * its noncontinuous memory access pattern, it incures many cache misses
     * when applied to large minimization problems (i.e. problems involving a
     * large number of free variables and measurements), in which jac is too
     * large to fit in the L1 cache. For such problems, a cache-efficient
     * blocking scheme is preferable. On the other hand, the straightforward
     * algorithm is faster on small problems since in this case it avoids the
     * overheads of blocking.
     */
 ...
 * unavailable, use LEVMAR_LEC_DIF() bellow
 *
 */
int LEVMAR_LEC_DER(
     ...
     */
    LM_REAL *work,  /* working memory at least LM_LEC_DER_WORKSZ() reals large,
                       allocated if NULL */
                  ...
                  */
{
  ...
  if (!ptr) {
    fprintf(stderr,
            LCAT(LEVMAR_LEC_DER, "(): memory allocation request failed\n"));
    return LM_ERROR;
  }
  data.p = p;
  p0 = ptr;
    ...
    if (FABS(tmp - p0[i]) > LM_CNST(1E-03))
      fprintf(stderr,
              RCAT("Warning: component %d of starting point not feasible in ",
                   LEVMAR_LEC_DER) "()! [%.10g reset to %.10g]\n",
              i, p0[i], tmp);
  }

  if (!info)
    info =
        locinfo; /* make sure that LEVMAR_DER() is called with non-null info */
  /* note that covariance computation is not requested from LEVMAR_DER() */
  ret = LEVMAR_DER(LMLEC_FUNC, LMLEC_JACF, pp, x, mm, n, itmax, opts, info,
                   work, NULL, (void *)&data);
...
/* Similar to the LEVMAR_LEC_DER() function above, except that the Jacobian is
...
int LEVMAR_LEC_DIF(
     ...
     * solved, i.e. # attempts for reducing error
     */
    LM_REAL *work,  /* working memory at least LM_LEC_DIF_WORKSZ() reals large,
                       allocated if NULL */
    LM_REAL *covar, /* O: Covariance matrix corresponding to LS solution; mxm.
                       Set to NULL if not needed. */
    void *adata) /* pointer to possibly additional data, passed uninterpreted to
                  ...
                  */
{
  ...
  if (!ptr) {
    fprintf(stderr,
            LCAT(LEVMAR_LEC_DIF, "(): memory allocation request failed\n"));
    return LM_ERROR;
  }
  data.p = p;
  p0 = ptr;
  ...
  for (i = 0; i < m; ++i) {
    ...
    if (FABS(tmp - p0[i]) > LM_CNST(1E-03))
      fprintf(stderr,
              RCAT("Warning: component %d of starting point not feasible in ",
                   LEVMAR_LEC_DIF) "()! [%.10g reset to %.10g]\n",
              i, p0[i], tmp);
  }

  if (!info)
    info =
        locinfo; /* make sure that LEVMAR_DIF() is called with non-null info */
  ...
  /* compute the Jacobian with finite differences and use it to estimate the
   * covariance */
  if (covar) {
    LM_REAL *hx, *wrk, *jac;
    ...
    LEVMAR_FDIF_FORW_JAC_APPROX(func, p, hx, wrk, (LM_REAL)LM_DIFF_DELTA, jac,
                                m, n, adata);    /* compute the Jacobian at p */
