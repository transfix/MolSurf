...
static void LNSRCH(int m, LM_REAL *x, LM_REAL f, LM_REAL *g, LM_REAL *p,
                   LM_REAL alpha, LM_REAL *xpls, LM_REAL *ffpls,
                   void (*func)(LM_REAL *p, LM_REAL *hx, int m, int n,
                                void *adata),
                   struct FUNC_STATE state, int *mxtake, int *iretcd,
                   LM_REAL stepmx, LM_REAL steptl, LM_REAL *sx) {
  /* Find a next newton iterate by backtracking line search.
   * Specifically, finds a \lambda such that for a fixed alpha<0.5 (usually
   1e-4),
   * f(x + \lambda*p) <= f(x) + alpha * \lambda * g^T*p
   *
   * Translated (with minor changes) from Schnabel, Koontz & Weiss uncmin.f,
   v1.3
   ...
   *                          state is not modified in xlnsrch (but can be
   modified by func).
   *      iretcd  <--      return code
  ...
  *iretcd = 2;
  ...
  if (!sx) /* no scaling */
    ...
    for (i = 0; i < m; ++i) {
      tmp1 = (FABS(x[i]) >= LM_CNST(1.) / sx[i]) ? FABS(x[i])
                                                 : LM_CNST(1.) / sx[i];
      tmp2 = FABS(p[i]) / tmp1;
      if (rln < tmp2)
        rln = tmp2;
      ...
      *iretcd = 1;
      ...
      if (!LM_FINITE(fpls)) {
        ...
        if (firstback) { /*     first backtrack: quadratic fit */
          ...
          a3 = LM_CNST(3.) * t3 *
               (t1 / (lambda * lambda) - t2 / (plmbda * plmbda));
          ...
          if (disc > b * b)
            ...
            tlmbda =
                (-b + ((a3 < 0) ? -(LM_REAL)sqrt(disc) : (LM_REAL)sqrt(disc))) /
                a3;
          else
...
/* Projections to feasible set \Omega: P_{\Omega}(y) := arg min { ||x - y|| : x
 * \in \Omega},  y \in R^m */
 ...
 * measurements vector x under box constraints. More precisely, given a vector
 * function  func : R^m --> R^n with n>=m, it finds p s.t. func(p) ~= x, i.e.
 * the squared second order (i.e. L2) norm of e=x-func(p) is minimized under the
 * constraints lb[i]<=p[i]<=ub[i]. If no lower bound constraint applies for
 * p[i], use -DBL_MAX/-FLT_MAX for lb[i]; If no upper bound constraint applies
 * for p[i], use DBL_MAX/FLT_MAX for ub[i].
 *
 * This function requires an analytic Jacobian. In case the latter is
 * unavailable, use LEVMAR_BC_DIF() bellow
 *
 * Returns the number of iterations (>=0) if successful, LM_ERROR if failed
 *
 * For details, see C. Kanzow, N. Yamashita and M. Fukushima:
 * "Levenberg-Marquardt methods for constrained nonlinear equations with strong
 * local convergence properties", Journal of Computational and Applied
 * Mathematics 172, 2004, pp. 375-397. Also, see K. Madsen, H.B. Nielsen and O.
 * Tingleff's lecture notes on unconstrained Levenberg-Marquardt at
...
int LEVMAR_BC_DER(
    ...
    LM_REAL opts[4], /* I: minim. options [\mu, \epsilon1, \epsilon2,
                      ...
                      * ||e||_2. Set to NULL for defaults to be used. Note that
                      * ||J^T e||_inf is computed on free (not equal to lb[i] or
                      * ub[i]) variables only.
                      */
     ...
     */
    LM_REAL *work,  /* working memory at least LM_BC_DER_WORKSZ() reals large,
                       allocated if NULL */
                  ...
                  */
{
  ...
  LM_REAL alpha = LM_CNST(1e-4), beta = LM_CNST(0.9), gamma = LM_CNST(0.99995),
          gamma_sq = gamma * gamma, rho = LM_CNST(1e-8);
  LM_REAL t, t0;
  LM_REAL steptl = LM_CNST(1e3) * (LM_REAL)sqrt(LM_REAL_EPSILON), jacTeDp;
  LM_REAL tmin = LM_CNST(1e-12),
          tming = LM_CNST(1e-18); /* minimum step length for LS and PG steps */
  const LM_REAL tini =
      LM_CNST(1.0); /* initial step length for LS and PG steps */
  ...
  if (n < m) {
    fprintf(stderr,
            LCAT(LEVMAR_BC_DER, "(): cannot solve a problem with fewer "
  ...
  if (!jacf) {
    fprintf(stderr, RCAT("No function specified for computing the Jacobian in ",
                         LEVMAR_BC_DER)
                        RCAT("().\nIf no such function is available, use ",
                             LEVMAR_BC_DIF)
                            RCAT("() rather than ", LEVMAR_BC_DER) "()\n");
    return LM_ERROR;
  }
  ...
  if (!work) {
    ...
    if (!work) {
      fprintf(stderr,
              LCAT(LEVMAR_BC_DER, "(): memory allocation request failed\n"));
      return LM_ERROR;
    }
    freework = 1;
  }
  ...
  for (i = 0; i < m; ++i)
    if (pDp[i] != p[i])
      fprintf(stderr,
              RCAT("Warning: component %d of starting point not feasible in ",
                   LEVMAR_BC_DER) "()! [%g projected to %g]\n",
              i, pDp[i], p[i]);
...
if(!(k%100)){
  ...
  printf("-- errors %.9g %0.9g, #active %d [%d]\n", jacTe_inf, p_eL2, numactive, j);
    ...
    /* determine increment using a combination of adaptive damping, line search
     * and projected gradient search */
    while (1) {
...
#ifdef HAVE_LAPACK
      /* 6 alternatives are available: LU, Cholesky, 2 variants of QR
       * decomposition, SVD and LDLt. Cholesky is the fastest but might be
       * inaccurate; QR is slower but more accurate; SVD is the slowest but most
       * accurate; LU offers a tradeoff between accuracy and speed
       */
      ...
      // ++nlss; linsolver=AX_EQ_B_CHOL; issolved=AX_EQ_B_QR(jacTjac, jacTe, Dp,
      // m); ++nlss; linsolver=AX_EQ_B_QR; issolved=AX_EQ_B_QRLS(jacTjac, jacTe,
      // Dp, m, m); ++nlss; linsolver=(int (*)(LM_REAL *A, LM_REAL *B, LM_REAL
      // *x, int m))AX_EQ_B_QRLS; issolved=AX_EQ_B_SVD(jacTjac, jacTe, Dp, m);
      // ++nlss; linsolver=AX_EQ_B_SVD;
      ...
      if (issolved) {
        ...
        if (pDp_eL2 <= gamma_sq * p_eL2) {
          ...
          if (dL > 0.0) {
            ...
            mu =
                (mu >= pDp_eL2) ? pDp_eL2 : mu; /* pDp_eL2 is the new pDp_eL2 */
#else
      ...
      if (jacTeDp <= -rho * pow(Dp_L2, _POW_ / LM_CNST(2.0))) {
...
#if 1
        /* use Schnabel's backtracking line search; it requires fewer "func"
         * evaluations */
        LNSRCH(m, p, p_eL2, jacTe, Dp, alpha, pDp, &pDp_eL2, func, fstate,
               &mxtake, &iretcd, stepmx, steptl,
               NULL); /* NOTE: LNSRCH() updates hx */
        if (iretcd != 0)
          goto gradproj; /* rather inelegant but effective way to handle
                            LNSRCH() failures... */
#else
        ...
        for (t = tini; t > tmin; t *= beta) {
          for (i = 0; i < m; ++i) {
            pDp[i] = p[i] + t * Dp[i];
            // pDp[i]=__MEDIAN3(lb[i], pDp[i], ub[i]); /* project to feasible
            // set */
          }
          ...
          // if(LM_CNST(0.5)*pDp_eL2<=LM_CNST(0.5)*p_eL2 + t*alpha*jacTeDp)
          // break;
        ...
        /* NOTE: new estimate for p is in pDp, associated error in hx and its
         * norm in pDp_eL2. These values are used below to update their
         * corresponding variables
         */
      } else {
      gradproj: /* Note that this point can also be reached via a goto when
                   LNSRCH() fails */

        /* jacTe is a descent direction; make a projected gradient step */

        /* if the previous step was along the gradient descent, try to use the t
        ...
        t0 = (tmp <= tini) ? tmp
                           : tini; /* guard against poor scaling & large steps;
                                      see (3.50) in C.T. Kelley's book */
        ...
        for (t = (gprevtaken) ? t : t0; t > tming; t *= beta) {
          ...
          if (gprevtaken &&
              pDp_eL2 <= p_eL2 + LM_CNST(2.0) * LM_CNST(0.99999) *
                                     tmp) { /* starting t too small */
            t = t0;
            gprevtaken = 0;
            continue;
          }
...
static void LMBC_DIF_JACF(LM_REAL *p, LM_REAL *jac, int m, int n, void *data) {
  ...
  if (dta->ffdif) {
    ...
    LEVMAR_FDIF_FORW_JAC_APPROX(dta->func, p, dta->hx, dta->hxx, dta->delta,
                                jac, m, n, dta->adata);
  } else
    LEVMAR_FDIF_CENT_JAC_APPROX(dta->func, p, dta->hx, dta->hxx, dta->delta,
                                jac, m, n, dta->adata);
}

/* No Jacobian version of the LEVMAR_BC_DER() function above: the Jacobian is
 * approximated with the aid of finite differences (forward or central, see the
 * comment for the opts argument) Ideally, this function should be implemented
 * with a secant approach. Currently, it just calls LEVMAR_BC_DER()
 */
int LEVMAR_BC_DIF(
    void (*func)(LM_REAL *p, LM_REAL *hx, int m, int n,
                 void *adata), /* functional relation describing measurements. A
     ...
     * solved, i.e. # attempts for reducing error
     */
    LM_REAL *work,  /* working memory at least LM_BC_DIF_WORKSZ() reals large,
                       allocated if NULL */
    LM_REAL *covar, /* O: Covariance matrix corresponding to LS solution; mxm.
                       Set to NULL if not needed. */
    void *adata) /* pointer to possibly additional data, passed uninterpreted to
                  ...
                  */
{
  ...
  // fprintf(stderr, RCAT("\nWarning: current implementation of ",
  // LEVMAR_BC_DIF) "() does not use a secant approach!\n\n");

  data.ffdif = !opts || opts[4] >= 0.0;

  data.func = func;
  data.hx = (LM_REAL *)malloc(
      2 * n * sizeof(LM_REAL)); /* allocate a big chunk in one step */
  if (!data.hx) {
    fprintf(stderr,
            LCAT(LEVMAR_BC_DIF, "(): memory allocation request failed\n"));
    return LM_ERROR;
  }
  data.hxx = data.hx + n;
  data.adata = adata;
  ...
  ret = LEVMAR_BC_DER(LMBC_DIF_FUNC, LMBC_DIF_JACF, p, x, m, n, lb, ub, itmax,
                      opts, info, work, covar, (void *)&data);

  if (info) { /* correct the number of function calls */
    if (data.ffdif)
      info[7] +=
          info[8] *
          (m + 1); /* each Jacobian evaluation costs m+1 function calls */
    else
      info[7] +=
          info[8] *
          (2 * m); /* each Jacobian evaluation costs 2*m function calls */
  }
