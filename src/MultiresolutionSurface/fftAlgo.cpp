...
extern bool multiplyWith1DTensor(fftw_complex *dest, fftw_complex *input1,
...
bool getBlurMapWithFullFFT(fftw_complex *blurmap, fftw_complex *rbfCenterValues,
  ...
  fftw_complex *centerFrequencies = (fftw_complex *)fftw_malloc(
      sizeof(fftw_complex) * gridSize * gridSize * gridSize);
  {
    int i;
  ...
  // get the frequencies of the centers.
  {
    ...
    //    If small extent rbf functions, then we only need to 0 -pad with that
    //    size, else, we may need to 0-pad with cuberoot(3)* N.
    ...
    if (!getNormalizedPositions(rbfCenterPositions, xk, yk, zk, rbfWidth,
                                gridSizeSmall, numCenters, interpFuncExtent,
                                numFreq, alpha, min, max)) {
    ...
    fftw_complex *spaceValsGrid = (fftw_complex *)fftw_malloc(
        sizeof(fftw_complex) * gridSize * gridSize * gridSize);
    ...
    fftAlgoTime = getTime();
    {
      ...
      if (!compute3DFFT(centerFrequencies, spaceValsGrid, gridSize, gridSize,
                        gridSize, FFTW_FORWARD, FFTW_ESTIMATE)) {
  ...
  fftw_complex *frequencies = (fftw_complex *)fftw_malloc(
      sizeof(fftw_complex) * gridSize * gridSize * gridSize);
  if (!multiplyWith1DTensor(frequencies, centerFrequencies, RBFFrequencies,
                            gridSize, gridSize)) {
  ...
  fftw_free(centerFrequencies);
  fftw_free(RBFFrequencies);
  if (!compute3DFFT(blurmap, frequencies, gridSize, gridSize, gridSize,
