...
extern bool computeFFT3d(fftw_complex *dest, fftw_complex *src, int dim1,
...
bool getBlurMapWithSlowAlgo(fftw_complex *blurmap,
    ...
    for (c1 = zStart; c1 <= zEnd; c1++) {
      ...
      for (c2 = yStart; c2 <= yEnd; c2++) {
        ...
        for (c3 = xStart; c3 <= xEnd; c3++) {
          blurmap[c1 * gridSize * gridSize + c2 * gridSize + c3][0] +=
              zyGaussian * rbf1D[abs(xpos - c3)][0];
        }
      }
    }
  }
