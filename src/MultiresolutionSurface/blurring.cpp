...
extern bool getBlurMapWithSlowAlgo(fftw_complex *blurmap,
                                   fftw_complex *rbfCenterValues,
                                   double *rbfCenterPositions,
                                   fftw_complex *rbf1D, int numCenters,
                                   int numFreq, int gridSize, double alpha,
                                   int interpFuncExtent, int rbfWidth);
...
/* Computes FFT using the FFTW3 code. Please call fftw_whatever_they_want_to_do
 * with the */
/* appropriate sizes before calling this. Call this with preferably FFTW_MEASURE
 * flag. */
bool computeFFT1d(fftw_complex *dest, fftw_complex *src, int dim, int type,
                  unsigned int flags) {
  if (!dest || !src || (dim < 1)) {
    if (!dest) {
      cout << "no dest" << endl;
...
bool computeFFT2d(fftw_complex *dest, fftw_complex *src, int dim1, int dim2,
...
bool compute3DFFT(fftw_complex *dest, fftw_complex *src, int dim1, int dim2,
                  int dim3, int type, unsigned int flags) {
...
/* We should call this function for all sizes of ffts that we want to compute
 * later */
/* It is a good idea to call this somewhere in the main function as soon as we
 * know what */
/* sizes are relevant. */
bool initializeFFTW(int size) {
  ...
  if (!compute3DFFT(data, data, size, size, size, FFTW_BACKWARD,
                    FFTW_MEASURE)) {
    return false;
  }
...
/* Do some sanity check on the inputs. There are many many small things to take
 * care of. */
int checkInputs(int lowResGridSize, int interpFuncExtent, int numFreq,
                int gridSize, char *file1, char *file2) {
...
bool fftshift3D(fftw_complex *data, int length) {
  ...
  fftw_complex *temp =
      (fftw_complex *)malloc(sizeof(fftw_complex) * length * length * length);
  int c;
  ...
  for (i = 0; i < length; i++) {
    for (j = 0; j < length; j++) {
      for (k = 0; k < length; k++) {
        ...
        data[shift_i * length * length + shift_j * length + shift_k][0] =
            temp[i * length * length + j * length + k][0];
        data[shift_i * length * length + shift_j * length + shift_k][1] =
            temp[i * length * length + j * length + k][1];
      }
    }
  }
  fftw_free(temp);
...
bool multiplyWithLarger1DTensor(fftw_complex *frequencies, int numFreq,
                                fftw_complex *largerbf, int gridSize) {
  ...
  int i, j, k;
  fftw_complex *rbf =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * numFreq);
  {
    int i;
    for (i = 0; i < numFreq; i++) {
      if (i < numFreq / 2) {
...
bool multiplyWith1DTensor(fftw_complex *dest, fftw_complex *input1,
                          fftw_complex *input2, int length, int length2) {
  if (!dest || !input1 || !input2 || (length < 1) || (length2 < 1)) {
    return false;
  }
  fftw_complex *shiftedRBFFrequencies =
  ...
  for (j = 0; j < length; j++) {
    for (k = 0; k < length; k++) {
      for (i = 0; i < length; i++) {
        ...
        sum_re += val_re;
        sum_im += val_im;
        double t1_re = val_re * shiftedRBFFrequencies[i][0] -
                       val_im * shiftedRBFFrequencies[i][1];
        double t1_im = val_re * shiftedRBFFrequencies[i][1] +
                       val_im * shiftedRBFFrequencies[i][0];
        double t2_re = t1_re * shiftedRBFFrequencies[j][0] -
                       t1_im * shiftedRBFFrequencies[j][1];
        double t2_im = t1_re * shiftedRBFFrequencies[j][1] +
                       t1_im * shiftedRBFFrequencies[j][0];
        double t3_re = t2_re * shiftedRBFFrequencies[k][0] -
                       t2_im * shiftedRBFFrequencies[k][1];
        double t3_im = t2_re * shiftedRBFFrequencies[k][1] +
                       t2_im * shiftedRBFFrequencies[k][0];
        dest[c][0] = t3_re;
        dest[c][1] = t3_im;
        c++;
      }
...
bool zeroPad(fftw_complex *dest, int destSize, fftw_complex *input,
             int inputSize) {
  ...
  for (i = 0; i < inputSize; i++) {
    for (j = 0; j < inputSize; j++) {
      for (k = 0; k < inputSize; k++) {
        ...
        dest[i_index * destSize * destSize + j_index * destSize + k_index][0] =
            input[i * inputSize * inputSize + j * inputSize + k][0];
        dest[i_index * destSize * destSize + j_index * destSize + k_index][1] =
            input[i * inputSize * inputSize + j * inputSize + k][1];
      }
    }
  }
  return true;
}
bool getNormalizedPositions(double *rbfCenterPositions, double *xk, double *yk,
                            double *zk, int rbfWidth, int gridSize,
                            int numCenters, int interpFuncExtent, int numFreq,
                            double alpha, double *min, double *max) {
  if (!rbfCenterPositions || !xk || !yk || !zk || (rbfWidth < 1) ||
      (gridSize < 1) || (numCenters < 1)) {
    return false;
  }
  ...
  }
  {
    ...
    for (i = 0; i < numCenters; i++) {
      ...
      xk[i] =
          xk[i] *
          scale_all[0]; // within -0.5 to 0.5, including appropriate 0 padding
      yk[i] = rbfCenterPositions[i * 3 + 1] + yTrans; // from -w/2 to +w/2
      yk[i] =
          yk[i] *
          scale_all[1]; // within -0.5 to 0.5, including appropriate 0 padding
      zk[i] = rbfCenterPositions[i * 3 + 2] + zTrans; // from -w/2 to +w/2
      zk[i] =
          zk[i] *
          scale_all[2]; // within -0.5 to 0.5, including appropriate 0 padding
    }
  }
  {
    double scale = 1;
    for (i = 0; i < numCenters; i++) {
      xk[i] =
          xk[i] * scale; // within -0.5 to 0.5, including appropriate 0 padding
      yk[i] =
          yk[i] * scale; // within -0.5 to 0.5, including appropriate 0 padding
      zk[i] =
          zk[i] * scale; // within -0.5 to 0.5, including appropriate 0 padding
    }
  }
  {
    int i;
...
bool getCenterFreq(fftw_complex *centerFrequencies,
                   ...
                   float max[3], SmoothingFunction *smoothingFunction) {
  ...
  // get the frequencies of the centers.
  {
    ...
    if (!getNormalizedPositions(rbfCenterPositions, xk, yk, zk, rbfWidth,
                                gridSize, numCenters, interpFuncExtent, numFreq,
                                alpha, dmin, dmax)) {
      return false;
    }
    if (!getCenterFrequencies(numCenters, alpha, interpFuncExtent, numFreq,
                              gridSize, xk, yk, zk, rbfCenterValues,
                              centerFrequencies, 0, smoothingFunction)) {
      return false;
    }
...
bool getRBFFreq(fftw_complex *rbf1D, int gridSize,
  ...
  if (!computeFFT1d(RBFFrequencies, rbf1D, gridSize, FFTW_FORWARD,
                    FFTW_MEASURE)) {
...
bool multiplyWithTensor(fftw_complex *centerFrequencies,
                        ...
                        int numFreq) {
  if (!multiplyWithLarger1DTensor(centerFrequencies, numFreq, RBFFrequencies,
                                  gridSize)) {
    return false;
  }
  return true;
...
bool inverseFFT(fftw_complex *blurmap, fftw_complex *centerFrequencies,
  ...
  if (!compute3DFFT(blurmap, blurmap, gridSize, gridSize, gridSize,
...
bool getBlurMap(fftw_complex *blurmap, fftw_complex *rbfCenterValues,
  ...
  if (!blurmap || !rbfCenterValues || !rbfCenterPositions || !rbf1D ||
      (numCenters < 1) || (numFreq < 1) || (gridSize < 1) || (alpha < 1) ||
      (interpFuncExtent < 1)) {
    return false;
  }
  int intAlpha = (int)alpha;
  fftw_complex *centerFrequencies = (fftw_complex *)fftw_malloc(
      sizeof(fftw_complex) * numFreq * numFreq * numFreq);
  ...
  fftw_complex *RBFFrequencies =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * gridSizeBig);
  if (!computeFFT1d(RBFFrequencies, rbf1D, gridSizeBig, FFTW_FORWARD,
                    FFTW_MEASURE)) {
    return false;
  }
  ...
  if (!compute3DFFT(blurmap, blurmap, gridSizeBig, gridSizeBig, gridSizeBig,
                    FFTW_BACKWARD, FFTW_MEASURE)) {
    return false;
  }
