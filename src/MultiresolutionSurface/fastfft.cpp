...
void gridding(fftw_complex *fk, double *xk, double *yk, double *zk, double n,
              int numAtoms, int m, fftw_complex *Ian1,
  ...
  for (s = 0; s < numAtoms; s++) {
    ...
    int zU = ceil(zk[s] * n) + m;
    {
      ...
      for (zt = zL; zt <= zU; zt++) {
        ...
        for (yt = yL; yt <= yU; yt++) {
          double yzbellval =
              zbellval * smoothingFunction->getPhi(yBellValIndex);
          index = zindex + yindex + (int)(xL + inc);
          double xBellValIndex = xL - n * xk[s];
          for (xt = xL; xt <= xU; xt++) {
            double bellVal =
                yzbellval * smoothingFunction->getPhi(xBellValIndex);
...
/* The input frequencies 'ourFrequencies' is compared with an actual DFT. This
 * function could take too long for  */
/* many frequencies. */
void compareWithDFT(fftw_complex *ourFrequencies, int v, int numAtoms,
                    double *xk, double *yk, double *zk, fftw_complex *fk,
                    FILE *fOut) {
    ...
    for (k = 0; k < v; k++) {
      for (i = 0; i < v; i++) {
        ...
        }
        {
          dftFrequencies[c][0] =
              dftFrequencies[c][0] *
              pow(-1, (double)((i - v / 2) + (j - v / 2) + (k - v / 2)));
          dftFrequencies[c][1] =
              dftFrequencies[c][1] *
              pow(-1, (double)((i - v / 2) + (j - v / 2) + (k - v / 2)));
        }
  ...
  for (c = 0; c < v * v * v; c++) {
    double diffVal;
    double val;
    diffVal = sqrt((dftFrequencies[c][0] - ourFrequencies[c][0]) *
                       (dftFrequencies[c][0] - ourFrequencies[c][0]) +
                   (dftFrequencies[c][1] - ourFrequencies[c][1]) *
                       (dftFrequencies[c][1] - ourFrequencies[c][1]));
    if (diffVal > maxDiffVal) {
      maxDiffVal = diffVal;
    }
    val = sqrt(dftFrequencies[c][0] * dftFrequencies[c][0] +
               dftFrequencies[c][1] * dftFrequencies[c][1]);
    if (val > maxVal) {
      maxVal = val;
    }
    sumActualSq += (val * val);
...
/* Our NDFT algorithm is used to obtain the low frequencies from the input
 * molecule.                             */
...
bool getCenterFrequencies(int numAtoms, double alpha, int interpFuncExtent,
                          int numFreq, int gridSize, double *xk, double *yk,
                          double *zk, fftw_complex *fk,
  ...
  }
  {
    ...
    gridding(fk, xk, yk, zk, n, numAtoms, interpFuncExtent, Ian1,
             smoothingFunction);
  }
  {
    ...
    int alphaM = (int)n;
    ourMoreFrequencies = (fftw_complex *)malloc(sizeof(fftw_complex) *
                                                (alphaM) * (alphaM) * (alphaM));
    ...
    // do the shifts to the middle cube + 8 corner cubes + 12 side bars + 6
    // faces. Add all into the same area as FFT is commutative in addition
    ...
    for (i = 0; i < 2 * m + alphaM; i++) {
      for (j = 0; j < 2 * m + alphaM; j++) {
        for (k = 0; k < 2 * m + alphaM; k++) {
          ...
          newJ = newJ % alphaM;
          newK = newK % alphaM;
          ourMoreFrequencies[newI * alphaM * alphaM + newJ * alphaM + newK]
                            [0] += Ian1[i * gSize * gSize + j * gSize + k][0];
          ourMoreFrequencies[newI * alphaM * alphaM + newJ * alphaM + newK]
                            [1] += Ian1[i * gSize * gSize + j * gSize + k][1];
        }
      }
    }
    {
      double fftTime = getTime();
      if (!compute3DFFT(ourMoreFrequencies, ourMoreFrequencies, alphaM, alphaM,
                        alphaM, FFTW_FORWARD, FFTW_ESTIMATE)) {
        return false;
      }
    }
    c = 0;
    ...
    }
    {
        ...
        for (k = 0; k < alphaM; k++) {
          for (i = 0; i < alphaM; i++) {
            if ((j < v / 2 || j >= alphaM - v / 2) &&
              ...
              ourFrequencies[j1 * v * v + k1 * v + i1][0] =
                  ourMoreFrequencies[c][0];
              ourFrequencies[j1 * v * v + k1 * v + i1][1] =
                  ourMoreFrequencies[c][1];
            }
            c++;
          }
        }
      }
      {
        ...
        for (i = 0; i < v; i++) {
          ...
          for (j = 0; j < v; j++) {
            double ijPsiBar =
                iPsiBar * smoothingFunction->getPhiHat(abs(j - v / 2));
            for (k = 0; k < v; k++) {
              double ijkPsiBar =
                  ijPsiBar * smoothingFunction->getPhiHat(abs(k - v / 2));
