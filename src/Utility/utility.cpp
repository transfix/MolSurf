...
bool CVCUtility::beginsWith(const char *string, const char *substring) {
  const char *temp;
...
bool CVCUtility::strcmpCaseInsensitive(const char *str1, const char *str2) {
...
int CVCUtility::stringToInt(string mystr, bool optional) {
  int temp = 0;
...
char CVCUtility::stringToChar(string mystr, bool optional) {
  char temp = 0;
  stringstream ss(mystr);
  ss >> temp;
  if (!optional) {
...
void CVCUtility::error(string message) {
  cout << message << endl;
  exit(-1);
}

// Error-checking fread wrapper function
size_t CVCUtility::freadSafely(void *ptr, size_t size, size_t count,
                               FILE *stream) {
...
char *CVCUtility::fgetsSafely(char *str, int num, FILE *stream) {
...
void *CVCUtility::mallocSafely(size_t size) {
  if (size == 0)
...
void *CVCUtility::callocSafely(size_t size) {
  if (size == 0)
...
void *CVCUtility::reallocSafely(void *ptr, size_t size) {
  if (size == 0)
...
int CVCUtility::minimum(int a, int b) { return (a < b) ? a : b; }

// Maximum of two numbers
int CVCUtility::maximum(int a, int b) { return (a > b) ? a : b; }

FILE *CVCUtility::fileRead(const char *fileName) {
  FILE *fp;
  fp = fopen(fileName, "r");
  if (fp == NULL) {
...
FILE *CVCUtility::fileWrite(const char *fileName) {
  FILE *fp;
  fp = fopen(fileName, "w");
  if (fp == NULL) {
    printf("could not open file %s for read\n", fileName);
...
FILE *CVCUtility::fileWrite(const string &fileName) {
  return fileWrite(fileName.c_str());
}

FILE *CVCUtility::fileRead(const string &fileName) {
