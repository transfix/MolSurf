...
{
  cout	<< endl << "Usage: MolSurf -aSpline -raw <raw input file> <raw output file> <resolution>"
	<< endl << "    Generate A-spline model (ASMS) from a triangular mesh."
	<< endl ;
}
...
{
  cout	<< endl << "Usage: MolSurf -aSpline -raw+quad <raw input file> <raw output file>"
	<< endl << "               <raw output w/ quad points> <quadrature file>"
	<< endl << "               <resolution> <quadrature rule> <quadrature order>"
	<< endl << "   Generate A-spline model (ASMS) from a triangular mesh and generate quadrature points."
	<< endl;
}
...
bool aSplineRawQuad(int argc, char* argv[])
{
  ...
  Geometry* newGeo = aSplineRawQuad(geometry, numOfPts, resolution, output1, output2, type);
...
{
  cout	<< endl << "Usage: MolSurf -getSurfaceUsingGaussianBlur <pdb/pqr input file> <raw output file> "
	<< endl << "         <isovalue>   <size> <blobbiness> <radius type> <level> <optimzeRadii> "
	<< endl << "    Generate molecular surface using the Gaussian blur algorithm."
	<< endl
	<< endl ;
}
...
bool surfaceUsingGaussianBlur(int argc, char* argv[])
{
  ...
  Geometry* geo = surfaceUsingGaussianBlur(molecule, isovalue, size, blobbiness, radiusInt, level, optimizeRadii);
...
{
  cout	<< endl << "Usage: MolSurf -getSurfaceUsingGaussianBlur2 <pdb/pqr input file> <raw output file> "
	<< endl << "         <isovalue>   <resolution> <blobbiness> <radius type> <level>"
	<< endl << "    Generate molecular surface using the Gaussian blur algorithm."
	<< endl
	<< endl ;
}
...
bool surfaceUsingGaussianBlur2(int argc, char* argv[])
{
  ...
  //Read a file, call the function, write a file.
  PDBParser::GroupOfAtoms* molecule = loadGOA(pdbFile);	
  Geometry* geo = surfaceUsingGaussianBlur2(molecule, isovalue, resolution, blobbiness, radiusInt, level);
  saveAndDeleteGeometry(surfaceFile, geo);
...
{
  cout<<"Usage: MolSurf -surfaceUsingFastSurf <pdb/pqr input file> <dim0> <dim1> <dim2> <bool SAS> <output.raw>"
      << endl << "    Generate molecular surface using a fast algorithm."
      << endl ;
}
...
void usageSurfaceUsingAdaptiveGrid()
{
  ...
  cout	<< endl << "Usage: MolSurf -surfaceUsingAdaptiveGrid <pdb/pqr input file> <raw output file> <size> [ <enlarged width> ]"
	<< endl << "    Generate molecular surface using the adaptive grid algorithm."
	<< endl ;
}
...
{
  cout	<< endl << "Usage: MolSurf -surfaceUsingHLS <pdb/pqr input file> <raw output file> "
	<< endl << "          <size> [nIterations [weight]]"
	<< endl << "    Generate molecular surface using the higher order level-set algorithm."
	<< endl << "    nIterations and weight are optional arguments."
	<< endl ;
}
...
{
	cout<<endl << "Usage: MolSurf -surfaceUsingCardinalBSpline <pdb/pqr input file> <int size>"
	  << endl << "		<int order(0-4)> [float isovalue (0-0.5 (default 0.05)) ] <raw output file>"
	  << endl << " 		Generate molecular surface using cardinal B spline algorithm "
	  << endl;
}
...
bool surfaceUsingCardinalBSpline(int argc, char* argv[])
{
	...
	PDBParser::GroupOfAtoms* molecule = loadGOA(inputFile);
	Geometry* geometry = surfaceUsingCardinalBSpline(molecule, size, order, isovalue);
	delete molecule;
	saveAndDeleteGeometry(outputFile, geometry);
	return true;
}
...
void printASplineUsage() {
  cout << endl << "Usage: MolSurf -aSpline -[raw|quad|raw+quad|volume] [args]" << endl << endl;
}
...
{
	 cout << endl << "Usage: MolSurf -mergeGeometry  <input file1> <input file2 ...> <outputfle>" << endl << endl;
}
...
{
	cout<< endl <<"Usage: ./MolSurf -mergeProtein <radius type: vdw or pqr> <inputFile1> <inputFile2> <outputFile> ";
	cout<<endl;
}
...
void usagePQR2PDB()
{
	cout<< endl <<"Usage: ./MolSurf -PQR2PDB -withH -withChain <inputFile.pqr>  <outputFile.pdb> ";
	cout<<endl <<" e.g.,./MolSurf -PQR2PDB 1 1 <inputFile.pqr>  <outputFile.pdb> with hydrogen, with chain"; 
	cout<<endl <<" e.g.,./MolSurf -PQR2PDB 0  0 <inputFile.pqr>  <outputFile.pdb> no hydrogen, no chain"; 
	cout<<endl;
}
...
void usagePQR2XYZR()
{
	cout<< endl <<"Usage: ./MolSurf -PQR2XYZR <inputFile.pqr>  <outputFile.xyzr> ";
	cout<<endl;
}
...
bool pqr2xyzr(int argc, char* argv[])
{
	...
	fout<<input->m_Atoms[i]->position[0]<<" "<< input->m_Atoms[i]->position[1]<<" "<< input->m_Atoms[i]->position[2]<<" "<<input->m_Atoms[i]->radius << endl;
	fout.close();
...
void usageSplitPDB()
{
	cout<< endl <<"Usage: ./MolSurf -splitPDB2Chains <inputFile.pdb>  <outputFile (without ext)> ";
	cout<<endl;
}
...
//	else 
 //		radiusType = MOLECULE::VDW_RADIUS;
	...
	for(int i=0; i< input->m_Atoms.size(); i++)
	{
		...
		{	
			fprintf(fpout, "TER %7d      %3s %c%4d\n", j, input->m_Atoms[i-1]->resName, input->m_Atoms[i-1]->chainID, input->m_Atoms[i-1]->resSeq-tmpResSeq+1 );
			fclose(fpout);
			j=1;
		...
		fprintf(fpout, "ATOM%7d %-3s %3s %c%4d    %8.3f%8.3f%8.3f%6.2f%6.2f           %c\n", 
				j,
				at->atomName,
				at->resName,
				at->chainID,
				at->resSeq -tmpResSeq+1,
				at->position[0],
				at->position[1],
				at->position[2],
				at->occupancy,
	...
	fprintf(fpout, "TER %7d      %3s %c%4d\n", j, input->m_Atoms.back()->resName, input->m_Atoms.back()->chainID, input->m_Atoms.back()->resSeq-tmpResSeq+1 );
...
{
	cout<< endl <<"Usage: ./MolSurf -colorMeshByAtomType <inputFile.pdb> <inputMesh.raw>  <outputMesh.rawc> ";
	cout<<endl;
}
...
//	int pt_index = 0;
	for(int i=0; i< input->m_Atoms.size(); i++)
	{
		DPG::WeightedPoint* pt = new DPG::WeightedPoint(input->m_Atoms[i]->position[0], input->m_Atoms[i]->position[1], input->m_Atoms[i]->position[2], (double)i);
//		const DPG::WeighPoint pp = *pt;
		
//		(pt->getX(), pt->getY(), pt->getZ());

//		std::cout<<"pp.x: "<< pp.getX() <<" " << pp.getY() << " " <<pp.getZ() << endl;
		pts.push_back(pt);
		pg->addPoint(pts[i]);
...
//		}
	...
	for(int i=0; i< geometry->m_NumTriVerts; i++)
	{
		...
		DPG::Point* p = new DPG::Point(geometry->m_TriVerts[3*i+0], geometry->m_TriVerts[3*i+1],geometry->m_TriVerts[3*i+2]); 
		...
		for(int j=0; j< indices.size(); j++)
		{
			...
			for(int t=0; t<6; t++)
			{
			    if(indices[j]>input->m_Atoms.size()) printf(" indices %d\n", indices[j]);
				ascii = (int)(input->m_Atoms[indices[j]]->atomName[t]);
				if(ascii<=90 && ascii >=65)
				break; 
			}
			if(ascii ==67) ColorID = 0;
			else if(ascii == 79) ColorID = 1;
			else if(ascii == 78) ColorID = 2;
			else ColorID = 3;

			geometry->m_TriVertColorsTransparent[4*i+0] += color[ColorID][0]/(float)indices.size();
			geometry->m_TriVertColorsTransparent[4*i+1] += color[ColorID][1]/(float)indices.size();
			geometry->m_TriVertColorsTransparent[4*i+2] += color[ColorID][2]/(float)indices.size();
		}
		indices.clear();
	}
...
{
	cout<<endl<<"Usage: ./MolSurf -bbox <pqrlist?(0 or 1)> <inputPQR/PQRlistfile>"<<endl;
}
	...
	double max[3] = {-100000000.0, -100000000.0, -100000000.0};
	   ...
	   FILE* lfp = fopen( argv[3], "r");
           ...
           while( NULL != fgets( line, 1024, lfp ) )
           {
              ...
              if( !fp ) { fprintf( stderr, "%s file open fail\n", pqrfile ); continue; }
	      MOLECULE::GroupOfAtoms* input = new MOLECULE::GroupOfAtoms();
  	      input = ParsePQR(fp, radiusType);
	      fclose( fp );
	      ...
	      for(int i=0; i< input->m_Atoms.size(); i++)
		for(int j=0; j< 3; j++) {
	  	   if( min[j] > input->m_Atoms[i]->position[j] ) min[j] = input->m_Atoms[i]->position[j];
	  	   if( max[j] < input->m_Atoms[i]->position[j] ) max[j] = input->m_Atoms[i]->position[j];
                }
              delete input;
           }
           fclose( lfp );
...
{
	cout<<endl<<"Usage: ./Molsurf -atom2surfaceDistance <inputPQR> <inputMesh> <outputCDR>";
	cout<<endl<<"Warning. outputCDR is (P)ositio replaced by (C)olor, (Q)charge replace by (D)istance	to the surface and R invarant";
	cout<<endl;
}
...
//		meshPoints.push_back(p);
	...
	for(int i=0; i< input->m_Atoms.size(); i++)
	{
		DPG::Point* pt = new DPG::Point(input->m_Atoms[i]->position[0], input->m_Atoms[i]->position[1], input->m_Atoms[i]->position[2]);
...
{
	cout<<endl<<"Usage: ./Molsurf -tinkerPDBXYZ2PQR <inputPDB> <inputXYZ> <inputBornRadiiFile> <outputPQR>";
...
//	fscanf(fpXYZ, "%d\n", atmNum);
	...
	{
		cout<<"The number of atoms in XYZ file is different from PDB file, exit!"<<endl;
		return false;
	}
...
//			cout<<tmp<<endl;
		...
		fpXYZ>> atnInd>> atnName>> weightPoints[i][0]>> weightPoints[i][1]>> weightPoints[i][2];
		weightPoints[i][3]= atof(text[2*(i%5)+1].c_str());
		readUntilNewline(fpXYZ);
//		cout<<  weightPoints[i][0] << " " <<  weightPoints[i][1] << " " <<  weightPoints[i][2] << " " <<  weightPoints[i][3] << endl;
		Index.push_back(i);
	}
	...
	for(int i=0; i< input->m_Atoms.size(); i++)
	{
		...
		for ( iit=Index.begin(); iit < Index.end(); iit++ )
  		{
		//	cout<< *iit <<" " << weightPoints[*iit][0] << weightPoints[*iit][1] << weightPoints[*iit][2]<<" " <<weightPoints[*iit][3]  << endl;
		//	cout<<input->m_Atoms[i]->position[0]<<" " <<input->m_Atoms[i]->position[1] <<" " <<input->m_Atoms[i]->position[2]  << endl;
			if(fabs(weightPoints[*iit][0]-input->m_Atoms[i]->position[0])<0.001  &&
			fabs(weightPoints[*iit][1]-input->m_Atoms[i]->position[1])<0.001 && 
			fabs(weightPoints[*iit][2]-input->m_Atoms[i]->position[2]) < 0.001 )
			{
...
//		cout<<"atom i has "<< " " << input->m_Atoms[i]->radius <<" " << input->m_Atoms[i]->charge << endl;
	}
...
{
	cout<<endl<<"Usage: ./Molsurf -amberAtom2surfaceDistPQR <inputPQR>  <inputMesh> <inputAmberOutput> <outputCDR>";
	cout<<endl<<"Info. outputCDR is (Q)charge replace by (D)istance	to the surface and (R) replaced by  Born radius from Amber output.";
...
//		meshPoints.push_back(p);
   ...
   for(int i=0; i<input->m_Atoms.size(); i++)
   {
		...
		{
			cout<<"Error reading the Born file, exit! i= "<< i<< " ID is  " << text[0] << endl;
			return false;
		}
...
{
	cout<<endl<<"Usage: ./Molsurf -amberMDExtractInfo <inputPQR>  <input [force (.for)], [acceleration (.acc)], [velocity (.vel)]> <output root>";
	cout<<endl<<"Or  ./Molsurf -amberMDExtractInfo <inputPQR> <input velocity (.vel)> <input [force (.for)], [acceleration (.acc)], [velocity (.vel)]> <float dt> <output root>";
		...
		if(founda != string::npos){
			...
			if(foundb != string::npos){
				...
				for(int i=0; i< input->m_Atoms.size(); i++)
				{
					...
					kinea += 0.5 * (vel[0]*vel[0] + vel[1]*vel[1] + vel[2]*vel[2])*input->m_Atoms[i]->mass;
					kineb += 0.5 * (velb[0]*velb[0] + velb[1]*velb[1] + velb[2]*velb[2])*input->m_Atoms[i]->mass;
...
{
	cout<<endl<<"Usage: ./Molsurf -cvcAtom2surfaceDistPQR <inputPQR>  <inputMesh> <inputCVCBornOutput> <outputCDR>";
	cout<<endl<<"Info. outputCDR is (Q)charge replace by (D)istance	to the surface and (R) replaced by Born radius from CVC Born radius output.";
	cout<<endl<<"Warning: Not a standard PQR output.";
	cout<<endl;
}
...
//		meshPoints.push_back(p);
   ...
   if(atmNum!= input->m_Atoms.size())
   {
   	  cout<<"Atom numbers in PQR file and Born radii file are not consistent!"<< endl;
	  return false;
   }
...
{
	cout<<"MolSurf -changeCharge4PQR <inputPQRA> <inputPQRB> <outputPQRC> " << endl;
...
bool	ChangeCharge4PQR(int argc, char* argv[])
{
	...
	for(int i=0; i< input->m_Atoms.size(); i++)
	{
	//		cout<< "atom i: " <<i<<" "<< input->m_Atoms[i]->position[0]<<" " <<input->m_Atoms[i]->position[1] <<" " <<input->m_Atoms[i]->position[2] <<" " << input->m_Atoms[i]->radius << endl;

		for(wit = WeightPoints.begin(); wit < WeightPoints.end(); wit++)
		{
	//		cout<< (*wit).p[0] << " " <<  (*wit).p[1] << " " <<  (*wit).p[2] << " " <<  (*wit).rad << endl;
			...
			{
				cout<<"Warning: no match for atom: " << i <<"using 1.0 as default"<< endl;
				input->m_Atoms[i]->setCharge(1.0);
			}
		}
...
{
	cout<<"MolSurf -setVDWParametersFromAmber <inputPQR> <input aminolib> <input amino nt lib> <input amino ct lib> <input param file>  " << endl;
}
...
//							text[1].erase(text[1].begin());
							...
							text[0].erase(remove(text[0].begin(), text[0].end(),'"'), text[0].end());
							text[1].erase(remove(text[1].begin(), text[1].end(),'"'), text[1].end());
...
//		cout<<"atom id: atom name, atomtype,  res id, res name: "<< i <<" " << input->m_Atoms[i]->atomName << " "
//		<<" " << input->m_Atoms[i]->atomType << " "<<  input->m_Atoms[i]->getResIndex() << " " << input->m_Atoms[i]->getResName() << endl;
	}
	...
	for(int i=0; i< input->m_Atoms.size(); i++)
	{
		...
		if(tmpAtomType[0] == 'C') {
		   if( strcmp( tmpAtomType, "CT" ) && strcmp( tmpAtomType, "C" ) && strcmp( tmpAtomType, "C0" ) && strcmp( tmpAtomType, "Cs" ) && strcmp( tmpAtomType, "Cl" ) )
		...
		//	cout<<"atom id: atom name, atomtype,   LJ r, LJ e: "<< i<< " "<<  input->m_Atoms[i]->atomName << " "<<  input->m_Atoms[i]->atomType <<" " << input->m_Atoms[i]->ljradius << " "<< input->m_Atoms[i]->ljwellth << endl;
...
void printUsage()
{
  ...
  cout << "      surfaceUsingHSL, surfaceUsingLBIE, surfaceUsingFastSurf " << endl;
  cout << "      area, volume, qualityImprove, removeInteriorPockets, normals,reoriente, aSpline" << endl;
  cout << "      offsetSurface, mergeGeometry" << endl;
  cout<<"      volumeUsingHLS, getVolumeWithElectron, getVolumeWithHydrophobicity, getVoluemWithChargeDensity" << endl;
  cout << "if WITH_CG, more commands,  mergeProtein, splitPDB2Chains, PQR2PDB,PQR2XYZR, colorMeshByAtomType, atom2surfaceDistance" << endl;
  cout	<<" tinkerPDBXYZ2PQR,changeCharge4PQR, amberAtom2surfaceDistPQR, cvcAtom2surfaceDistPQR, setVDWParametersFromAmber " << endl;
