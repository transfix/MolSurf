...
Octree::Octree(std::string pqrFN,  std::string rawFN, int mindpth, int maxdpth, float blob, float iso)
{
	minDepth = mindpth;
	maxDepth = maxdpth;
	blobbyness = blob;
	...
	if(readPQR(pqrFN))
	{
		...
		if(initDataStructures())
		{
			...
			if(buildTree())		// computes values and splits the cells recursively. after termination the activeNodes contains all leaves at depth = maxDepth
			{
				...
				cout<<"Total number of triangles: "<<allMeshSimplices.size()<<endl;
				cout<<"Total number of mesh vertices: "<<allMeshVertices.size()<<endl;
...
bool Octree::initDataStructures()
{
	...
	if(minbbox[1]<dpgxlate) dpgxlate = minbbox[1];
	if(minbbox[2]<dpgxlate) dpgxlate = minbbox[2];
	...
	root = 0;
	allNodes.clear();
	allNodes.push_back(new OTCell(vs, (minbbox[0]+maxbbox[0])/2.0, (minbbox[1]+maxbbox[1])/2.0, (minbbox[2]+maxbbox[2])/2.0, size, 1));
...
//		cout<<"Already split"<<endl;
	...
	float xs[3] = {allVertices[cell->vertices[0]]->xyz[0], (allVertices[cell->vertices[0]]->xyz[0]+allVertices[cell->vertices[7]]->xyz[0])/2.0, allVertices[cell->vertices[7]]->xyz[0]};
	float ys[3] = {allVertices[cell->vertices[0]]->xyz[1], (allVertices[cell->vertices[0]]->xyz[1]+allVertices[cell->vertices[7]]->xyz[1])/2.0, allVertices[cell->vertices[7]]->xyz[1]};
	float zs[3] = {allVertices[cell->vertices[0]]->xyz[2], (allVertices[cell->vertices[0]]->xyz[2]+allVertices[cell->vertices[7]]->xyz[2])/2.0, allVertices[cell->vertices[7]]->xyz[2]};
...
//					cout<<"vertex "<<i<<" "<<j<<" "<<k<<" : [ "<<xs[i]<<" "<<ys[j]<<" "<<zs[k]<<" ]"<<" with id "<<verts[i][j][k]<<endl;
				}
			}
		}
	}
...
//				cout<<" ]"<<endl;
				...
				allNodes.push_back(new OTCell(vs, (xs[ii]+xs[ii+1])/2.0, (ys[jj]+ys[jj+1])/2.0, (zs[kk]+zs[kk+1])/2.0, cell->span/2.0, cell->depth+1));
			}
		}
	}
...
//	cout<<"Trying to locate "<<x<<"  "<<y<<"  "<<z<<endl;
	...
	while(result->depth < depth && !result->isLeaf)
	{
		...
		if(cx < x) childIndex += 4;
		if(cy < y) childIndex += 2;
		if(cz < z) childIndex += 1;

//		cout<<"New cell at depth "<<depth<<endl;
//		cout<<"Position "<<cx<<"  "<<cy<<"  "<<cz<<endl;
//		cout<<"Moving to child index "<<childIndex<<endl;
//		cout<<"with node id "<<result->children[childIndex]<<endl<<endl;
		
		result = allNodes[result->children[childIndex]];
	}

//	cout<<"Located node "<<result->xyz[0]<<"  "<<result->xyz[1]<<"  "<<result->xyz[2]<<endl<<endl<<endl;
...
//	cout<<"Marking neighbors of cell at position " << cx << " " << cy << " " << cz << "   with span " << sp << "  and depth " << dpth << endl;
...
void Octree::vmarchcube(OTCell* cell)
{
	...
	for(iVertex = 0; iVertex < 8; iVertex ++)
	{
		...
		if(allVertices[cell->vertices[marchingCubeIndex]]->value <= isoValue) iFlagIndex |= 1<<iVertex;
	}
	...
	for(iEdge = 0; iEdge < 12; iEdge ++)
	{
		...
		if(iEdgeFlags & (1<<iEdge))
		{
			...
			fOffset = fGetOffset(allVertices[cell->vertices[endpoint1index]]->value, allVertices[cell->vertices[endpoint2index]]->value, isoValue);

			float x = (1-fOffset)*allVertices[cell->vertices[endpoint1index]]->xyz[0] + fOffset*allVertices[cell->vertices[endpoint2index]]->xyz[0];
			float y = (1-fOffset)*allVertices[cell->vertices[endpoint1index]]->xyz[1] + fOffset*allVertices[cell->vertices[endpoint2index]]->xyz[1];
			float z = (1-fOffset)*allVertices[cell->vertices[endpoint1index]]->xyz[2] + fOffset*allVertices[cell->vertices[endpoint2index]]->xyz[2];
	...
	for(iTriangle = 0; iTriangle < 5; iTriangle ++)
	{
		...
		int iVertex1 = vertind[ a2iTriangleConnectionTable[iFlagIndex][3*iTriangle] ];
		int iVertex2 = vertind[ a2iTriangleConnectionTable[iFlagIndex][3*iTriangle + 1] ];
		int iVertex3 = vertind[ a2iTriangleConnectionTable[iFlagIndex][3*iTriangle + 2] ];
...
bool Octree::computeValue(OTVertex* v)
{
	...
	{
		float ax = nearAtoms[i]->getX(), ay = nearAtoms[i]->getY(), az = nearAtoms[i]->getZ(), r = ((DPG::Ball*)nearAtoms[i])->getRadius();
		float distsq = (x-ax)*(x-ax) + (y-ay)*(y-ay) + (z-az)*(z-az);
...
void Octree::PrintMesh(string fname) 
{
	...
	{
		fout << allMeshVertices[i].xyz[0] << " " << allMeshVertices[i].xyz[1] << " " << allMeshVertices[i].xyz[2] << endl;
	}
	for (int i=0; i<allMeshSimplices.size(); i++) 
	{
		fout << allMeshSimplices[i].verts[0] << " " << allMeshSimplices[i].verts[1] << " " << allMeshSimplices[i].verts[2] << endl;
	}
}
...
bool Octree::readPQR(string fname) 
{
	...
	size = dimX;
	if(size < dimY) size = dimY;
	if(size < dimZ) size = dimZ;
