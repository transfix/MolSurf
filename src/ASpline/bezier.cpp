...
void ASpline::Triangle_Bezier_Patch(int resolution) {
  int count = 0;
  double b300[3], b210[3], b120[3], b030[3], b021[3], b012[3], b003[3], b102[3],
      b201[3];
  double b111_1[9], b111_2[9], b111_3[9];
  for (int i = 0; i < numbtris; i++) {
    double p1[3], p2[3], p3[3], n1[3], n2[3], n3[3];
    for (int j = 0; j < 3; j++) {
      p1[j] = verticies[faces[i]->indexInRAW[0]]->point[j];
      p2[j] = verticies[faces[i]->indexInRAW[1]]->point[j];
      p3[j] = verticies[faces[i]->indexInRAW[2]]->point[j];
      n1[j] = verticies[faces[i]->indexInRAW[0]]->normal[j];
      n2[j] = verticies[faces[i]->indexInRAW[1]]->normal[j];
      n3[j] = verticies[faces[i]->indexInRAW[2]]->normal[j];
    }

    /* Compute Bezier coefficients    */
    Cubic_Bezier_Coeffs(p1, p2, p3, n1, n2, n3, b300, b210, b120, b030, b021,
                        b012, b003, b102, b201);
    /* Compute three b_{111}          */
    Simple_Bezier_B111(p1, p2, p3, n1, n2, n3, b300, b210, b120, b030, b201,
                       b021, b111_3);
    Simple_Bezier_B111(p2, p3, p1, n2, n3, n1, b030, b021, b012, b003, b120,
                       b102, b111_1);
    Simple_Bezier_B111(p3, p1, p2, n3, n1, n2, b003, b102, b201, b300, b012,
                       b210, b111_2);
    /* Compute the inner nodes                            */
    Triangle_Patch(i, p1, p2, p3, n1, n2, n3, b300, b210, b120, b030, b021,
                   b012, b003, b102, b201, b111_1, b111_2, b111_3, resolution);
  }
}

void ASpline::Triangle_Bezier_Patch_With_Quadrature(const char *type,
                                                    int numOfPts,
                                                    int resolution) {
  int count = 0;
  Quadrature *my_quadr = new Quadrature(type, numOfPts);
  double b300[3], b210[3], b120[3], b030[3], b021[3], b012[3], b003[3], b102[3],
      b201[3];
  double b111_1[9], b111_2[9], b111_3[9];
  for (int i = 0; i < numbtris; i++) {
    ...
    Simple_Bezier_B111(p3, p1, p2, n3, n1, n2, b003, b102, b201, b300, b012,
                       b210, b111_2);
    /* Compute the inner nodes                            */
    Triangle_Patch_With_Quadrature(my_quadr, i, p1, p2, p3, n1, n2, n3, b300,
                                   b210, b120, b030, b021, b012, b003, b102,
                                   b201, b111_1, b111_2, b111_3, resolution);
  }
}

void ASpline::Triangle_Bezier_Patch_Nodes(const char *type, int numOfPts) {
  double b300[3], b210[3], b120[3], b030[3], b021[3], b012[3], b003[3], b102[3],
      b201[3];
  double b111_1[9], b111_2[9], b111_3[9];
  int i;
  ...
  for (i = 0; i < numbtris; i++) {
    ...
    Triangle_Patch_Nodes(my_quadr, i, p1, p2, p3, n1, n2, n3, b300, b210, b120,
                         b030, b021, b012, b003, b102, b201, b111_1, b111_2,
                         b111_3);
    ...
    //    Gaussian_Triangle_Patch(i,p1,p2,p3,n1,n2,n3,b300,b210,b120,b030,b021,b012,b003,b102,
    //    b201,b111_1,b111_2,b111_3);
  }
}

void ASpline::Triangle_Patch_Volume() {
  double b300[3], b210[3], b120[3], b030[3], b021[3], b012[3], b003[3], b102[3],
      b201[3];
  double b111_1[9], b111_2[9], b111_3[9];
  int i;
  Quadrature *my_quadr = new Quadrature((char *)"Gaussian", 3); // 3-pt Gaussian
  volume = 0.0;
  for (i = 0; i < numbtris; i++) {
    double p1[3], p2[3], p3[3], n1[3], n2[3], n3[3];
    int j;
    ...
    Simple_Bezier_B111(p3, p1, p2, n3, n1, n2, b003, b102, b201, b300, b012,
                       b210, b111_2);
    // Compute Quadrature nodes and weights
    volume += Patch_Volume(my_quadr, i, p1, p2, p3, n1, n2, n3, b300, b210,
                           b120, b030, b021, b012, b003, b102, b201, b111_1,
                           b111_2, b111_3);
    // Compute Gaussian nodes and weights over linear piece
    //        Gaussian_Triangle(i,p1,p2,p3,n1,n2,n3);
    // Compute Gaussian nodes and weights over cubic patch
...
void ASpline::Cubic_Bezier_Coeffs(double *p1, double *p2, double *p3,
                                  double *n1, double *n2, double *n3,
                                  double *b300, double *b210, double *b120,
                                  double *b030, double *b021, double *b012,
                                  double *b003, double *b102, double *b201) {
  int j;
...
void ASpline::Simple_Bezier_B111(double *p1, double *p2, double *p3, double *n1,
                                 double *n2, double *n3, double *b300,
                                 double *b210, double *b120, double *b030,
                                 double *b201, double *b021, double *b111) {
  int i;
  double v21[3], v13[3], v23[3], n21[3], n13[3], n23[3], an12[3], m1[3], m2[3];
  ...
  */
  ...
  det[1] = Determinant(n13, v23, an12) + Determinant(v13, n23, an12);
  ...
  Flmd[0] =
      (b300[1] + 3 * (b210[1] + b120[1]) + b030[1]) / 8.0; // constant term
  Flmd[1] =
      (b300[2] + 3 * (b210[2] + b120[2]) + b030[2]) / 4.0; // 1st degree term
  ...
  */
  ...
  left[4] = 0.0;
  /*                     End  */
  /* The above change did on 2/8 does not affect the result because d3 dot an12
   * = 0 */
...
// arand: 4/22/2010. new function for computing refined a-spline triangulation
// and quadrature together
void ASpline::Triangle_Patch_With_Quadrature(
    Quadrature *my_quadr, int t, double *p1, double *p2, double *p3, double *n1,
    double *n2, double *n3, double *b300, double *b210, double *b120,
    double *b030, double *b021, double *b012, double *b003, double *b102,
    double *b201, double *b111_1, double *b111_2, double *b111_3,
    int resolution) {
  // do the subdivision...
  Triangle_Patch(t, p1, p2, p3, n1, n2, n3, b300, b210, b120, b030, b021, b012,
                 b003, b102, b201, b111_1, b111_2, b111_3, resolution);
  double b1, b2, b3;
  int i, j, k, l;
  ...
  // get quadrature for each subfacet
  //
  //           ----
  //          |\   |
  ...
  for (j = 0; j < resolution; j++) {
    ...
    for (i = 0; i < resolution - 1 - j; i++) {
      ...
      // b1,b2,b3 is the barycentric coords for the lower left corner...
      for (k = 0; k < my_quadr->numOfPts_perTri; k++) {
        // add quadrature points for two triangles
        double b1lower = b1 + my_quadr->xi1[k] * scale;
        double b2lower = b2 + my_quadr->xi2[k] * scale;
        double b3lower = 1.0 - b1lower - b2lower;
        lmd = BB2lmd(b300, b210, b120, b030, b021, b012, b003, b102, b201,
                     b111_1, b111_2, b111_3, b1lower, b2lower, b3lower);
        for (l = 0; l < 3; l++) {
          p1_lmd[l] = p1[l] + lmd * n1[l];
          p2_lmd[l] = p2[l] + lmd * n2[l];
          p3_lmd[l] = p3[l] + lmd * n3[l];
        }
        BB_Gradient(p1_lmd, p2_lmd, p3_lmd, n1, n2, n3, b300, b210, b120, b030,
                    b021, b012, b003, b102, b201, b111_1, b111_2, b111_3,
                    b1lower, b2lower, b3lower, lmd, n);
        Vertex *node = new Vertex();
        for (l = 0; l < 3; l++) {
          node->point[l] =
              b1lower * p1_lmd[l] + b2lower * p2_lmd[l] + b3lower * p3_lmd[l];
          node->normal[l] = n[l];
        }
        x_b1 = p1_lmd[0] - p3_lmd[0];
        y_b1 = p1_lmd[1] - p3_lmd[1];
        z_b1 = p1_lmd[2] - p3_lmd[2];
        x_b2 = p2_lmd[0] - p3_lmd[0];
        y_b2 = p2_lmd[1] - p3_lmd[1];
        z_b2 = p2_lmd[2] - p3_lmd[2];
        E = x_b1 * x_b1 + y_b1 * y_b1 + z_b1 * z_b1;
        F = x_b1 * x_b2 + y_b1 * y_b2 + z_b1 * z_b2;
        G = x_b2 * x_b2 + y_b2 * y_b2 + z_b2 * z_b2;
        node->weight = my_quadr->w[k] * sqrt(E * G - F * F) * scale * scale /
                       2.0; // arand: added scale
        faces[t]->node.push_back(node);
        double b1upper = b1 + scale - my_quadr->xi1[k] * scale;
        double b2upper = b2 + scale - my_quadr->xi2[k] * scale;
        double b3upper = 1.0 - b1upper - b2upper;
        lmd = BB2lmd(b300, b210, b120, b030, b021, b012, b003, b102, b201,
                     b111_1, b111_2, b111_3, b1upper, b2upper, b3upper);
        for (l = 0; l < 3; l++) {
          p1_lmd[l] = p1[l] + lmd * n1[l];
          p2_lmd[l] = p2[l] + lmd * n2[l];
          p3_lmd[l] = p3[l] + lmd * n3[l];
        ...
        BB_Gradient(p1_lmd, p2_lmd, p3_lmd, n1, n2, n3, b300, b210, b120, b030,
                    b021, b012, b003, b102, b201, b111_1, b111_2, b111_3,
                    b1upper, b2upper, b3upper, lmd, n);
        node = new Vertex();
        for (l = 0; l < 3; l++) {
          node->point[l] =
              b1upper * p1_lmd[l] + b2upper * p2_lmd[l] + b3upper * p3_lmd[l];
          node->normal[l] = n[l];
        }
        x_b1 = p1_lmd[0] - p3_lmd[0];
        y_b1 = p1_lmd[1] - p3_lmd[1];
    ...
    // add quadrature points for last triangle
    b1 = ((double)resolution - 1.0 - (double)j) / resolution;
    for (k = 0; k < my_quadr->numOfPts_perTri; k++) {
      // add quadrature points for two triangles
      double b1lower = b1 + my_quadr->xi1[k] * scale;
      double b2lower = b2 + my_quadr->xi2[k] * scale;
      double b3lower = 1.0 - b1lower - b2lower;
      lmd = BB2lmd(b300, b210, b120, b030, b021, b012, b003, b102, b201, b111_1,
                   b111_2, b111_3, b1lower, b2lower, b3lower);
      for (l = 0; l < 3; l++) {
        p1_lmd[l] = p1[l] + lmd * n1[l];
        p2_lmd[l] = p2[l] + lmd * n2[l];
        p3_lmd[l] = p3[l] + lmd * n3[l];
      ...
      BB_Gradient(p1_lmd, p2_lmd, p3_lmd, n1, n2, n3, b300, b210, b120, b030,
                  b021, b012, b003, b102, b201, b111_1, b111_2, b111_3, b1lower,
                  b2lower, b3lower, lmd, n);
      Vertex *node = new Vertex();
      for (l = 0; l < 3; l++) {
        node->point[l] =
            b1lower * p1_lmd[l] + b2lower * p2_lmd[l] + b3lower * p3_lmd[l];
...
// compute refined triangulation on the A-Spline
void ASpline::Triangle_Patch(int t, double *p1, double *p2, double *p3,
                             double *n1, double *n2, double *n3, double *b300,
                             double *b210, double *b120, double *b030,
                             double *b021, double *b012, double *b003,
                             double *b102, double *b201, double *b111_1,
  ...
  int i, j, k;
  int *p_index =
      new int[(resolution + 1) * (resolution + 2) / 2]; // new vertices
  int numOfVts_now = verticies.size();                  // original vertices
  int count1 = 0;
  int count2 = 0;
  for (j = 0; j < resolution + 1; j++) {
    ...
    for (i = 0; i < resolution + 1 - j; i++) {
      b1 = (double)i / resolution;
      b3 = 1.0 - b1 - b2;
      // cout << "Bary " << b1 << " " << b2 << " " << b3 << endl;
      if (b1 != 1.0 && b2 != 1.0 && b3 != 1.0) {
        // compute lambda value in A-spline representation given bezier
        // coefficients and the barycentric coordinates
        lmd = BB2lmd(b300, b210, b120, b030, b021, b012, b003, b102, b201,
                     b111_1, b111_2, b111_3, b1, b2, b3);
        for (k = 0; k < 3; k++) {
          p1_lmd[k] = p1[k] + lmd * n1[k];
          p2_lmd[k] = p2[k] + lmd * n2[k];
          p3_lmd[k] = p3[k] + lmd * n3[k];
        }
        // get the normal to the surface at the new point
        BB_Gradient(p1_lmd, p2_lmd, p3_lmd, n1, n2, n3, b300, b210, b120, b030,
                    b021, b012, b003, b102, b201, b111_1, b111_2, b111_3, b1,
                    b2, b3, lmd, n);
        Vertex *p = new Vertex();
        // compute the new vertex location and assign its normal
        for (k = 0; k < 3; k++) {
          p->point[k] = b1 * p1_lmd[k] + b2 * p2_lmd[k] + b3 * p3_lmd[k];
          p->normal[k] = n[k];
          p->color[k] = verticies[faces[t]->indexInRAW[0]]->color[k];
        }
        // cout << endl;
        addPoint(p);
      ...
      if (b1 == 1.0) {
        p_index[count1] = faces[t]->indexInRAW[0];
      }
      if (b2 == 1.0) {
        p_index[count1] = faces[t]->indexInRAW[1];
      }
      if (b3 == 1.0) {
        p_index[count1] = faces[t]->indexInRAW[2];
      }
      count1++;
    }
  }
  for (j = 0; j < resolution; j++) {
    for (i = 0; i < resolution - 1 - j; i++) {
      ...
      faces[t]->subFacets.push_back(f_low);
      Face *f_up = new Face();
      ...
      faces[t]->subFacets.push_back(f_up);
    }
    Face *f_end = new Face();
    ...
    faces[t]->subFacets.push_back(f_end);
  }
}

void ASpline::Triangle_Patch_Nodes(Quadrature *my_quadr, int i, double *p1,
                                   double *p2, double *p3, double *n1,
                                   double *n2, double *n3, double *b300,
                                   double *b210, double *b120, double *b030,
                                   double *b021, double *b012, double *b003,
                                   double *b102, double *b201, double *b111_1,
                                   double *b111_2, double *b111_3) {
  double lmd;
  double p1_lmd[3], p2_lmd[3], p3_lmd[3];
  double n[3];
  double x_b1, y_b1, z_b1, x_b2, y_b2, z_b2, E, F, G;
  ...
  for (k = 0; k < my_quadr->numOfPts_perTri; k++) {
    ...
    BB_Gradient(p1_lmd, p2_lmd, p3_lmd, n1, n2, n3, b300, b210, b120, b030,
                b021, b012, b003, b102, b201, b111_1, b111_2, b111_3, b1, b2,
                b3, lmd, n);
    Vertex *node = new Vertex();
    for (j = 0; j < 3; j++) {
    ...
    y_b2 = p2_lmd[1] - p3_lmd[1];
    z_b2 = p2_lmd[2] - p3_lmd[2];
    E = x_b1 * x_b1 + y_b1 * y_b1 +
        z_b1 * z_b1; // see Wenqi Zhao's thesis to make sense of this...
    F = x_b1 * x_b2 + y_b1 * y_b2 + z_b1 * z_b2;
    G = x_b2 * x_b2 + y_b2 * y_b2 + z_b2 * z_b2;
    node->weight =
        my_quadr->w[k] * sqrt(E * G - F * F) / 2.0; // added /2.0 on 12-6-2011
    faces[i]->node.push_back(node);
  }
}

double ASpline::Patch_Volume(Quadrature *my_quadr, int i, double *p1,
                             double *p2, double *p3, double *n1, double *n2,
                             double *n3, double *b300, double *b210,
                             double *b120, double *b030, double *b021,
                             double *b012, double *b003, double *b102,
                             double *b201, double *b111_1, double *b111_2,
                             double *b111_3) {
  double lmd;
  double p1_lmd[3], p2_lmd[3], p3_lmd[3];
  double n[3];
  ...
  int j;
  double p_volume = 0.0;
  double tri_area = Triangle_area(p1, p2, p3);
  int k;
  for (k = 0; k < my_quadr->numOfPts_perTri; k++) {
    b1 = my_quadr->xi1[k];
    b2 = my_quadr->xi2[k];
    ...
    lmd = BB2lmd(b300, b210, b120, b030, b021, b012, b003, b102, b201, b111_1,
                 b111_2, b111_3, b1, b2, b3);
    p_volume += tri_area * my_quadr->w[k] * lmd;
  }
  return p_volume;
}

double ASpline::BB2lmd(double *b300, double *b210, double *b120, double *b030,
                       double *b021, double *b012, double *b003, double *b102,
                       double *b201, double *b111_1, double *b111_2,
                       double *b111_3, double b1, double b2, double b3) {
  double lmd;
  int i;
...
double ASpline::Bezier_LambdaPolyZeroC1(double *F1, double *N1, double *N2,
                                        double *N3, double *D1, double *D2,
                                        double *D3) {
  double DF1[2], DN1[7], DN2[7], DN3[7], W1[5], W2[5], W3[5], WW1[7], WW2[7],
      WW3[7];
...
double ASpline::Bezier_ThreeLambdaPolyValueC1(double *F1, double *N1,
                                              double *N2, double *N3,
                                              double *D1, double *D2,
                                              double *D3, double lmd) {
  double w, f1, f2, f3, F;
...
void ASpline::BB_Gradient(double *p1_lmd, double *p2_lmd, double *p3_lmd,
                          double *n1, double *n2, double *n3, double *b300,
                          double *b210, double *b120, double *b030,
                          double *b021, double *b012, double *b003,
                          double *b102, double *b201, double *b111_1,
                          double *b111_2, double *b111_3, double b1, double b2,
                          double b3, double lmd, double *n) {
  double bb200, bb020, bb002, bb110, bb101, bb011, w1, w2, w3, w;
  double bb300, bb030, bb003, bb210, bb201, bb120, bb021, bb102, bb012, bb111;
  double pol[3], p31[4], p32[4], p33[4], q21[3], q22[3], q23[3];
  double dpol[2], DN1[7], DN2[7], DN3[7], W1[5], W2[5], W3[5], WW1[7], WW2[7],
      WW3[7];
  ...
  df_lmd = Bezier_ThreeLambdaDervPolyValueC1(dpol, DN1, DN2, DN3, q21, q22, q23,
                                             lmd);
  /* Compute T^{-1}  */
  for (i = 0; i < 3; i++) {
...
double ASpline::Bezier_ThreeLambdaDervPolyValueC1(double *F1, double *N1,
                                                  double *N2, double *N3,
                                                  double *D1, double *D2,
                                                  double *D3, double lmd) {
  double w1, w2, w3, F;
...
double ASpline::Triangle_area(double *v1, double *v2, double *v3) {
  /* Heron's formula  */
  double a, b, c, s;
  a = sqrt((v1[0] - v2[0]) * (v1[0] - v2[0]) +
           (v1[1] - v2[1]) * (v1[1] - v2[1]) +
           (v1[2] - v2[2]) * (v1[2] - v2[2]));
  b = sqrt((v2[0] - v3[0]) * (v2[0] - v3[0]) +
           (v2[1] - v3[1]) * (v2[1] - v3[1]) +
           (v2[2] - v3[2]) * (v2[2] - v3[2]));
  c = sqrt((v1[0] - v3[0]) * (v1[0] - v3[0]) +
           (v1[1] - v3[1]) * (v1[1] - v3[1]) +
           (v1[2] - v3[2]) * (v1[2] - v3[2]));
  s = (a + b + c) / 2.0;
