...
float VolumeExcludedFunction(float dis, float radius, float width)
{
	...
	{
		return (0.5+ 0.75*(dis - radius)/width - 0.25 * (dis-radius)*(dis-radius)*(dis-radius)/(width*width*width));
	}
	else
	{
...
bool getAtomListAndExtent(GroupOfAtoms* molecule, vector<PDBParser::Atom*> & atomList, float* minExt, float* maxExt)
{
	if(!molecule)
	{
		return false;
	...
	for(int i=0; i< atomList.size(); i++)
	{
		...
		for(int j=0; j<3; j++)
		{
			stapnt[j]=(atomList[i]->m_Position[j]-radius)<stapnt[j]?atomList[i]->m_Position[j]-radius:stapnt[j];
			endpnt[j]=(atomList[i]->m_Position[j]+radius)>endpnt[j]?atomList[i]->m_Position[j]+radius:endpnt[j];
		}
	}
	float scale= 1.2;
	for(int j=0; j<3; j++)
	{
		minExt[j] = (endpnt[j] + stapnt[j])/2.0-(endpnt[j] - stapnt[j])*scale/2.0;
		maxExt[j] = minExt[j] + (endpnt[j]-stapnt[j])*scale;
	}
	return true;
}

void getVolumeData(float* data,unsigned  int* dim, vector<PDBParser::Atom*> & atomList, float* minExt, float dx, bool SAS)
{
		...
		for(u[0] = a[0]-range; u[0]<a[0]+range; u[0]++)
		{
			...
			for(u[1] = a[1]-range; u[1]<a[1]+range; u[1]++)
			{
				...
				for(u[2] = a[2]-range; u[2]<a[2]+range; u[2]++)
				{
					...
					{
						p[j]= minExt[j]+ u[j]*dx;
						distance += (p[j]-atomList[i]->m_Position[j])*(p[j]-atomList[i]->m_Position[j]);
					}
					distance = sqrt(distance);
					//cout<<"function value: " << VolumeExcludedFunction(distance, radius, width)<<endl;
...
int constructFastMoleSurface(int argc, char* argv[])
{
  /*
	if(argc<7)
	{
		cout<<"Usage: ./FastMoleSurface <input.pdb> <dim0> <dim1> <dim2> <bool SAS> <output.raw>" <<endl;
		return 1;
	}
  */
	...
	cout<<"Molecule "<<argv[2] <<" has "<< atomList.size() <<" atoms." <<endl;
	float interval[3];
	for(int i=0; i<3; i++)
	{
	...
	delete geome;
	delete sdata;  //sdata has deleted the data for me. It shouldn't work like this.
	cout<<"done!"<<endl;
	return 0;
}
