...
bool PG::range(Point *q, double delta, vector<Point *> &result) {
  ...
  int lx, hx, ly, hy, lz,
      hz; // used for defining the integer range. l for low, h for high
  ...
  lz = cz - d - 1;
  hz = cz + d + 1;
  vector<tuple<plane *>> planesInRange =
      g.RR->report(lz, hz); // searching planes in grid
  ...
  else // some planes found in the range.
  {
    ...
    for (int i = 0; i < numPlanes;
         i++) // for each plane, look for lines in them within the range
    {
      /*                      int cellDistZ = cz - (planesInRange[i].ptr)->ID.z;
                              int cellDistZSq = cellDistZ*cellDistZ;

                              int reducedQueryDistYSq = dSq - cellDistZSq; // in
         planes further from the point, a smaller range need to checked int
         reducedQueryDistY = sqrt(reducedQueryDistYSq);      // due to the query
         being spherical
      ...
      if (linesInRange.empty())
        ...
        for (int j = 0; j < numLines; j++) {
          /*                                      int cellDistY = cy -
             (linesInRange[j].ptr)->ID.y; int cellDistYSq = cellDistY*cellDistY;

                                                  int reducedQueryDistXSq =
             reducedQueryDistYSq - cellDistYSq;    // in lines further from the
             point, a smaller range need to checked int reducedQueryDistX =
             sqrt(reducedQueryDistXSq);              // due to the query being
             spherical.

                                                  lx = cx - reducedQueryDistX -
             1; hx = cx + reducedQueryDistX + 1;*/
        ...
        for (int j = 0; j < numLines; j++) {
          /*                                      int cellDistY = cy -
             (linesInRange[j].ptr)->ID.y; int cellDistYSq = cellDistY*cellDistY;
                                                  ...
                                                  int reducedQueryDistXSq =
             reducedQueryDistYSq - cellDistYSq;    // in lines further from the
             point, a smaller range need to checked

                                                  int reducedQueryDistX =
             sqrt(reducedQueryDistXSq);              // due to the query being
             spherical.
...
void PG::addPoint(Point *a) {
  ...
  vector<tuple<line *>> linesInRange =
      planesInRange[0].ptr->RR->report(cy, cy); // identify line
  ...
  vector<tuple<gridcell *>> cellsInRange =
      linesInRange[0].ptr->RR->report(cx, cx); // identify cell
...
void PG::removePoint(Point *a) {
  ...
  vector<tuple<gridcell *>> cellsInRange =
      linesInRange[0].ptr->RR->report(cx, cx);
  if (cellsInRange.empty()) {
    cout << "Atom does not exist" << endl;
    return;
  }
  ...
  // cout<<"Given Point = "<<a->getX()<<" "<<a->getY()<<" "<<a->getZ()<<"
  // "<<endl;

  for (int i = 0; i < (int)(cellsInRange[0].ptr->balls).size(); i++) {
    if (a->getX() == cellsInRange[0].ptr->balls[i]->getX() &&
        a->getY() == cellsInRange[0].ptr->balls[i]->getY() &&
        a->getZ() == cellsInRange[0].ptr->balls[i]->getZ()) // found
    {
      index = i;
      break;
    }
  ...
  if (index == -1) {
    ...
    cellsInRange[0].ptr->balls[index] =
        cellsInRange[0].ptr->balls[cellsInRange[0].ptr->balls.size() - 1];
