...
// Get Functions
int BufferedIO::get(char *data, int n) { return getraw(data, sizeof(char), n); }
...
// Put Functions
void BufferedIO::put(const char *data, int n) { putraw(data, n); }

void BufferedIO::put(const unsigned char *data, int n) { putraw(data, n); }
...
bool BufferedIO::open() {
  ...
  switch (m_mode) {
  case DiskIO::READ:
    if ((m_fp = fopen(m_fname, "r+b")) == NULL) {
      return false;
    }
    break;
  case DiskIO::WRITE:
    if ((m_fp = fopen(m_fname, "w+b")) == NULL) {
      return false;
    }
    break;
  default:
...
bool BufferedIO::reopen() {
  ...
  case DiskIO::READ:
    ...
    break;
  case DiskIO::WRITE:
    if ((m_fp = fopen(m_fname, "a+b")) == NULL) {
      return false;
    }
    break;
  default:
...
void BufferedIO::flush(bool fill) {
  switch (m_mode) {
  case DiskIO::READ:
    // !!! read header only moves to the next disk block boundary
    if (m_bufptr % DBSIZE != 0) {
      m_bufptr += (DBSIZE - (m_bufptr % DBSIZE));
    }
    break;
  case DiskIO::WRITE:
    // writes out all modified bytes up to a disk block boundary
    ...
    if (m_bufptr != 0) {
      fwrite(m_buffer, 1, m_bufptr, m_fp);
    }
...
int BufferedIO::getraw(void *data, int size, int nu) {
  ...
  while (!m_eof && (m_bufptr + size * nu >= m_bufsize)) {
    ...
    if ((m_bufsize = fread(m_buffer + re, 1, m_total - re, m_fp)) <
        m_total - re) {
      m_eof = true;
    };
    m_bufsize += re;
