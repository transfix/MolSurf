...
*/
// ContourGeom - Class for a contour surface
...
int ContourGeom::AddVert(float x, float y, float z, float nx, float ny,
                         float nz, float cr, float cg, float cb) {
  int n = nvert++;
  if (nvert > vsize) {
    vsize <<= 1;
...
int ContourGeom::AddVertUnique(float x, float y, float z, float nx, float ny,
                               float nz, float cr, float cg, float cb) {
  VPosition vtx;
  vtx.idx = nvert;
  vtx.cord[0] = x;
...
int ContourGeom::AddVertUnique(float p[3], float n[3], float c[3],
                               EdgeIndex ei) {
  ei.idx = nvert;
...
int ContourGeom::AddTri(u_int v1, u_int v2, u_int v3) {
  int n = ntri++;
  if (ntri > tsize) {
    tsize <<= 1;
...
void ContourGeom::merge(ContourGeom *con) {
  int _ntri = ntri;
  int _nvert = nvert;
  ntri += con->ntri;
  nvert += con->nvert;
