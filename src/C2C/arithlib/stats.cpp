...
int install_symbol(context *pContext, int symbol) {
  ...
  while (symbol >= pContext->max_length) {
    pContext->tree = (unsigned long *)realloc(
        pContext->tree, pContext->max_length * GROWTH_RATE * sizeof(long));
    if (pContext->tree == NULL) {
      fprintf(stderr, "stats: not enough memory to expand context\n");
      return NO_MEMORY;
    }
    /* clear new part of table to zero */
    for (i = pContext->max_length; i < GROWTH_RATE * pContext->max_length;
         i++) {
      pContext->tree[i] = 0;
    }
    /*
...
int encode(context *pContext, int symbol) {
  ...
  if (low == high) {
    ...
    arithmetic_encode(pContext->total, pContext->total + pContext->nSingletons,
...
int decode(context *pContext) {
  ...
  if (target >= (int)pContext->total) {
    arithmetic_decode(pContext->total, pContext->total + pContext->nSingletons,
                      pContext->total + pContext->nSingletons);
    return NOT_KNOWN;
  }
  symbol = 0;
