...
// File "$Id: ByteStream.cpp,v 1.1.1.1 2008/06/06 19:42:10 wzhao Exp $"
...
int ByteStream::is_seekable() const { return 0; }

void ByteStream::seek(long offset, int whence) {
  int nwhere = 0;
  int ncurrent = tell();
  switch (whence) {
  case SEEK_SET:
  ...
  case SEEK_CUR:
    ...
    break;
  case SEEK_END:
    THROW("Seek mode SEEK_END is not supported by this ByteStream");
  default:
    THROW("Illegal argument in ByteStream::seek");
  }
  nwhere += offset;
  if (nwhere < ncurrent) {
...
size_t ByteStream::readall(void *buffer, size_t size) {
  size_t total = 0;
  while (size > 0) {
...
size_t ByteStream::writall(const void *buffer, size_t size) {
  size_t total = 0;
  while (size > 0) {
...
size_t ByteStream::copy(ByteStream &bsfrom, size_t size) {
  size_t total = 0;
  char buffer[1024];
  for (;;) {
...
void ByteStream::write8(unsigned int card) {
  unsigned char c[1];
  c[0] = (card) & 0xff;
...
void ByteStream::write16(unsigned int card) {
  unsigned char c[2];
  c[0] = (card >> 8) & 0xff;
  c[1] = (card) & 0xff;
...
void ByteStream::write32(unsigned int card) {
  unsigned char c[4];
  c[0] = (card >> 24) & 0xff;
  c[1] = (card >> 16) & 0xff;
  c[2] = (card >> 8) & 0xff;
...
StdioByteStream::StdioByteStream(FILE *f, const char *mode) {
  ...
  for (const char *s = mode; s && *s; s++)
    ...
    case 'b':
      break;
    default:
      THROW("Illegal mode in StdioByteStream");
    }
}
...
StdioByteStream::StdioByteStream(const char *filename, const char *mode) {
  ...
  if (strcmp(filename, "-") != 0) {
    ...
    if (!fp) {
      char buffer[4096];
      sprintf(buffer, "Cannot open '%s': %s", filename, strerror(errno));
      THROW(buffer);
    };
  } else {
    must_close = 0;
...
size_t StdioByteStream::read(void *buffer, size_t size) {
  if (!can_read) {
...
size_t StdioByteStream::write(const void *buffer, size_t size) {
  if (!can_write) {
    THROW("StdioByteStream not opened for writing");
  }
...
void StdioByteStream::flush() {
  if (fflush(fp) < 0) {
    THROW(strerror(errno));
  }
}

long StdioByteStream::tell() {
  long x = ftell(fp);
  if (x >= 0) {
    pos = x;
  }
...
int StdioByteStream::is_seekable() const { return can_seek; }

void StdioByteStream::seek(long offset, int whence) {
  if (!can_seek) {
...
size_t MemoryByteStream::write(const void *buffer, size_t sz) {
  int nsz = (int)sz;
  if (nsz <= 0) {
    return 0;
  }
...
size_t MemoryByteStream::readat(void *buffer, size_t sz, int pos) {
  if ((int)sz > bsize - pos) {
    sz = bsize - pos;
  }
  int nsz = (int)sz;
...
size_t MemoryByteStream::read(void *buffer, size_t sz) {
  sz = readat(buffer, sz, where);
  where += sz;
  return sz;
}

long MemoryByteStream::tell() { return where; }

int MemoryByteStream::is_seekable() const { return 1; }

void MemoryByteStream::seek(long offset, int whence) {
  int nwhere = 0;
  switch (whence) {
  case SEEK_SET:
    nwhere = 0;
  ...
  case SEEK_END:
    ...
    break;
  default:
    THROW("Illegal argument in MemoryByteStream::seek()");
  }
  nwhere += offset;
...
size_t StaticByteStream::read(void *buffer, size_t sz) {
...
size_t StaticByteStream::write(const void *buffer, size_t sz) {
  THROW("Attempt to write into a StaticByteStream");
  return 0;
}

int StaticByteStream::is_seekable(void) const { return 1; }

void StaticByteStream::seek(long offset, int whence) {
  int nwhere = 0;
  switch (whence) {
  case SEEK_SET:
    nwhere = 0;
  ...
  case SEEK_END:
    ...
    break;
  default:
    THROW("Illegal argument in StaticByteStream::seek()");
  }
  nwhere += offset;
  if (nwhere < 0 || nwhere > bsize) {
...
long StaticByteStream::tell() { return where; }
