...
GException::GException() : cause(0), file(0), func(0), line(0) {}

const char *const GException::outofmemory = "Out of memory";
...
void GException::perror(const char *msg) const {
  fflush(NULL);
...
void GExceptionHandler::emthrow(const GException &gex) {
  if (head) {
    head->current = gex;
    longjmp(head->jump, -1);
  } else {
