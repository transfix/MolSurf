...
ZPCodec::ZPCodec(ByteStream &bs, int encoding) : bs(&bs), encoding(!!encoding) {
...
inline int ZPCodec::ffz(unsigned int x) {
...
int ZPCodec::decode_sub(BitContext &ctx, unsigned int z) {
  /* Save bit */
  int bit = (ctx & 1);
...
int ZPCodec::decode_sub_simple(int mps, unsigned int z) {
  /* Test MPS/LPS */
  if (z > code) {
    /* LPS branch */
    z = 0x10000 - z;
...
int ZPCodec::decode_sub_nolearn(int mps, unsigned int z) {
#ifdef ZPCODER
  unsigned int d = 0x6000 + ((z + a) >> 2);
  if (z > d) {
    z = d;
...
void ZPCodec::emit(int b) {
   ...
   * similarity between this bit counting and the scheme of Witten, Neal &
   * Cleary (WN&C).  Corresponding changes have been made in outbit and eflush.
...
void ZPCodec::encode_mps(BitContext &ctx, unsigned int z) {
  /* Avoid interval reversion */
#ifdef ZPCODER
  unsigned int d = 0x6000 + ((z + a) >> 2);
  if (z > d) {
...
void ZPCodec::encode_lps(BitContext &ctx, unsigned int z) {
  /* Avoid interval reversion */
#ifdef ZPCODER
  unsigned int d = 0x6000 + ((z + a) >> 2);
  if (z > d) {
...
void ZPCodec::encode_mps_simple(unsigned int z) {
  /* Code MPS */
  a = z;
  /* Export bits */
  if (a >= 0x8000) {
...
void ZPCodec::encode_lps_simple(unsigned int z) {
  /* Code LPS */
  z = 0x10000 - z;
  subend += z;
  a += z;
...
void ZPCodec::encode_mps_nolearn(unsigned int z) {
#ifdef ZPCODER
  unsigned int d = 0x6000 + ((z + a) >> 2);
  if (z > d) {
    z = d;
...
void ZPCodec::encode_lps_nolearn(unsigned int z) {
#ifdef ZPCODER
  unsigned int d = 0x6000 + ((z + a) >> 2);
  if (z > d) {
    z = d;
...
static float p_to_plps(unsigned short p) {
  ...
  if (fp <= (1.0 / 6.0)) {
    ...
    fplps = (1.5 * fp - 0.25) - (1.5 * fp + 0.25) * log(1.5 * fp + 0.25) +
            (0.5 * fp - 0.25) * log2;
  }
#endif
  return fplps;
