#include <C2C/arithlib/coder.h>
#include <C2C/arithlib/stats.h>
#include <C2C/arithlib/utils.h>
#include <C2C/arithlib/bitbuffer.h>
#include <C2C/arithlib/libdjvu++/ZPCodec.h>
...
void BitBuffer::writeFile(FILE *ofp) {
  fwrite(&currbyte, sizeof(int), 1, ofp);
  fwrite(&currbit, sizeof(char), 1, ofp);
  int nbits = currbyte + ((currbit > 0) ? 1 : 0);
  if (nbits > 0) {
    fwrite(buf, 1, nbits, ofp);
  }
}

void BitBuffer::readFile(FILE *ifp) {
  put_reset(); // clean current bit buffer
  freadSafely(&currbyte, sizeof(int), 1, ifp);
  freadSafely(&currbit, sizeof(char), 1, ifp);
  size = currbyte + 10;
  buf = (u_char *)malloc(size);
  memset(buf, 0, size);
  int nbits = currbyte + ((currbit > 0) ? 1 : 0);
  if (nbits > 0) {
    freadSafely(buf, 1, nbits, ifp);
  }
}

int BitBuffer::getBuffer(unsigned char *ecd) {
  ...
  memcpy(ecd + sizeof(int) + sizeof(char), buf,
         nb - sizeof(int) - sizeof(char));
  return nb;
}

void BitBuffer::put_a_float(int res, float f) {
  if (res <= 4 || fabs(f) > 1.0) {
    printf("res = %d, f = %f\n", res, f);
...
void BitBuffer::Encode_Positive(int n) {
  assert(n >= 0);
  int bits1, bits2;
  bits1 = Comp_Bits(n);
...
void BitBuffer::put_a_bit(BIT b) {
  if (currbyte == size) {
    int prev_size = size;
    if (buf == NULL) {
...
******/
void BitBuffer::put_bits(int n, int x) {
  BIT b;
#ifdef _DEBUG
  assert(n < 32);
#endif
...
******/
void BitBuffer::put_bits(int n, BIT *bits) {
  /*    for (int i = 0; i < n; i++) {
          put_a_bit(bits[i]);
      }
          */
...
******/
void BitBuffer::put_sign_bits(int n, int x) {
  BIT sign = (x >= 0) ? 0 : 1;
  put_a_bit(sign);
  put_bits(n, abs(x));
}
...
******/
void BitBuffer::put_a_float(float x) {
  float ax = fabs(x);
  int power;
  float nx;
  if (x == 0) {
...
void BitBuffer::arith_decode() {
  ...
  contexts = (binary_context **)malloc(sizeof(binary_context *) * ncontext);
  if (contexts == NULL) {
    fprintf(stderr, "bits: unable to malloc %lu bytes\n",
...
void BitBuffer::put_a_int(u_int x) {
  put_bits(31, x);
  put_a_bit(0);
}

void BitBuffer::put_a_short(u_short x) { put_bits(16, x); }

void BitBuffer::put_a_char(u_char x) { put_bits(8, x); }

int BitBuffer::get_a_int() {
  int n = get_bits(31);
...
int BitBuffer::get_a_short() {
  int n = get_bits(16);
  return n;
}

int BitBuffer::get_a_char() {
...
float BitBuffer::get_a_float(int res) {
  float x = 0;
  BIT sign;
...
u_int BitBuffer::get_bits(int n) {
  // assert(n < 32 && n > 0);
  if (n <= 0 || n >= 32) {
