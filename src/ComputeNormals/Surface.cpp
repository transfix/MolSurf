...
void Surface::TriangleNormal(double *p1, double *p2, double *p3,
                             double *normal) {
...
void Surface::NormalByLoop() {
  ...
  int l, i, j, k, ii, jj, kk, iii, jjj, kkk, i0, j0, currentvalence, tou, wei,
      newwei, curnum, w;
  // first by averging
  NormalByAveraging();
  for (l = 0; l < numbpts; l++) {
    currentvalence = m_Vertices[l]->facevalence;
    if ((currentvalence > 30) || (currentvalence < 2) ||
        (m_Vertices[l]->facevalence !=
         m_Vertices[l]->edgevalence)) // begin if--1
    {
      printf("facevalence[%d]  = %d, edgevalence[%d] = %d\n", l,
             m_Vertices[l]->facevalence, l, m_Vertices[l]->edgevalence);
    } else {
      // initialization
