...
XYZRFile XYZRFile::ms_XYZRFileRepresentative;

XYZRFile::XYZRFile() {}

XYZRFile::~XYZRFile() {}

PDBParser::GroupOfAtoms *XYZRFile::loadFile(const string &fileName,
                                            bool deleteWater) {
...
bool XYZRFile::checkType(const string &fileName) { return false; }

bool XYZRFile::saveFile(PDBParser::GroupOfAtoms *molecule,
  ...
  for (int i = 0; i < n; i++) {
    fprintf(fp, "%lf %lf %lf %lf\n", m_AtomList[i]->m_Position[0],
            m_AtomList[i]->m_Position[1], m_AtomList[i]->m_Position[2],
...
GOAFileType *XYZRFile::getRepresentative() {
  return &ms_XYZRFileRepresentative;
}
