...
bool isCloseTo(double x1, double y1, double z1, double r, double x2, double y2,
               double z2, double *d) {
  double d1 =
...
void loadBornRadius(MOLECULE::GroupOfAtoms *molecule, FILE *fp) {
  ...
  for (int i = 0; i < molecule->numOfAtoms; i++) {
    if (fscanf(fp, "%lf\n", &br) != 1)
      error("Couldn't read Born radius!");
...
void usageArea() {
  cout << endl
       << "Usage: MolSurf -area <raw input file> <txt output file>" << endl
       << "    Compute the surface area of a triangular mesh." << endl;
}
...
Geometry *aSplineRawQuad(Geometry *geometry, int numOfPts, int resolution,
  ...
  ASpline aspline = ASpline(geometry);
  aspline.Triangle_Bezier_Patch_With_Quadrature(type.c_str(), numOfPts,
                                                resolution);
...
void usageASplineQuad() {
  cout << endl
       << "Usage: MolSurf -aSpline -quad <raw input file> <quadrature rule> "
       << endl
       << "          <points per triangle> <txt output file>" << endl
       << "    Generate quadrature points for an ASMS from a triangular mesh."
       << endl;
}
...
void usageASplineVolume() {
  cout << endl
       << "Usage: MolSurf -aspline -volume <raw input file> <txt output file>"
       << endl
       << "    Compute the volume bounded by an ASMS from a triangular mesh."
       << endl;
}
...
void usageBornRadius() {
  cout << endl
       << "Usage: MolSurf -bornRadius <radius type: vdw or pqr> <input pqr "
          "file> <surface geometry file> <output file>"
       << endl
       << "    Undocumented get born radius. Good luck." << endl;
}
...
bool bornRadius(int argc, char *argv[]) {
  ...
  GEOMETRY::Surface *surface =
      GEOMETRY::GeometryParser().SurfaceTriangulation(inputgeo);
  fclose(inputgeo);
...
void usageCoarseGrainCharge() {
  cout << endl
       << "Usage: MolSurf -coarseGrainCharge <radius type: vdw or pqr> <input "
          "file atomic model 1> < input file atomic model 2>"
       << endl
       << "    <input file cg model 1> <input file cg model 2> <input file cg "
          "model 3> <output file>"
       << endl
       << "    Undocumented coarse grain charge. Good luck." << endl;
}
...
bool coarseGrainCharge(int argc, char *argv[]) {
  ...
  // Input file cg model
  MOLECULE::GroupOfAtoms *groupOfBeads =
      ParsePQR(cgModel1, MOLECULE::PQR_RADIUS);
  fclose(cgModel1);
  ...
  cout << molecule->numOfAtoms << " atoms, " << molecule->numOfRes
       << " residues, " << groupOfBeads->numOfAtoms << " cg beads" << endl;
...
void usageCoarseGrain() {
  cout << endl
       << "Usage: MolSurf -coarseGrain <radius type: pqr or vdw> <PDB/PQR "
          "input> <pqr output> <cg info output>"
       //           << endl << "    Improve the mesh quality using geometric
       //           flow."
       << endl
       << "    If the input file name contains optimal, then we parse an "
          "optimal pqr."
       << endl;
}
...
bool coarseGrain(int argc, char *argv[]) {
  ...
  fclose(input);
  cout << molecule->numOfAtoms << " atoms, " << molecule->numOfRes << "residues"
       << endl;

  MOLECULE::GroupOfAtoms *groupOfBeads =
      MOLECULE::CoarseGrain().CoarseGrainMolecule(molecule);
...
void usageOptimizeCGCharge() {
  cout << endl
       << "Usage: MolSurf -optimizeCGCharge <size> <iteration> <ARM> "
          "<Hierachical Information> < ????? >"
       << endl
       << "    Undocumented optimize CG charge. Good luck." << endl;
}
...
void usageOptimizeCGGeometry() {
  cout << endl
       << "Usage: MolSurf -optimizeCGGeometry <size> <iterations> <pdb/pqr "
          "input file> "
       << endl
       << "          [[[[UNUSED<coarse-grained pdb/pqr input file>]]]] <txt "
          "hierarchy info input file> "
       << endl
       << "          <pbq/pqr output file>" << endl
       << "    Optimize coarse-grained model (radius and centers) based on "
          "hierarchical "
       << endl
       << "    information." << endl;
}
...
void usageConvertToRawnUsingHLS() {
  cout << endl
       << "Usage: MolSurf -convertToRawnUsingHLS <pqr/pdb input file> <rawn "
          "output file>  <outputfile2> <size> [nIterations [weight]] "
       << endl
       << "    Generate molecular surface with surface normal vectors using "
          "the higher "
       << endl
       << "    order level-set algorithm.    " << endl;
}
...
bool convertToRawnUsingHLS(int argc, char *argv[]) {
  ...
  SimpleVolumeData *sData = HLevelSet().getHigherOrderLevelSetSurface(
      molecule, dim, nIterations, weight);
...
void usageDecimatePercentage() {
  cout << endl
       << "Usage: MolSurf --decimatePercentage <raw input file> <raw output "
          "file>"
       << endl
       << "                       <percentage> <max variation> <min angle>"
       << endl
       << "     Decimate a mesh by vertex removal until fixed percentage "
          "remains."
       << endl;
}
...
bool decimatePercentage(int argc, char *argv[]) {
  ...
  Decimation().DecimatePercentage(input.c_str(), output.c_str(), percentage,
                                  -1.0, variation, innerangle);
  return true;
}
...
void usageDecimate() {
  cout << endl
       << "Usage: MolSurf --decimate <raw input file> <raw output file> "
          "<iterations> <max variation> <min angle>"
       << endl
       << "     Decimate a mesh by vertex removal." << endl;
}
...
bool decimate(int argc, char *argv[]) {
  ...
  Decimation().Decimate(input.c_str(), output.c_str(), niter, NULL, variation,
                        innerangle);
  return true;
}
...
void usageNormals() {
  cout << endl
       << "Usage: MolSurf -normals -[average|loop] <raw input file> <rawn "
          "output file>"
       << endl
       << "Or Usage: MolSurf -normals -invert <rawn input file> <rawn output "
          "file>"
       << endl
       << "    Compute surface normals at vertices of a triangular mesh using "
          "face normal "
       << endl
       << "    averaging or Loop's algorithm or invert the normal direction."
       << endl;
}
...
bool normals(int argc, char *argv[]) {
  ...
  if (strcmp(argv[2], "-invert") == 0) {
    ...
    // Function proper
    COMPUTENORMALS::Surface *mySurface =
        COMPUTENORMALS::GeometryParser().SurfaceTriangulation(input);
    fclose(input);
    ...
    for (int i = 0; i < mySurface->numbpts; i++) {
      if (printColors) {
        fprintf(output, "%f %f %f %f %f %f %f %f %f\n",
                mySurface->m_Vertices[i]->Point[0],
                mySurface->m_Vertices[i]->Point[1],
                mySurface->m_Vertices[i]->Point[2],
                mySurface->m_Vertices[i]->Normal[0],
                mySurface->m_Vertices[i]->Normal[1],
                mySurface->m_Vertices[i]->Normal[2],
                mySurface->m_Vertices[i]->Color[0],
                mySurface->m_Vertices[i]->Color[1],
                mySurface->m_Vertices[i]->Color[2]);
      } else {
    ...
    for (int i = 0; i < mySurface->numbtris; i++) {
      fprintf(output, "%d %d %d\n", mySurface->m_Faces[i]->IndexInRAW[0],
              mySurface->m_Faces[i]->IndexInRAW[1],
              mySurface->m_Faces[i]->IndexInRAW[2]);
    }
    fclose(output);
  }
}
...
void usageReorientate() {
  cout << endl
       << "Usage: MolSurf -reoriente <input geom file> <output geom file>"
       << endl
       << "    change the orientation of the surface" << endl;
}
...
void usagePocketTunnel() {
  cout << endl
       << "Usage: MolSurf -pocketTunnel <input file> <opc> <otc> <output file> "
          "<output file 2>"
       << endl
       << "    Undocumented call pocket tunnel. Good luck." << endl;
}
...
void usagePocket() {
  cout << endl
       << "Usage: MolSurf -pocket <pdbFileName> <distance> <size> "
          "<outputFileName>"
       << endl
       << "    Undocumented call pocket. Good luck." << endl;
}
...
bool pocket(int argc, char *argv[]) {
  ...
  Geometry *geometry;
  Pocket().constructPockets(molecule, output.c_str(), distance, &geometry,
                            size);
  return true;
}
...
void usagePopulateSAS() {
  cout
      << endl
      << "Usage: MolSurf -populateSAS <pdb/pqr input file> <output file> <size>"
      << endl
      << "          <probe radius> <gap between surface and skin> <radius type>"
      << endl;
}
...
bool populateSAS(int argc, char *argv[]) {
  ...
  return BlurMapsDataManager::populateSAS(input.c_str(), output.c_str(), size,
                                          size, size, probeRadius,
                                          floatingBandRadius, radiusType);
}

// Populate SAS using mesh
void usagePopulateSASUsingMesh() {
  cout << endl
       << "Usage: MolSurf -populateSASUsingMesh <pqr input file> <rawn input "
          "file> <xyz output file>"
       << endl
       << "          <probe radius> <gap between surface and skin> <minimum "
          "gap between centers of skin atoms> <radius type>"
       << endl;
}

/*
gap between surface and skin = see the definition of floating skin layer in the
F2Dock paper minimum gap between centers of skin atoms = used to control the
density of the skin atoms sampled. This is useful to keep the number of skin
atoms almost constant, no matter how dense the triangulation is. radius type =
see PDBparser::GroupOfAtoms for details
*/
...
bool populateSASUsingMesh(int argc, char *argv[]) {
  ...
  return BlurMapsDataManager::populateSASUsingMesh(
      pqr, surface, output, probeRadius, floatingBandRadius, clashDistance,
      radiusType);
}
...
void usageGenerateF2d() {
  cout << endl
       << "Usage: MolSurf -generateF2d <pqr input file> <xyz file> <output "
          "file> <int receptor=1/ligand=0> <radius type> <skin atom radius>"
       << endl;
}
...
bool generateF2d(int argc, char *argv[]) {
  ...
  return BlurMapsDataManager::generateF2d(pqr, xyz, output, receptor,
                                          radiusType, skinAtomRad);
}

// Generating rmsd file for docking
void usageGetInterfaceAtoms() {
  cout << endl
       << "Usage: MolSurf -getInterfaceAtoms <receptor pqr file> <ligand pqr "
          "file> <txt output file>"
       << endl
       << "          <interface width> <string atomType = {all / backbone / "
          "any combination of {C,CA,N,O} (comma separated) }>"
       << endl;
}

/*
InterfaceWidth: if an atom of the ligand is within interfaceWidth of any atom of
the receptor, then the ligand atom is on the interface

atomType: "all" = all interface atoms are printed in the rmsd file
          "backbone" = Only C, CA and N on the interface are printed
          comma separated list = Only interface atoms whose type is in the list
is printed
*/
...
bool getInterfaceAtoms(int argc, char *argv[]) {
  ...
  return BlurMapsDataManager::getInterfaceAtoms(
      pqr1, pqr2, output, interfaceWidth, atomTypes, radiusType);
}

// Generating rmsd file for docking
void usageGetInterfaceSurface() {
  cout << endl
       << "Usage: MolSurf -getInterfaceSurface <receptor rawn/rawnc file> "
          "<ligand rawn/rawnc file> <interface width> <int coloredSurface (0 = "
          "rawn, 1 = rawnc)>"
       << endl;
}
...
bool getInterfaceSurface(int argc, char *argv[]) {
  ...
  return BlurMapsDataManager::getInterfaceSurface(surf1, surf2, interfaceWidth,
                                                  isColoredBool, area1, area2);
...
void usageColorByCharge() {
  cout << endl
       << "Usage: MolSurf -colorByCharge <pqr file> <surface(RAWN) file> "
          "<output surface file(RAWNC)>"
       << endl
       << "          <mode = {0 = Gaussian, 1 = average}> <distance cutoff> "
          "<blobbyness (optional)>"
       << endl;
}
...
void usageQualityImprove() {
  cout << endl
       << "Usage: MolSurf -qualityImprove <raw input file> <raw output file> "
          "[parameter]"
       << endl
       << "    Improve the mesh quality using geometric flow." << endl;
}
...
void usageRotatePDB() {
  cout << endl
       << "Usage: MolSurf -rotatePDB <pdb input file> <transoformation file> "
          "<pdb output file>"
       << endl
       << "    Rotate a molecule." << endl;
}
...
bool rotatePDB(int argc, char *argv[]) {
  ...
  CCVOpenGLMath::Matrix transformation(
      ...
      trans[7], trans[8], trans[9], trans[10], trans[11], trans[12], trans[13],
      trans[14], trans[5]);
...
void usageSignedDistanceFunction() {
  cout << endl
       << "Usage: MolSurf -signedDistanceFunction <raw input file> <rawiv "
          "output file> <size>"
       << endl
       << "    Signed distance function using multi_sdf." << endl;
}
...
void usageSignedDistanceFunction2() {
  cout
      << endl
      << "Usage: MolSurf -signedDistanceFunction2 <raw input file> <rawiv "
         "output file> <size x> <size y> <size z> "
      << endl
      << "                <bbox x> <bbox y> <bbox z> <span x> <span y> <span z>"
      << endl
      << "    Signed distance function using multi_sdf." << endl;
}
...
void usageSurfaceAtoms() {
  cout << endl
       << "Usage: MolSurf -surfaceAtoms <pdb input file> <output file>" << endl
       << "    Compute which atoms of a molecule touch the molecular surface."
       << endl;
}
...
bool surfaceAtoms(int argc, char *argv[]) {
  ...
  PDBParser::GroupOfAtoms::RADIUS_TYPE radiusType =
      PDBParser::GroupOfAtoms::VDW_RADIUS;
  SimpleVolumeData *sData = 0;
  sData = BlurMapsDataManager::getVolume(
      molecule, "", 128, 128, 128, PDBParser::GroupOfAtoms::ELECTRON_DENSITY,
      false, -1.0, PDBParser::GroupOfAtoms::ATOM, 0, 0, radiusType,
      PDBParser::ATOM_TYPE);
  if (!sData) {
    return false;
  }
  Geometry *largestComponent =
      SimpleVolumeDataIsocontourer::getIsocontour(sData, 1.0);
  ...
  for (int i = 0; i < atomList.size(); i++) {
    ...
    for (int v = 0; v < largestComponent->m_NumTriVerts; v++) {
      ...
      double dist = 100;
      if (isCloseTo(atomList[i]->m_Position[0], atomList[i]->m_Position[1],
                    atomList[i]->m_Position[2], atomList[i]->getRadius() + 1.0,
                    largestComponent->m_TriVerts[v * 3 + 0],
                    largestComponent->m_TriVerts[v * 3 + 1],
                    largestComponent->m_TriVerts[v * 3 + 2], &dist))
  ...
  for (int i = 0; i < atomList.size(); i++) {
    if (boundaryAtom[i]) {
      ...
      fprintf(fpIds, "%d\n", i + 1);
      fprintf(fpSkin,
              "ATOM  11111  CA  GLU A1111    %8.3f%8.3f%8.3f%6.2f              "
              "  \n",
              atomList[i]->m_Position[0], atomList[i]->m_Position[1],
              atomList[i]->m_Position[2], atomList[i]->getRadius());
      numBdyAtoms++;
    } else {
      fprintf(fp, "0.0\n");
      fprintf(fpOut,
              "ATOM  11111  N   GLU A1111    %8.3f%8.3f%8.3f%6.2f              "
              "  \n",
              atomList[i]->m_Position[0], atomList[i]->m_Position[1],
              atomList[i]->m_Position[2], atomList[i]->getRadius());
    }
  ...
  for (int i = 0; i < atomList.size(); i++) {
    ...
    fprintf(fp, "I     %12.5lf %12.5lf %12.5lf %12.5f\n", at->m_Position[0],
            at->m_Position[1], at->m_Position[2], at->getCharge());
  }
  for (int i = 0; i < atomList.size(); i++) {
    if (boundaryAtom[i]) {
      PDBParser::Atom *at = atomList[i];
...
void usageSurfaceFromPDB() {
  cout << endl
       << "Usage: MolSurf -surfaceFromPDB <pdb file> <surface file> <isovalue> "
          "<size>"
       << endl
       << "    <blur?> <blobbiness> <radius type> <level> <optimize radii?>"
       << endl
       << "    Undocumented get surface from PDB. Good luck." << endl;
}
...
bool surfaceFromPDB(int argc, char *argv[]) {
  ...
  if (blurTheVolume) {
    ...
    SimpleVolumeData *sData = 0;
    if (optimizeRadii) {
      sData = BlurMapsDataManager::getMultiLevelBlurredVolume(
          molecule, "", size, size, size,
          PDBParser::GroupOfAtoms::ELECTRON_DENSITY, false, blobbiness,
          PDBParser::GroupOfAtoms::ATOM, 0, 0, radiusType, level);
    } else {
      sData = BlurMapsDataManager::getVolume(
          molecule, "", size, size, size,
          PDBParser::GroupOfAtoms::ELECTRON_DENSITY, false, blobbiness,
...
Geometry *surfaceUsingAdaptiveGrid(PDBParser::GroupOfAtoms *molecule,
                                   int size) {
...
Geometry *surfaceUsingAdaptiveGrid(PDBParser::GroupOfAtoms *molecule, int size,
  ...
  // PDBParser::GroupOfAtoms::RADIUS_TYPE radiusType;
  Geometry *geometry =
      MolecularSurface::getMolecularSurface(molecule, size, extraRadius);
  return geometry;
}
...
Geometry *surfaceUsingHLS(PDBParser::GroupOfAtoms *molecule, int size,
  ...
  SimpleVolumeData *sData =
      hls.getHigherOrderLevelSetSurface(molecule, dim, iterations, weight);
  Geometry *geometry = 0;
  printf("Isovalue: %f \n", IsoValueofHLS);
  geometry =
      SimpleVolumeDataIsocontourer::getIsocontour(sData, (IsoValueofHLS + 0.6));
...
Geometry *surfaceUsingGaussianBlur(PDBParser::GroupOfAtoms *molecule,
  ...
  // create the isosurface
  Geometry *geometry =
      SimpleVolumeDataIsocontourer::getIsocontour(sData, isovalue);
  delete sData;
...
Geometry *surfaceUsingGaussianBlur2(PDBParser::GroupOfAtoms *molecule,
  ...
  sData = BlurMapsDataManager::getVolumeFixedResolution(
      molecule, "", resolution, PDBParser::GroupOfAtoms::ELECTRON_DENSITY,
...
void usageSurfaceUsingLBIE() {
  cout << endl
       << "Usage: MolSurf -surfaceUsingLBIE <meshtype> <isovalue> <rawiv input "
          "file> <raw output file>"
       << endl
       << "    Generate tri/quad/tet/hex mesh from volumetric data using the "
          "LBIE library."
       << endl;
}
...
bool surfaceUsingLBIE(int argc, char *argv[]) {
  ...
  if (strstr(argv[2], "tri"))
    meshType = TRI;
  if (strstr(argv[2], "tet"))
    meshType = TET;
  if (strstr(argv[2], "tri2"))
    meshType = TRI2;
  if (strstr(argv[2], "tet2"))
    meshType = TET2;
  if (strstr(argv[2], "quad"))
    meshType = QUAD;
  if (strstr(argv[2], "hex"))
    meshType = HEX;
  if (strstr(argv[2], "nurbs"))
    meshType = NURBS;
...
void usageVolume() {
  cout << endl
       << "Usage: MolSurf -volume <raw input file> <txt output file>" << endl
       << "    Compute the volume bounded by a triangular surface mesh."
       << endl;
}
...
void usageVolumeUsingHLS() {
  cout << endl
       << "Usage: MolSurf -volumeUsingHLS <pdb/pqr input file> <rawiv output "
          "file> <size> [nIterations weight]"
       << endl
       << "    Generate volumetric level-set function." << endl;
}
...
void usageGetVolumeWithHydrophobicity() {
  cout << endl
       << "Usage: MolSurf -getVolumeWithHydrophobicity <pdb/pqr input file> "
          "<rawiv output file> <dim1> <dim2> <dim3> <blobbiness> [<bbxmin> "
          "<bbymin> <bbzmin> <bbxmax> <bbymax> <bbzmax> ]"
       << endl;
}
...
bool getVolumeWithHydrophobicity(int argc, char *argv[]) {
  ...
  if (argc != 14)
    sData = BlurMapsDataManager::getVolume(
        molecule, output.c_str(), dim1, dim2, dim3,
        PDBParser::GroupOfAtoms::PER_ATOM_HYDROPHOBICITY, false, blobby,
        PDBParser::GroupOfAtoms::ATOM, NULL, 0, radiusType,
        PDBParser::ATOM_TYPE);
  else
    sData = BlurMapsDataManager::getVolume(
        molecule, output.c_str(), dim1, dim2, dim3, bbmin, bbmax,
        PDBParser::GroupOfAtoms::PER_ATOM_HYDROPHOBICITY, false, blobby,
        PDBParser::GroupOfAtoms::ATOM, NULL, 0, radiusType,
        PDBParser::ATOM_TYPE);
  if (sData == 0) {
...
void usageGetVolumeWithElectron() {
  cout << endl
       << "Usage: MolSurf -getVolumeWithElectron <pdb/pqr input file> <rawiv "
          "output file> <dim1> <dim2> <dim3> <blobbiness>"
       << endl;
}
...
bool getVolumeWithElectron(int argc, char *argv[]) {
  ...
  sData = BlurMapsDataManager::getVolume(
      molecule, output.c_str(), dim1, dim2, dim3,
      PDBParser::GroupOfAtoms::ELECTRON_DENSITY, false, blobby,
      PDBParser::GroupOfAtoms::ATOM, NULL, 0, radiusType, PDBParser::ATOM_TYPE);
...
void usageGetVolumeWithChargeDensity() {
  cout << endl
       << "Usage: MolSurf -getVolumeWithChargeDensity <pqr input file> <rawiv "
          "output file> <dim1> <dim2> <dim3> <blobbiness> [<bbxmin> <bbymin> "
          "<bbzmin> <bbxmax> <bbymax> <bbzmax> ]"
       << endl;
}
...
bool getVolumeWithChargeDensity(int argc, char *argv[]) {
  ...
  if (argc != 14)
    sData = BlurMapsDataManager::getVolume(
        molecule, output.c_str(), dim1, dim2, dim3,
        PDBParser::GroupOfAtoms::ELECTROSTATIC_POTENTIAL, false, blobby,
        PDBParser::GroupOfAtoms::ATOM, NULL, 0, radiusType,
        PDBParser::ATOM_TYPE);
  else
    sData = BlurMapsDataManager::getVolume(
