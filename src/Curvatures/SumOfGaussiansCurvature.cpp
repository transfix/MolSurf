...
SumOfGaussiansCurvature::SumOfGaussiansCurvature(
    int numberOfGaussians, double *gaussianCenters, int numberOfGridDivisions,
    double maxFunctionError, double blobbiness, int numberOfPoints,
    float *points, double *HandK, double *normals, double *k1Vector,
    double *k2Vector)
...
void SumOfGaussiansCurvature::getIndices(double x, double y, double z,
...
void SumOfGaussiansCurvature::createGrid() {
  ...
  getMinMax();
  m_CurvaturesGridVoxels = new CurvaturesGridVoxel[m_NumberOfGridDivisions *
                                                   m_NumberOfGridDivisions *
                                                   m_NumberOfGridDivisions];
  double maxAtomRadius = 4.0;
  double overlapExtent =
      (log(m_MaxFunctionError) + m_Blobbiness) * maxAtomRadius / m_Blobbiness;
...
inline void SumOfGaussiansCurvature::evalCurvatureDueToOneAtom(
...
inline void SumOfGaussiansCurvature::evalCurvature(double *phiX, double *phiY,
                                                   double *phiZ, double *phiXX,
                                                   double *phiYY, double *phiZZ,
                                                   double *phiXY, double *phiXZ,
                                                   double *phiYZ, double x,
                                                   double y, double z) {
  ...
  for (k = 0; k < numAtoms; k++) {
    ...
    double az = a[2];
    double rad = a[3];
    evalCurvatureDueToOneAtom(phiX, phiY, phiZ, phiXX, phiYY, phiZZ, phiXY,
                              phiXZ, phiYZ, ax, ay, az, rad, x, y, z);
  }
}
