...
Curvature::Curvature(int numberOfPoints, float *points, double *HandK,
                     double *normals, double *k1Vector, double *k2Vector) {
...
bool Curvature::getCurvatures() {
  ...
  for (i = 0; i < m_NumberOfPoints; i++) {
    ...
    double tK1Vector[3];
    double tK2Vector[3];
    tK1Vector[0] = tK1Vector[1] = tK1Vector[2] = tK2Vector[0] = tK2Vector[1] =
        tK2Vector[2] = 0;
    ...
    if (fabs(k1 - k2) > 1e-10) {
      ...
      double g3 = g * g * g;
      double g2 = g * g;
      c11 = (phiXX * g2 - phiX * (phiX * phiXX + phiY * phiXY + phiZ * phiXZ)) /
            g3;
      c12 = (phiXY * g2 - phiX * (phiX * phiXY + phiY * phiYY + phiZ * phiYZ)) /
            g3;
      c13 = (phiXZ * g2 - phiX * (phiX * phiXZ + phiY * phiYZ + phiZ * phiZZ)) /
            g3;
      c21 = (phiXY * g2 - phiY * (phiX * phiXX + phiY * phiXY + phiZ * phiXZ)) /
            g3;
      c22 = (phiYY * g2 - phiY * (phiX * phiXY + phiY * phiYY + phiZ * phiYZ)) /
            g3;
      c23 = (phiYZ * g2 - phiY * (phiX * phiXZ + phiY * phiYZ + phiZ * phiZZ)) /
            g3;
      c31 = (phiXZ * g2 - phiZ * (phiX * phiXX + phiY * phiXY + phiZ * phiXZ)) /
            g3;
      c32 = (phiYZ * g2 - phiZ * (phiX * phiXY + phiY * phiYY + phiZ * phiYZ)) /
            g3;
      c33 = (phiZZ * g2 - phiZ * (phiX * phiXZ + phiY * phiYZ + phiZ * phiZZ)) /
            g3;
      if (CCVOpenGLMath::LinearAlgebra::solveEigenSystem(
              c11, c12, c13, c21, c22, c23, c31, c32, c33, tK1Vector, tK2Vector,
              k1, k2)) {
        double d;
        d = sqrt(tK1Vector[0] * tK1Vector[0] + tK1Vector[1] * tK1Vector[1] +
                 tK1Vector[2] * tK1Vector[2]);
        tK1Vector[0] /= d;
        tK1Vector[1] /= d;
        tK1Vector[2] /= d;
        d = sqrt(tK2Vector[0] * tK2Vector[0] + tK2Vector[1] * tK2Vector[1] +
                 tK2Vector[2] * tK2Vector[2]);
        tK2Vector[0] /= d;
        tK2Vector[1] /= d;
        tK2Vector[2] /= d;
      ...
      {
        double e1 = (c11 - k1) * m_K1Vector[i * 3 + 0] +
                    c12 * m_K1Vector[i * 3 + 1] + c13 * m_K1Vector[i * 3 + 2];
        double e2 = c21 * m_K1Vector[i * 3 + 0] +
                    (c22 - k1) * m_K1Vector[i * 3 + 1] +
                    c23 * m_K1Vector[i * 3 + 2];
        double e3 = c31 * m_K1Vector[i * 3 + 0] + c32 * m_K1Vector[i * 3 + 1] +
                    (c33 - k1) * m_K1Vector[i * 3 + 2];
      }
    }
    if (((i + 1) % 20) == 0 || (i + 1) == m_NumberOfPoints) {
      printf("Evaluating functions %2.2f%% done (%012d)\r",
             100.0 * (i + 1) / (float)(m_NumberOfPoints), i + 1);
    }
  }
  printf("\n");
  return true;
...
bool Curvature::write(const char *filename) {
  ...
  if (!m_HandK || !m_Normals || !m_K1Vector || !m_K2Vector ||
      (m_NumberOfPoints < 1)) {
    return false;
  }
  ...
  fprintf(fp, "# Then, search for one of the following: HandK, Normal, "
              "K1Vector, K2Vector\n");
  fprintf(fp, "# Any of the above should be followed by the corresponding "
              "values with 2, 3, 3, 3 in each line\n");
  fprintf(fp, "#\n");
  ...
  fprintf(fp, "Normal\n");
  for (i = 0; i < m_NumberOfPoints; i++) {
    fprintf(fp, "%lf %lf %lf\n", m_Normals[i * 3 + 0], m_Normals[i * 3 + 1],
            m_Normals[i * 3 + 2]);
  }
  fprintf(fp, "K1Vector\n");
  for (i = 0; i < m_NumberOfPoints; i++) {
    fprintf(fp, "%lf %lf %lf\n", m_K1Vector[i * 3 + 0], m_K1Vector[i * 3 + 1],
            m_K1Vector[i * 3 + 2]);
  }
  fprintf(fp, "K2Vector\n");
  for (i = 0; i < m_NumberOfPoints; i++) {
    fprintf(fp, "%lf %lf %lf\n", m_K2Vector[i * 3 + 0], m_K2Vector[i * 3 + 1],
            m_K2Vector[i * 3 + 2]);
  }
  fclose(fp);
  return true;
...
bool Curvature::read(const char *filename, int *numberOfPoints, double **HandK,
                     double **normals, double **k1Vector, double **k2Vector) {
  if (!filename || !numberOfPoints || !HandK || !normals || !k1Vector ||
      !k2Vector) {
    return false;
  }
