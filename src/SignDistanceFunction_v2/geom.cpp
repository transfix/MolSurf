...
static bool edgeIntersectSquare(double y0, double z0, double y1, double z1,
  ...
  if (!isZero(y1 - y0)) {
    t = (ymin - y0) / (y1 - y0);
    double z = z0 + t * (z1 - z0);
    if ((isBetween(0.0, 1.0, t)) && (isBetween(zmin, zmax, z))) {
      return true;
    }
    t = (ymax - y0) / (y1 - y0);
    z = z0 + t * (z1 - z0);
  ...
  if (!isZero(z1 - z0)) {
    t = (zmin - z0) / (z1 - z0);
    double y = y0 + t * (y1 - y0);
    if ((isBetween(0.0, 1.0, t)) && (isBetween(ymin, ymax, y))) {
      return true;
    }
    t = (ymax - y0) / (y1 - y0);
    y = y0 + t * (y1 - y0);
...
static bool edgeIntersectFace(double x0, double y0, double z0, double x1,
                              double y1, double z1, double xmin, double ymin,
                              double ymax, double zmin, double zmax) {
  double t;
  if (isEqual(x0, xmin) &&
      isEqual(x1, xmin)) // edge (p0, p1) is in the x = xmin plane
  {
...
static bool edgeIntersectCube(const Point3f &p0, const Point3f &p1,
                              const Point3f &lower, const Point3f &upper) {
  ...
  zmax = upper[2];
  // 1) face 1 x = xmin...
  if (edgeIntersectFace(p0[0], p0[1], p0[2], p1[0], p1[1], p1[2], xmin, ymin,
                        ymax, zmin, zmax)) {
    return true;
  }
  // 1) b> face x = xmax...
  if (edgeIntersectFace(p0[0], p0[1], p0[2], p1[0], p1[1], p1[2], xmax, ymin,
                        ymax, zmin, zmax)) {
    return true;
  }
  // 1) face y = ymin...
  if (edgeIntersectFace(p0[1], p0[0], p0[2], p1[1], p1[0], p1[2], ymin, xmin,
                        xmax, zmin, zmax)) {
    return true;
  }
  // 1) face y = ymax ...
  if (edgeIntersectFace(p0[1], p0[0], p0[2], p1[1], p1[0], p1[2], ymax, xmin,
                        xmax, zmin, zmax)) {
    return true;
  }
  // 1) face z = zmin...
  if (edgeIntersectFace(p0[2], p0[0], p0[1], p1[2], p1[0], p1[1], zmin, xmin,
                        xmax, ymin, ymax)) {
    return true;
  }
  // 1) face z = zmax...
  if (edgeIntersectFace(p0[2], p0[0], p0[1], p1[2], p1[0], p1[1], zmax, xmin,
                        xmax, ymin, ymax)) {
    return true;
  }
  return false;
...
bool DistanceTransform::TriangleCubeIntersection(int nt, const Point3f &lower,
                                                 const Point3f &upper) {
