...
void DistanceTransform::init() {
  ...
  p_Data->getSpan(span);
  p_Data->getDim(dim);
  p_DistanceTransformCells = new DistanceTransformCell[p_Data->getNCells()];
  ...
  for (int nt = 0; nt < p_Surf->triCount(); nt++) {
    ...
    for (j = 0; j < 3; j++) {
      if (v1[j] < lower[j]) {
        lower[j] = v1[j];
      } else if (v1[j] > upper[j]) {
        upper[j] = v1[j];
      }
      if (v2[j] < lower[j]) {
        lower[j] = v2[j];
      } else if (v2[j] > upper[j]) {
        upper[j] = v2[j];
      }
    }
    int minId[3], maxId[3];
    ...
    for (k = minId[2]; k <= maxId[2]; k++) {
      for (j = minId[1]; j <= maxId[1]; j++) {
        for (i = minId[0]; i <= maxId[0]; i++) {
          ...
          // cout << nc << " " << p_Data->getNCells() << endl;
          assert(nc < p_Data->getNCells() && nc >= 0);
          Vector3f norm;
          p_Surf->getTriNormal(nt, norm);
          if (intersectDistanceTransformCell(v0, norm, i, j, k)) {
            Point3f lower, upper;
            ...
            if (TriangleCubeIntersection(nt, lower, upper)) {
              p_DistanceTransformCells[nc].triList.insert(nt);
            }
          }
        }
      }
...
DistanceTransform::DistanceTransform(FaceVertSet3D &fvs, int dim[3], float dist,
  ...
  p_Surf = &fvs;
  BoundingBox2 bbox = p_Surf->getExtent();
...
DistanceTransform::DistanceTransform(FaceVertSet3D &fvs, int dim[3],
                                     float bbox[3], float spacing[3]) {
  // arand: I could have forgotten something...

  p_Surf = &fvs;
  p_Data = new Reg3Data<float>(dim, NULL);
  p_Data->setOrig(bbox);
  p_Data->setSpan(spacing);
  init();
}

DistanceTransform::DistanceTransform(FaceVertSet3D &fvs,
                                     const Reg3Data<float> &reg3) {
...
DistanceTransform::~DistanceTransform(void) {
  ...
  if (p_DistanceTransformCells) {
    delete[] p_DistanceTransformCells;
  }
}
  ...
  for (k = 0; k < dim[2]; k++) {
    for (j = 0; j < dim[1]; j++) {
      for (i = 0; i < dim[0]; i++) {
        int nv = p_Data->index2vert(i, j, k);
        if (parent[nv] != nv) {
          Point3f p(orig[0] + i * span[0], orig[1] + j * span[1],
                    orig[2] + k * span[2]);
...
float DistanceTransform::computeNearDistance(int ix, int iy, int iz,
                                             Point3f &nearPnt) {
  std::set<int> triSet;
  int dim[3];
  p_Data->getDim(dim);
  for (int kk = -1; kk <= 0; kk++) {
    for (int jj = -1; jj <= 0; jj++) {
      for (int ii = -1; ii <= 0; ii++) {
        ...
        cj = iy + jj;
        ck = iz + kk;
        if (ci < 0 || ci >= dim[0] - 1 || cj < 0 || cj >= dim[1] - 1 ||
            ck < 0 || ck >= dim[2] - 1) {
          continue;
        }
        int nc = p_Data->index2cell(ci, cj, ck);
        for (int i = 0; i < p_DistanceTransformCells[nc].triList.length();
             i++) {
          triSet.insert(p_DistanceTransformCells[nc].triList[i]);
        }
      }
    }
  }
  ...
  while (it != triSet.end()) {
    ...
    if (d <= dis - TOLERANCE) // new closest intersection
    {
      ...
      if (pointDistance(nearPnt, myNear) <
          TOLERANCE) // same intersection for another triangle
      {
        nearTri[nclose++] = tid;
      }
    }
...
int DistanceTransform::nearestPlane(const dynamic_array<int> &triList,
                                    const Point3f &vert) {
...
int DistanceTransform::inOrOut(const dynamic_array<int> &triList,
                               const Point3f &vert, const Point3f &nearPnt) {
  int i, j, len = triList.length();
  assert(len > 1);
  Point3f center, cpnt;
  Point3f v0, v1, v2;
...
static double signedDist2Plane(const Point3f &pnt, const Point3f &v0,
                               const Vector3f &norm) {
  Vector3f vdiff;
  vdiff[0] = pnt[0] - v0[0];
  vdiff[1] = pnt[1] - v0[1];
...
double DistanceTransform::rayTriangleIntersection(int nt, const Point3f &begin,
                                                  const Point3f &end) {
  const double TOLERANCE = 1.0e-6;
  Vector3f norm;
  Point3f v0, v1, v2;
  ...
  if (fabs(dbeg) < TOLERANCE) {
    if (pointInTriangle(begin, v0, v1, v2, norm)) {
      return 0;
    }
  }
  if (fabs(dend) < TOLERANCE) {
    if (pointInTriangle(end, v0, v1, v2, norm)) {
      return 1;
    }
  }
  if (dbeg * dend > 0) {
  ...
  if (fabs(dbeg) < TOLERANCE && fabs(dend) < TOLERANCE) {
    return 0.5; // a hack that needs to fixed later
  }
  ...
  if (pointInTriangle(intersect, v0, v1, v2, norm)) {
    return t;
  }
  return MAX_FLOAT;
}

double DistanceTransform::distance2Triangle(const Point3f &pnt, int nt,
                                            Point3f &nearPnt) {
  double dist, temp[3];
  Point3f v0, v1, v2;
  p_Surf->getTriVerts(nt, v0, v1, v2);
  ...
  Point3f edgePnt;
  // 1) First compute the shortest distance from the pnt to the plane of the
  // Triangle.
  vdiff[0] = pnt[0] - v0[0];
  vdiff[1] = pnt[1] - v0[1];
  vdiff[2] = pnt[2] - v0[2];
...
double DistanceTransform::distance2Edge(const Point3f &pnt, const Point3f &v1,
                                        const Point3f &v2, Point3f &ne) {
...
bool DistanceTransform::pointInTriangle(const Point3f &res,
                                        const Point3f &vert0,
                                        const Point3f &vert1,
                                        const Point3f &vert2,
                                        const Vector3f &norm) {
...
bool DistanceTransform::intersectDistanceTransformCell(const Point3f &v0,
                                                       const Vector3f &norm,
                                                       int i, int j, int k) {
  ...
  for (int kk = 0; kk <= 1; kk++) {
    for (int jj = 0; jj <= 1; jj++) {
      for (int ii = 0; ii <= 1; ii++) {
        ...
        if (dot * dot2 <=
            0) // two vertices on the opposite sides of the triangle plane
        {
          return true;
        }
      }
  ...
  for (int kk = -1; kk <= 0; kk++) {
    for (int jj = -1; jj <= 0; jj++) {
      for (int ii = -1; ii <= 0; ii++) {
        ...
        int nc = p_Data->index2cell(ci, cj, ck);
        if (p_DistanceTransformCells[nc].triList.length() > 0) {
          return true;
        }
      }
    }
...
void DistanceTransform::transform1D(int n, float f[], float d[], int parent[],
                                    float span) {
  ...
  if (min >= MAX_FLOAT) {
    return; // every number is max, no need to transform.
  }
  for (i = 0; i < n; i++) {
    if (f[i] < MAX_FLOAT) {
      f[i] = f[i] / s2;
