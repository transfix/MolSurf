...
using CCVContouring::RGBAExtractor;

RGBAExtractor::RGBAExtractor() { setDefaults(); }

RGBAExtractor::~RGBAExtractor() { setDefaults(); }

void RGBAExtractor::extractContour(ContourGeometry *contourGeometry,
                                   float isovalue, float R, float G,
                                   float B) const {
  ...
  // the new vertex
  unsigned int v1xuint, v1yuint, v1zuint, v2xuint, v2yuint, v2zuint,
      vertOffset1, vertOffset2;
  ...
  for (k = 0; k < numCellsZ; k++) // for every slice
  {
    ...
    for (j = 0; j < numCellsY; j++) // for every row
    {
      ...
      for (i = 0; i < numCellsX; i++) // for every cell
      {
        ...
        cubeCase = determineCase(m_Data, withinSliceOffsetTable, offsetInSlice,
                                 isovalue);
        if (cubeCase != 0 && cubeCase != 255) {
          ...
          for (edgeCounter = 0; edgeCounter < cubeedges[cubeCase][0];
            ...
            if (!cacheAvailable[edgeID]) {
              ...
              vertOffset1 =
                  v1xuint + v1yuint * m_Width + v1zuint * m_Width * m_Height;
              vertOffset2 =
                  v2xuint + v2yuint * m_Width + v2zuint * m_Width * m_Height;
              v1x = (GLfloat)(v1xuint);
              v1y = (GLfloat)(v1yuint);
              v1z = (GLfloat)(v1zuint);
              v2x = (GLfloat)(v2xuint);
...
void RGBAExtractor::setData(unsigned char *data, unsigned char *red,
...
void RGBAExtractor::classifyVertices(unsigned int k, unsigned int *cacheMemory,
...
inline unsigned int RGBAExtractor::determineCase(unsigned char *data,
...
void RGBAExtractor::calcOffsets(unsigned int *offsets, unsigned int width,
                                unsigned int height) {
