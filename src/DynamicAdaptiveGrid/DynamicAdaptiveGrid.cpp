...
DynamicAdaptiveGrids::DynamicAdaptiveGrids(double grid_spacing,
                                           double pg_spacing, int m,
                                           double solvent_rad, double iso,
...
DynamicAdaptiveGrids::~DynamicAdaptiveGrids() { cleanup(); }
...
void DynamicAdaptiveGrids::createGrid()
{
        ...
        double centerDist = gridSpacing * 0.87; // using slightly higher than
sqrt(3)/2. It is used as query distance.
        ...
        for(int i=0; i<zCells; i++)
        {
                ...
                for(int j=0; j<yCells; j++)
                {
                        ...
                        for(int k=0; k<xCells; k++)
                        {
                                ...
                                vector<DPG::Point *> nei =
gridPointPG->range(&gridCellArray[i][j][k], centerDist);
        ...
        for(int i=0; i<zCells+1; i++)
        {
                for(int j=0; j<yCells+1; j++)
                {
                        for(int k=0; k<xCells+1; k++)
                        {
                                gridPointArray[i][j][k].setGridCells(gridCellPG->range(&gridPointArray[i][j][k],
centerDist));
                        }
                }
        }
...
void DynamicAdaptiveGrids::findGridPointsInBall(double x, double y, double z,
                                                double d,
  ...
  double bboxMinGPX =
      gridSpacing *
      (double)((int)(bboxMinX /
                     gridSpacing)); // find the min corner of the bounding box
  if (bboxMinX < 0)
    bboxMinGPX -= gridSpacing;
  double bboxMinGPY = gridSpacing * (double)((int)(bboxMinY / gridSpacing));
  if (bboxMinY < 0)
    bboxMinGPY -= gridSpacing;
  double bboxMinGPZ = gridSpacing * (double)((int)(bboxMinZ / gridSpacing));
  if (bboxMinZ < 0)
    bboxMinGPZ -= gridSpacing;

  for (int i = 0; i < numPoints; i++) {
    for (int j = 0; j < numPoints; j++) {
      for (int k = 0; k < numPoints; k++) {
        GridPoint *gp = new GridPoint(bboxMinGPX + gridSpacing * i,
                                      bboxMinGPY + gridSpacing * j,
                                      bboxMinGPZ + gridSpacing * k);

        //                              cout<<bboxMinGPX+gridSpacing*i<<" "<<
        //                              bboxMinGPY+gridSpacing*j<<" "<<
        //                              bboxMinGPZ+gridSpacing*k<<endl;
...
bool DynamicAdaptiveGrids::updateCell(GridCell *gc) {
  ...
  double mod[8][3] = {{-dist, -dist, -dist}, {dist, -dist, -dist},
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (xmod < 0.001 && xmod > -0.001)
      xmod = 0.0;
    if (ymod < 0.001 && ymod > -0.001)
      ymod = 0.0;
    if (zmod < 0.001 && zmod > -0.001)
      zmod = 0.0;
...
bool DynamicAdaptiveGrids::updateCells() {
  ...
  for (cellIt = cellStart; cellIt != cellEnd; cellIt++) {
    ...
    if (newClass == VDW_CELL ||
        newClass == BAND_CELL) // Need to create/update mesh.
    {
    ...
    } else if (oldClass == VDW_CELL ||
               oldClass == BAND_CELL) // Need to delete mesh.
    {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      for (int k = 0; k < 2; k++) {
        ...
        if (newlyUpdated.find(str) ==
            newlyUpdated.end()) // Need to add it to the update list
        {
...
bool DynamicAdaptiveGrids::addAtom(PDBParser::Atom *a) {
  ...
  // Find the gridpoints inside the SAS and VDW volume of the atom. Mark them as
  // exposed/burried and update the neighboring cells if necessary.

  vector<GridPoint *> gridPointsInSAS;
  //      gridPointsInSAS = gridPointPG->range(a, rad + solventRadius);

  findGridPointsInBall(a->getX(), a->getY(), a->getZ(), rad + solventRadius,
                       gridPointsInSAS);
  ...
  for (int i = 0; i < numGridPointsInSAS; i++) {
    ...
    // the three cases, where an update is necessary are OUT->SAS, OUT->VDW,
    // SAS->VDW update may be needed for VDW->VDW, SAS->SAS OUT->OUT,  cannot
    // happen for gp VDW->SAS cannot happen during insertion of an atom
    ...
    if (newState == VDW_POINT) {
      ...
      else if (oldState == SAS_POINT) // remove point from exposed list
      {
        ...
        markNeighborCells(
            gp); // might need to update the mesh. but not the state
        vdwToVdwPoint++;
      }
  ...
  // If this atom contributed to the surface (converted a GridPoint), then mark
  // it as exposed. Check if it burried any previously exposed atom.

  if (isExposedAtom) {
    vector<DPG::Point *> neighborExposedAtoms =
        exposedAtomPG->range(a, rad + solventRadius);
...
bool DynamicAdaptiveGrids::removeAtom(PDBParser::Atom *a) {
  ...
  for (int i = 0; i < numGridPointsInSAS; i++) {
    ...
    if (oldState == SAS_POINT) {
      if (!allAtomPG->pointsWithinRange(
              gp, 2 * solventRadius)) // not inside SAS of other atoms
      {
      ...
      if (!allAtomPG->pointsWithinRange(gp,
                                        solventRadius)) //  it is no longer VDW
      {
        vector<DPG::Point *> newlyExposedAtom =
            allAtomPG->range(gp, 2 * solventRadius);
...
bool DynamicAdaptiveGrids::moveAtom(PDBParser::Atom *from,
                                    PDBParser::Atom *to) {
  removeAtom(from);
  addAtom(to);
}
...
bool DynamicAdaptiveGrids::printSurface(string surfaceFileName) {
  ...
  /*
          ...
          {
                  cellFile<< cells[i]->getX() <<" "<< cells[i]->getY() <<" "<<
     cells[i]->getZ() <<" 0.05"<<endl;
          }
          cellFile.close();
  ...
  */
  ...
  surfFile << numVerts << " " << numTraingles
           << endl; // printing the number of vertices and triangles

  vertCon.printVerticesAndUpdateMap(
      &surfFile); // prints the mesh vertices and updates a map from
                  // vertex->index in the raw file
