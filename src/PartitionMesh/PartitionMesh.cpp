...
void usagePartitionMesh() {
  cout << endl
       << "Usage: MolSurf -partitionMesh <raw input file> <# of partitions>"
       << endl
       << "    Partition mesh for parallel computation." << endl;
}
...
bool partitionMesh(int argc, char **argv) {
  ...
  for (int iP = 0; iP < nP; iP++) {
    ...
    for (int i = vstart[iP]; i <= vend[iP]; i++) {
      fout << x[new2old[i]] << " " << y[new2old[i]] << " " << z[new2old[i]];

      if (haveNormal) {
        fout << " " << n1[new2old[i]] << " " << n2[new2old[i]] << " "
             << n3[new2old[i]];
      }
