...
BiomoleculeTransformations::BiomoleculeTransformations(
    const BiomoleculeTransformations &b) {
  chainIds = b.chainIds;
  rotation = b.rotation;
...
bool BiomoleculeTransformations::print(FILE *fp) {
  ...
  for (i = 0; i < numRotations / 9; i++) {
    ...
    fprintf(fp, "\t[%f %f %f]\n", translation.at(i * 3 + 0),
            translation.at(i * 3 + 1), translation.at(i * 3 + 2));
  }
  return true;
}
...
bool BiomoleculeTransformations::getRotations(char currentChainId,
                                              double *rotationsArray) {
  if (!rotationsArray) {
    return false;
  }
  int n = chainIds.size();
...
bool BiomoleculeTransformations::getTranslations(char currentChainId,
