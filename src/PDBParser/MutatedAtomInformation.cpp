...
MutatedAtomInformation::MutatedAtomInformation(float radius, const char *name,
                                               unsigned char residueID,
                                               float charge) {
  m_Radius = radius;
  if (name) {
    strcpy(m_Name, name);
...
MutatedAtomInformation::MutatedAtomInformation(
    const MutatedAtomInformation &m) {
  m_Radius = m.m_Radius;
...
unsigned char MutatedAtomInformation::getResidueId() { return m_ResidueId; }
...
void MutatedAtomInformation::resetColor() {
  ...
  int elementIndex =
      p->mapPQRElementAndResidue(m_Name, residueTable[m_ResidueId].residueName);
  delete p;
