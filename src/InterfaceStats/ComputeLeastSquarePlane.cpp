...
bool ComputeLeastSquarePlane::compute(vector<DPG::Point *> &pts,
                                      InterfaceStats *is) {
  intStat = is;
...
bool ComputeLeastSquarePlane::computePlane(vector<DPG::Point *> &pts) {
  ...
  double a[LDA * N] = {a00, 0.00, 0.00, a10, a11, 0.0, a20, a21, a22};
  ...
  if (info > 0) {
    ...
    // w[0] is the minimal eignevalue, a[0-2] is corresponding unit eigen
    // vector. This is the least squares solution
...
void ComputeLeastSquarePlane::computeStats(vector<DPG::Point *> &pts) {
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    dist = fabs((plane[0] * x + plane[1] * y + plane[2] * z + plane[3]) /
                normalizingDenumerator);
  ...
  intStat->setCircularityIndex(
      eigenValues[1] /
      eigenValues[2]); // ratio of the minor/major components on the plane
}
