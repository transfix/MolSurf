...
int main(int argc, char **argv) {
  if (argc < 9) {
    printf("Usage: findHitsUsingInterfaceOverlap <receptor.rawn> "
           "<ligand1.rawn> <ligand2.rawn> <overlapRatioThreshold> <interface "
           "width> <xformFile> <startIndex> <endIndex>");
    return -1;
  }
  ...
  InterfaceFootprintOverlap *ifo = new InterfaceFootprintOverlap(
  ...
  if (areaOfReceptorsInterfaceWithStaticLigand == 0.0) {
    printf("Static Ligand does not have interface with receptor. Cannot "
           "compute Overlap. Quitting");
    return -2;
  }
  ...
  for (int i = 0; i <= endIndex; i++) {
    ...
    if (i >= startIndex) {
      if (ifo->computeInterfaces(mtx)) {
        ...
        areaOfMovingLigandsInterfaceWithReceptor =
            ifo->getAreaOfMovingLigandInterface();
        ratio =
            areaOfinterfaceOverlap / areaOfReceptorsInterfaceWithStaticLigand;
        ratio2 =
            areaOfinterfaceOverlap / areaOfMovingLigandsInterfaceWithReceptor;
