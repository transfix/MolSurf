...
BindingSiteSummarizer::BindingSiteSummarizer(string sf1, string sf2, string sf3,
                                             double iw, bool v) {
  verbose = v;
...
BindingSiteSummarizer::~BindingSiteSummarizer() { cleanup(); }
...
bool BindingSiteSummarizer::detectInterfacePoints(Matrix xform) {
  ...
  if (verbose)
    cout << "Receptor's interface with moving computed\n";
...
bool BindingSiteSummarizer::computeSiteSummary(vector<Matrix> &xforms) {
  ...
  for (int i = 0; i < numXforms; i++) {
    if (verbose)
      cout << "Computing interface for pose " << i + 1 << endl;
...
bool BindingSiteSummarizer::prepareBindingSiteSurfaceFile() {
  ...
  if (!bindingSitesFile) {
    cout << "could not create surface file " << bindingSitesFileName.c_str()
         << endl;
    return false;
  }
