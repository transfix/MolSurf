...
AtomsOnInterface::AtomsOnInterface(vector<PDBParser::Atom *> &molecule1,
                                   vector<PDBParser::Atom *> &molecule2) {
  setDefaults();
  initStats();
...
void AtomsOnInterface::setDefaults(void) {
  ...
  string residueNames[20] = {"ILE", "VAL", "LEU", "PHE", "CYS", "MET", "ALA",
                             "GLY", "THR", "SER", "TRP", "TYR", "PRO", "HIS",
                             "GLU", "GLN", "ASP", "ASN", "LYS", "ARG"};
...
int AtomsOnInterface::constructAtomsOctree(int atomsStartID, int atomsEndID,
  ...
  PDBParser::Atom *atm;
  if (stc)
    atm = staticAtoms[atomsStartID];
  else
    atm = movingAtoms[atomsStartID];
  ...
  if ((atomsEndID - atomsStartID + 1 > maxLeafSize) &&
      (cr > minRadius)) // checking if splitting is needed
  {
    aon.leaf = false;
    ...
    for (int i = atomsStartID; i <= atomsEndID; i++) {
      if (stc)
        atomsT[i] = staticAtoms[i];
      else
        atomsT[i] = movingAtoms[i];
      ...
      int splitCellIndex = (zeroIfLess(temp->getZ(), cz) << 2) +
                           (zeroIfLess(temp->getY(), cy) << 1) +
                           (zeroIfLess(temp->getX(), cx));
    ...
    for (int i = 1; i < 8; i++)
      atomsCurIndex[i] = atomsStartIndex[i] =
          atomsStartIndex[i - 1] + atomsCount[i - 1];

    // Bucketing the atoms for the splitcells

    for (int i = atomsStartID; i <= atomsEndID; i++) {
      int splitCellIndex = (zeroIfLess(atomsT[i]->getZ(), cz) << 2) +
                           (zeroIfLess(atomsT[i]->getY(), cy) << 1) +
                           (zeroIfLess(atomsT[i]->getX(), cx));
    ...
    for (int i = 0; i < 8; i++) {
      if (atomsCount[i] > 0) {
        aon.cPtr[i] = constructAtomsOctree(
            atomsStartIndex[i], atomsStartIndex[i] + atomsCount[i] - 1, atomsT,
            stc);
      }
  ...
  if (stc) {
    ...
    return staticAtomsOctree.size() -
           1; // returning index, so that parent can set child pointers
  } else {
    movingAtomsOctree.push_back(aon);
    return movingAtomsOctree.size() -
           1; // returning index, so that parent can set child pointers
  }
}

void AtomsOnInterface::fillMovingAtomsSubtreeRootsArray(int nodeID,
                                                        int maxNodesInLevel) {
  if (nodeID < 0)
    return;
  ...
  if (movingAtomsOctree[nodeID].leaf ||
      (maxNodesInLevel >= movingAtomsSubtreeRootsSize)) {
    movingAtomsSubtreeRoots[maxSubtreeRoot++] = nodeID;
  } else {
    for (int i = 0; i < 8; i++)
      if (movingAtomsOctree[nodeID].cPtr[i] >= 0)
        fillMovingAtomsSubtreeRootsArray(movingAtomsOctree[nodeID].cPtr[i],
                                         8 * maxNodesInLevel);
  }
}
...
bool AtomsOnInterface::buildOctrees(void) {
  if ((minRadius != minRadiusUsed) || (maxLeafSize != maxLeafSizeUsed) ||
      (!staticAtomsOctreeBuilt))
    buildStaticAtomsOctree();
  if ((minRadius != minRadiusUsed) || (maxLeafSize != maxLeafSizeUsed) ||
      (!movingAtomsOctreeBuilt))
...
void AtomsOnInterface::approximateInteractions(Matrix transMat, int nodeS,
                                               int nodeM) {
  ...
  if (farEnough) {
    ...
    if (staticAtomsOctree[nodeS].leaf &&
        movingAtomsOctree[nodeM].leaf) // Base case: both are leaves.
    {
      for (int i = movingAtomsOctree[nodeM].atomsStartID;
           i <= movingAtomsOctree[nodeM].atomsEndID; i++) {
        Vector oldPos(movingAtoms[i]->getX(), movingAtoms[i]->getY(),
                      movingAtoms[i]->getZ(), 1.0);
        Vector newPos = transMat * oldPos;
        double xM = newPos[0], yM = newPos[1], zM = newPos[2];
        DPG::Point *tempM = new DPG::Point(xM, yM, zM);

        for (int j = staticAtomsOctree[nodeS].atomsStartID;
             j <= staticAtomsOctree[nodeS].atomsEndID; j++) {
          ...
          if (d2 < neighborDistCutoff *
                       neighborDistCutoff) // these atoms are on the interface
          {
        ...
        if (staticAtomsOctree[nodeS].cPtr[i] >= 0) {
          for (int j = 0; j < 8; j++) {
            if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
              approximateInteractions(transMat,
                                      staticAtomsOctree[nodeS].cPtr[i],
                                      movingAtomsOctree[nodeM].cPtr[j]);
            }
          }
        }
      }
      ...
      for (int i = 0; i < 8; i++) {
        if (staticAtomsOctree[nodeS].cPtr[i] >= 0) {
          approximateInteractions(transMat, staticAtomsOctree[nodeS].cPtr[i],
                                  nodeM);
        }
      }
    }
      ...
      for (int j = 0; j < 8; j++) {
        if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
          approximateInteractions(transMat, nodeS,
                                  movingAtomsOctree[nodeM].cPtr[j]);
        }
      }
    }
...
bool AtomsOnInterface::computeInteractions(Matrix transMat,
                                           InterfaceStats *intStat) {
  if (!staticAtomsOctreeBuilt || !movingAtomsOctreeBuilt)
    return false;
  ...
  cout << numAtoms[0] << " " << numCore[0] << " " << numRim[0] << " "
       << numPolar[0] << " " << numNonPolar[0] << " " << endl;

  approximateInteractions(transMat, staticAtomsOctreeRoot,
                          movingAtomsOctreeRoot);
  ...
  /*      std::ofstream outfile;
          ...
          for(int i=0;i<interfaceAtoms.size();i++)
                  outfile<<interfaceAtoms[i]->getX()<<"
     "<<interfaceAtoms[i]->getY()<<" "<<interfaceAtoms[i]->getZ()<<endl;
          outfile.close();*/
  ...
  cout << "Matrix: \n";
  transMat.print();
  cout << "Found " << numAtoms[0] << " and " << numAtoms[1]
       << " interface atoms" << endl;
  cout << "Total atoms " << staticAtoms.size() << " and " << movingAtoms.size()
       << endl;
