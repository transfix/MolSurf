/*
  Copyright 2011 The University of Texas at Austin
	...
	Authors: Muhibur Rasheed
 * <muhibur@ices.utexas.edu>
 ...
 * Software Foundation.
  ...
  MolSurf is distributed in the hope that it will be
 * useful,
 ...
 * GNU General Public License
  along with MolSurf; if not, write to the Free
 * Software
...
PointsOnInterface::PointsOnInterface(vector<DPG::WeightedPoint *> &pts1,
                                     vector<DPG::WeightedPoint *> &pts2) {
  setDefaults();
...
void PointsOnInterface::copyPoints(vector<DPG::WeightedPoint *> &pts,
                                   bool firstSet) {
  int size = pts.size();
...
int PointsOnInterface::constructOctree(int pointsStartID, int pointsEndID,
  ...
  if (firstSet)
    pt = pointList1[pointsStartID];
  else
    pt = pointList2[pointsStartID];
  ...
  if ((pointsEndID - pointsStartID + 1 > maxLeafSize) &&
      (cr > minRadius)) // checking if splitting is needed
  {
    ...
    for (int i = 1; i < 8; i++)
      ptsCurIndex[i] = ptsStartIndex[i] =
          ptsStartIndex[i - 1] + ptsCount[i - 1];

    // Bucketing the atoms for the splitcells

    for (int i = pointsStartID; i <= pointsEndID; i++) {
      int splitCellIndex = (zeroIfLess(ptsT[i]->getZ(), cz) << 2) +
                           (zeroIfLess(ptsT[i]->getY(), cy) << 1) +
                           (zeroIfLess(ptsT[i]->getX(), cx));
    ...
    for (int i = 0; i < 8; i++) {
      if (ptsCount[i] > 0) {
        pon.cPtr[i] =
            constructOctree(ptsStartIndex[i],
                            ptsStartIndex[i] + ptsCount[i] - 1, ptsT, firstSet);
      }
...
void PointsOnInterface::approximateInteractions(Matrix transMat,
                                                Matrix transMatInv, int nodeS,
                                                int nodeM) {
  double sumRad = staticOctree[nodeS].cr + movingOctree[nodeM].cr;
  double sumRad2 = sumRad * sumRad;

  Vector oldPos(movingOctree[nodeM].cx, movingOctree[nodeM].cy,
                movingOctree[nodeM].cz, 1.0);
  Vector newPos = transMat * oldPos;
  double cxM = newPos[0], cyM = newPos[1], czM = newPos[2];

  double dx = staticOctree[nodeS].cx - cxM, dy = staticOctree[nodeS].cy - cyM,
  ...
  if (farEnough) {
    ...
    if (staticOctree[nodeS].leaf &&
        movingOctree[nodeM].leaf) // Base case: both are leaves.
    {
      for (int i = movingOctree[nodeM].pointsStartID;
           i <= movingOctree[nodeM].pointsEndID; i++) {
        Vector oldPos(pointList2[i]->getX(), pointList2[i]->getY(),
                      pointList2[i]->getZ(), 1.0);
        Vector newPos = transMat * oldPos;
        double xM = newPos[0], yM = newPos[1], zM = newPos[2];

        DPG::WeightedPoint *temp =
            new DPG::WeightedPoint(xM, yM, zM, pointList2[i]->getWeight());
      ...
      for (int j = staticOctree[nodeS].pointsStartID;
           j <= staticOctree[nodeS].pointsEndID; j++) {
        Vector oldPos(pointList1[j]->getX(), pointList1[j]->getY(),
                      pointList1[j]->getZ(), 1.0);
        Vector newPos = transMatInv * oldPos;
        double xM = newPos[0], yM = newPos[1], zM = newPos[2];

        DPG::WeightedPoint *temp =
            new DPG::WeightedPoint(xM, yM, zM, pointList1[j]->getWeight());
        ...
        if (staticOctree[nodeS].cPtr[i] >= 0) {
          for (int j = 0; j < 8; j++) {
            if (movingOctree[nodeM].cPtr[j] >= 0) {
              approximateInteractions(transMat, transMatInv,
                                      staticOctree[nodeS].cPtr[i],
                                      movingOctree[nodeM].cPtr[j]);
            }
          }
        }
      }
      ...
      for (int i = 0; i < 8; i++) {
        if (staticOctree[nodeS].cPtr[i] >= 0) {
          approximateInteractions(transMat, transMatInv,
                                  staticOctree[nodeS].cPtr[i], nodeM);
        }
      }
    }
      ...
      for (int j = 0; j < 8; j++) {
        if (movingOctree[nodeM].cPtr[j] >= 0) {
          approximateInteractions(transMat, transMatInv, nodeS,
                                  movingOctree[nodeM].cPtr[j]);
        }
      }
    }
...
bool PointsOnInterface::computeInteractions(Matrix transMat,
                                            InterfaceStats *intStat) {
  if (!staticOctreeBuilt || !movingOctreeBuilt)
    return false;
  ...
  approximateInteractions(transMat, transMatInv, staticOctreeRoot,
                          movingOctreeRoot); // fills the interfacePointList

  cout << "Identified " << interfacePointList.size() << " interface points"
       << endl;
  ...
  /*	std::ofstream outfile;
          ...
          for(int i=0;i<interfacePointList.size();i++)
                  outfile<<interfacePointList[i]->getX()<<"
     "<<interfacePointList[i]->getY()<<" "<<interfacePointList[i]->getZ()<<endl;
          outfile.close();*/
