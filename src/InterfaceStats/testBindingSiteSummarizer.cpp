...
int main(int argc, char **argv) {
  if (argc < 8) {
    printf("Usage: testBindingSiteSummarizer <receptor.rawn> <ligand.rawn> "
           "<interface width> <xformFile> <startIndex> <endIndex> <output "
           "rawn>\n");
    return -1;
  }
  ...
  BindingSiteSummarizer *bss =
      new BindingSiteSummarizer(std::string(argv[1]), std::string(argv[2]),
                                std::string(argv[7]), interfaceWidth, true);
