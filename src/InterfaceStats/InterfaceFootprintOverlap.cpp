...
InterfaceFootprintOverlap::InterfaceFootprintOverlap(string sf1, string sf2,
                                                     string sf3, double iw,
                                                     bool v) {
  verbose = v;
...
InterfaceFootprintOverlap::~InterfaceFootprintOverlap() { cleanup(); }
...
void InterfaceFootprintOverlap::cleanup() {
  ...
  if (receptorPointsOnInterfaceWithLigand1)
    delete receptorPointsOnInterfaceWithLigand1;
  if (receptorPointsOnInterfaceWithLigand2)
    delete receptorPointsOnInterfaceWithLigand2;
  if (ligandPointsOnInterface1)
    delete ligandPointsOnInterface1;
  if (ligandPointsOnInterface2)
    delete ligandPointsOnInterface2;
  if (receptorPointsOnFootPrintOverlap)
    delete receptorPointsOnFootPrintOverlap;
...
bool InterfaceFootprintOverlap::openFiles() {
  ...
  if (!ligandSurfaceFile1) {
    cout << "could not open surface file " << ligandSurfaceFileName1.c_str()
         << endl;
    fclose(receptorSurfaceFile);
    return false;
  }
  ...
  if (!ligandSurfaceFile2) {
    cout << "could not open surface file " << ligandSurfaceFileName2.c_str()
         << endl;
    fclose(receptorSurfaceFile);
    fclose(ligandSurfaceFile1);
    return false;
...
bool InterfaceFootprintOverlap::readFiles() {
  ...
  for (int i = 0; i < numVertR; i++) {
    if (fscanf(receptorSurfaceFile, "%lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx,
               &ny, &nz) != 6) {
  ...
  for (int i = 0; i < numTriangR; i++) {
    ...
    receptorPoints[index1]->setWeight(receptorPoints[index1]->getWeight() +
                                      area / 3.0);
    receptorPoints[index2]->setWeight(receptorPoints[index2]->getWeight() +
                                      area / 3.0);
    receptorPoints[index3]->setWeight(receptorPoints[index3]->getWeight() +
                                      area / 3.0);
  }
  ...
  if (verbose)
    cout << "Ligand 1 has: " << numVertL1 << " vertices and " << numTriangL1
         << " triangles\n";

  ligandPointsOnInterface1 = new bool[numVertL1];

  for (int i = 0; i < numVertL1; i++) {
    if (fscanf(ligandSurfaceFile1, "%lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx,
               &ny, &nz) != 6) {
  ...
  if (verbose)
    cout << "Points read from ligand 1 surface and DPG created\n";

  for (int i = 0; i < numTriangL1; i++) {
    if (fscanf(ligandSurfaceFile1, "%d %d %d", &index1, &index2, &index3) !=
        3) {
      cout << "Wrong format in ligand surface file\n";
      return false;
    }
    ...
    dx =
        staticLigandPoints[index2]->getX() - staticLigandPoints[index1]->getX();
    dy =
        staticLigandPoints[index2]->getY() - staticLigandPoints[index1]->getY();
    dz =
        staticLigandPoints[index2]->getZ() - staticLigandPoints[index1]->getZ();
    a = sqrt(dx * dx + dy * dy + dz * dz);

    dx =
        staticLigandPoints[index3]->getX() - staticLigandPoints[index2]->getX();
    dy =
        staticLigandPoints[index3]->getY() - staticLigandPoints[index2]->getY();
    dz =
        staticLigandPoints[index3]->getZ() - staticLigandPoints[index2]->getZ();
    b = sqrt(dx * dx + dy * dy + dz * dz);

    dx =
        staticLigandPoints[index1]->getX() - staticLigandPoints[index3]->getX();
    dy =
        staticLigandPoints[index1]->getY() - staticLigandPoints[index3]->getY();
    dz =
        staticLigandPoints[index1]->getZ() - staticLigandPoints[index3]->getZ();
    c = sqrt(dx * dx + dy * dy + dz * dz);
    ...
    staticLigandPoints[index1]->setWeight(
        staticLigandPoints[index1]->getWeight() + area / 3.0);
    staticLigandPoints[index2]->setWeight(
        staticLigandPoints[index2]->getWeight() + area / 3.0);
    staticLigandPoints[index3]->setWeight(
        staticLigandPoints[index3]->getWeight() + area / 3.0);
  }

  fclose(ligandSurfaceFile1);

  if (verbose)
    cout << "Triangles read from ligand 1 surface\n";
  ...
  if (verbose)
    cout << "Ligand 2 has: " << numVertL2 << " vertices and " << numTriangL2
         << " triangles\n";

  ligandPointsOnInterface2 = new bool[numVertL2];

  for (int i = 0; i < numVertL2; i++) {
    if (fscanf(ligandSurfaceFile2, "%lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx,
               &ny, &nz) != 6) {
  ...
  if (verbose)
    cout << "Points read from ligand 2 surface and DPG created\n";

  for (int i = 0; i < numTriangL2; i++) {
    if (fscanf(ligandSurfaceFile2, "%d %d %d", &index1, &index2, &index3) !=
        3) {
    ...
    dx =
        movingLigandPoints[index2]->getX() - movingLigandPoints[index1]->getX();
    dy =
        movingLigandPoints[index2]->getY() - movingLigandPoints[index1]->getY();
    dz =
        movingLigandPoints[index2]->getZ() - movingLigandPoints[index1]->getZ();
    a = sqrt(dx * dx + dy * dy + dz * dz);

    dx =
        movingLigandPoints[index3]->getX() - movingLigandPoints[index2]->getX();
    dy =
        movingLigandPoints[index3]->getY() - movingLigandPoints[index2]->getY();
    dz =
        movingLigandPoints[index3]->getZ() - movingLigandPoints[index2]->getZ();
    b = sqrt(dx * dx + dy * dy + dz * dz);

    dx =
        movingLigandPoints[index1]->getX() - movingLigandPoints[index3]->getX();
    dy =
        movingLigandPoints[index1]->getY() - movingLigandPoints[index3]->getY();
    dz =
        movingLigandPoints[index1]->getZ() - movingLigandPoints[index3]->getZ();
    c = sqrt(dx * dx + dy * dy + dz * dz);
    ...
    movingLigandPoints[index1]->setWeight(
        movingLigandPoints[index1]->getWeight() + area / 3.0);
    movingLigandPoints[index2]->setWeight(
        movingLigandPoints[index2]->getWeight() + area / 3.0);
    movingLigandPoints[index3]->setWeight(
        movingLigandPoints[index3]->getWeight() + area / 3.0);
  }

  fclose(ligandSurfaceFile2);

  if (verbose)
    cout << "Triangles read from ligand 2 surface\n";
...
bool InterfaceFootprintOverlap::initializeStaticData() {
  ...
  if (verbose)
    cout << "DPG for static ligand created\n";
...
bool InterfaceFootprintOverlap::detectInterfacePointsForStaticLigand() {
  ...
  if (verbose)
    cout << "Receptor's interface with static ligand computed\n";
  ...
  if (verbose)
    cout << "Static Ligand's interface with receptor computed\n";
...
bool InterfaceFootprintOverlap::initializeMovingData() {
  ...
  for (int i = 0; i < numVertL2; i++) {
    ...
    DPG::WeightedPoint *p = new DPG::WeightedPoint(
        newx, newy, newz, movingLigandPoints[i]->getWeight());
    pgL2->addPoint(p);
    movingLigandPointsXformed.push_back(p);
  }

  if (verbose)
    cout << "DPG for moving ligand created\n";
...
bool InterfaceFootprintOverlap::detectInterfacePointsForMovingLigand() {
  ...
  if (verbose)
    cout << "Receptor's interface with moving ligand computed\n";
  ...
  if (verbose)
    cout << "Moving Ligand's interface with receptor computed\n";
...
bool InterfaceFootprintOverlap::prepareStaticInterfaceSurfaceFiles() {
  ...
  receptorHighlightedSurfaceFileName1 =
      receptorSurfaceFileName.substr(0, receptorSurfaceFileName.length() - 5) +
      "_interface_with_" +
      ligandSurfaceFileName1.substr(0, ligandSurfaceFileName1.length() - 5) +
      ".rawc";
  ligandHighlightedSurfaceFileName1 =
      ligandSurfaceFileName1.substr(0, ligandSurfaceFileName1.length() - 5) +
      "_interface_with_" +
      receptorSurfaceFileName.substr(0, receptorSurfaceFileName.length() - 5) +
      ".rawc";

  receptorHighlightedSurface1 =
      fopen(receptorHighlightedSurfaceFileName1.c_str(), "wt");
  if (!receptorHighlightedSurface1) {
    cout << "could not create surface file "
         << receptorHighlightedSurfaceFileName1.c_str() << endl;
    return false;
  }

  ligandHighlightedSurface1 =
      fopen(ligandHighlightedSurfaceFileName1.c_str(), "wt");
  if (!ligandHighlightedSurface1) {
    cout << "could not create surface file "
         << ligandHighlightedSurfaceFileName1.c_str() << endl;
    fclose(receptorHighlightedSurface1);
    return false;
  }
  ...
  for (int i = 0; i < numVertR; i++) {
    fprintf(receptorHighlightedSurface1, "%lf %lf %lf",
            receptorPoints[i]->getX(), receptorPoints[i]->getY(),
            receptorPoints[i]->getZ());
  ...
  for (int i = 0; i < numTriangR; i++) {
    fprintf(receptorHighlightedSurface1, "%d %d %d\n",
            receptorTriangles[i].vertex[0], receptorTriangles[i].vertex[1],
            receptorTriangles[i].vertex[2]);
  }
  ...
  for (int i = 0; i < numVertL1; i++) {
    fprintf(ligandHighlightedSurface1, "%lf %lf %lf",
            staticLigandPoints[i]->getX(), staticLigandPoints[i]->getY(),
            staticLigandPoints[i]->getZ());
  ...
  for (int i = 0; i < numTriangL1; i++) {
    fprintf(ligandHighlightedSurface1, "%d %d %d\n",
            staticLigandTriangles[i].vertex[0],
            staticLigandTriangles[i].vertex[1],
            staticLigandTriangles[i].vertex[2]);
  }
...
bool InterfaceFootprintOverlap::prepareMovingInterfaceSurfaceFiles(int poseId) {
  ...
  receptorHighlightedSurfaceFileName2 =
      receptorSurfaceFileName.substr(0, receptorSurfaceFileName.length() - 5) +
      "_interface_with_" +
      ligandSurfaceFileName2.substr(0, ligandSurfaceFileName2.length() - 5) +
      "_pose_" + pose + ".rawc";

  ligandHighlightedSurfaceFileName2 =
      ligandSurfaceFileName2.substr(0, ligandSurfaceFileName2.length() - 5) +
      "_pose_" + pose + "_interface_with_" +
      receptorSurfaceFileName.substr(0, receptorSurfaceFileName.length() - 5) +
      ".rawc";

  receptorFootprintSurfaceFileName =
      receptorSurfaceFileName.substr(0, receptorSurfaceFileName.length() - 5) +
      "_footprint_overlap_of_" +
      ligandSurfaceFileName1.substr(0, ligandSurfaceFileName1.length() - 5) +
      "_and_" +
      ligandSurfaceFileName2.substr(0, ligandSurfaceFileName2.length() - 5) +
      "_pose_" + pose + ".rawc";

  receptorHighlightedSurface2 =
      fopen(receptorHighlightedSurfaceFileName2.c_str(), "wt");
  if (!receptorHighlightedSurface2) {
    cout << "could not create surface file "
         << receptorHighlightedSurfaceFileName2.c_str() << endl;
    return false;
  }

  ligandHighlightedSurface2 =
      fopen(ligandHighlightedSurfaceFileName2.c_str(), "wt");
  if (!ligandHighlightedSurface2) {
    cout << "could not create surface file "
         << ligandHighlightedSurfaceFileName2.c_str() << endl;
    fclose(receptorHighlightedSurface2);
    return false;
  }

  receptorFootprintSurface =
      fopen(receptorFootprintSurfaceFileName.c_str(), "wt");
  if (!receptorFootprintSurface) {
    cout << "could not create surface file "
         << receptorFootprintSurfaceFileName.c_str() << endl;
    fclose(receptorHighlightedSurface2);
    fclose(ligandHighlightedSurface2);
    return false;
  }
  ...
  for (int i = 0; i < numVertL2; i++) {
    fprintf(ligandHighlightedSurface2, "%lf %lf %lf",
            movingLigandPoints[i]->getX(), movingLigandPoints[i]->getY(),
            movingLigandPoints[i]->getZ());
  ...
  for (int i = 0; i < numTriangL2; i++) {
    fprintf(ligandHighlightedSurface2, "%d %d %d\n",
            movingLigandTriangles[i].vertex[0],
            movingLigandTriangles[i].vertex[1],
            movingLigandTriangles[i].vertex[2]);
  }
  ...
  for (int i = 0; i < numVertR; i++) {
    fprintf(receptorFootprintSurface, "%lf %lf %lf", receptorPoints[i]->getX(),
