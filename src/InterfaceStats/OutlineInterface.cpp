...
OutlineInterface::OutlineInterface(string sf1, string sf2, double iw, bool rm, bool v)
{
	verbose = v;
...
void OutlineInterface::cleanup()
{
	...
	if(receptorPointsOnInterface) delete receptorPointsOnInterface;
	if(receptorPointsOnInterfaceBoundary) delete receptorPointsOnInterfaceBoundary;
}
...
bool OutlineInterface::initialize()
{
	...
	for(int i=0; i<numVertR; i++)
	{
        ...
        if(rawcMode)
        {
		    if(fscanf(receptorSurfaceFile, "%lf %lf %lf %lf %lf %lf", &x, &y, &z, &(vc.r), &(vc.g), &(vc.b)) != 6)
		    {
			    cout<<"Wrong format in receptor surface file. At vertex no: "<< i <<"\n";
			    cout<< "x = " << x << ", y = " << y << ", z = " << z << ", r = " << vc.r << ", g = " << vc.g << ", b = " << vc.b << "\n";
			    return false;
		    }
        }
        else
        {
		    if(fscanf(receptorSurfaceFile, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx, &ny, &nz, &(vc.r), &(vc.g), &(vc.b)) != 9)
		    {
			    cout<<"Wrong format in receptor surface file. At vertex no: "<< i <<"\n";
			    cout<< "x = " << x << ", y = " << y << ", z = " << z << ", r = " << vc.r << ", g = " << vc.g << ", b = " << vc.b << "\n";
			    return false;
...
bool OutlineInterface::detectInterfacePoints(Matrix xform)
{
	...
	if(verbose) cout<<"Receptor's interface with ligand computed\n";
	...
	for(int i=0; i<numTriangR; i++)
	{
		...
		if(vert1<0 || vert2<0 || vert3<0 || vert1>=numVertR || vert2>=numVertR || vert3>=numVertR)
		{
			cout<<"Error: Some vertex indices are out of bounds\n";
		}
		else
		{
			if( (receptorPointsOnInterface[vert1] || receptorPointsOnInterface[vert2] || receptorPointsOnInterface[vert3]) &&
				! (receptorPointsOnInterface[vert1] && receptorPointsOnInterface[vert2] && receptorPointsOnInterface[vert3]) )
			{
	...
	if(verbose) cout<<"Boundary vertices detected\n";

	return true;
}


bool OutlineInterface::computeOutline(Matrix xform, string outputFName, vertexcolor outlineColor)
{
...
bool OutlineInterface::prepareBindingSiteOutlineSurfaceFile(vertexcolor outlineColor)
{
	...
	for(int i=0; i<numVertR; i++)
	{
		...
		{
			fprintf(bindingSitesFile, " %lf %lf %lf\n", receptorPointColors[i].r, receptorPointColors[i].g, receptorPointColors[i].b);		
		}
	}

	if(verbose) cout<<"Points written to file\n";
	...
	if(verbose) cout<<"Triangles written to file\n";
