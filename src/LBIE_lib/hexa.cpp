...
void Octree::vflag_clear() { memset(vbit, 0, octcell_num * 4 / 8); }
...
int Octree::is_min_vertex(int oc_id, int v_id, unsigned int *vtx,
  ...
  case 5:
    if (is_refined(x, y, z - 1, level) || is_refined(x + 1, y, z - 1, level) ||
        ...
        is_refined(x + 1, y, z, level) || is_refined(x + 1, y + 1, z, level) ||
  ...
  if (vtx[0] >= 0 && vtx[1] >= 0 && vtx[2] >= 0 && vtx[3] >= 0 && vtx[4] >= 0 &&
      vtx[5] >= 0 && vtx[6] >= 0 && vtx[7] >= 0) {
    return 1;
  } else {
    return 0;
  }
...
void Octree::find_oc_id(int x, int y, int z, int level, int j, int intersect_id,
                        int *oc_id) {
  int i, temp_id[4];
  oc_id[0] = xyz2octcell(x, y, z, level);
  switch (j) {
