...
#include <LBIE_lib/cubes.h>
#include <LBIE_lib/vtkMarchingCubesCases.h>
...
using PDBParser::parserPDBtoGOA;

int *interpol_bit;

GroupOfAtoms *read_pdb(const char *fname) {
...
void Octree::pdbVolume(const char *pdb_fname) {
  ...
  volumeData = new SimpleVolumeData(dims);
  goaBlur->generateBlurmap(
      mol, volumeData, PDBParser::GroupOfAtoms::ELECTRON_DENSITY, 0, false,
      PDBParser::GroupOfAtoms::ATOM, PDBParser::GroupOfAtoms::VDW_RADIUS,
      PDBParser::ATOM_TYPE);
  VolumeLoader *volumeLoader = new VolumeLoader();
  string ses_file = pdb_fname;
  ses_file += ".rawiv";
  volumeLoader->saveFile(ses_file, volumeData);
  delete volumeLoader;
  loadVolume();
  int i;
  for (i = 0; i <= oct_depth; i++) {
  ...
  construct_octree((char *)pdb_fname);
  vol_min = minmax[0].min;
  vol_max = minmax[0].max;
}
...
void Octree::Octree_loadVolume(const char *dx_fname) {
  VolumeLoader *volumeLoader = new VolumeLoader();
  volumeData = volumeLoader->loadFile(dx_fname);
  loadVolume();
  int i;
  for (i = 0; i <= oct_depth; i++) {
    level_res[i] = (1 << i);
...
void Octree::loadVolume() {
  leaf_num = 0;
  prop_flag = 0;
  minext[0] = volumeData->getMinX();
  minext[1] = volumeData->getMinY();
  minext[2] = volumeData->getMinZ();
  maxext[0] = volumeData->getMaxX();
  maxext[1] = volumeData->getMaxY();
  maxext[2] = volumeData->getMaxZ();
  spans[0] = volumeData->getSpanX();
  spans[1] = volumeData->getSpanY();
  spans[2] = volumeData->getSpanZ();
  dim[0] = volumeData->getWidth();
  dim[1] = volumeData->getHeight();
  dim[2] = volumeData->getDepth();
  orig[0] = 0.0f;
  orig[1] = 0.0f;
  orig[2] = 0.0f;
  span[0] = 1.0f;
...
void Octree::loadData(SimpleVolumeData *volumeData, float *vol) {
  ...
  if (interior_flag) {
    float *tmp = NULL;
    tmp = (float *)volumeData->getData(0);
    if (!tmp) {
      return;
    }
    ...
    for (k = 0; k < dim[2]; k++)
      for (j = 0; j < dim[1]; j++)
        for (i = 0; i < dim[0]; i++) {
          index0 = (k - incr) * olddim[0] * olddim[1] + (j - incr) * olddim[0] +
                   (i - incr);
...
void Octree::initMem() {
  ...
  oct_array = (octcell *)callocSafely(sizeof(octcell) * octcell_num);
  minmax = (MinMax *)callocSafely(sizeof(MinMax) * octcell_num);
  cut_array = (int *)mallocSafely(sizeof(int) * 2 * cell_num);
  orig_vol = (float *)mallocSafely(sizeof(float) * dim[0] * dim[1] * dim[2]);
  ebit = (char *)callocSafely(sizeof(char) * octcell_num * 4 / 8);
  vbit = (char *)callocSafely(sizeof(char) * octcell_num * 4 / 8);
  vtx_idx_arr = (int *)mallocSafely(sizeof(int) * octcell_num);
  grid_idx_arr = (int *)mallocSafely(sizeof(int) * dim[0] * dim[1] * dim[2]);
  vtx_idx_arr_in = (int *)mallocSafely(sizeof(int) * octcell_num);
  vtx_idx_arr_refine = (int *)mallocSafely(sizeof(int) * octcell_num);
  qef_array = (double **)callocSafely(sizeof(double *) * octcell_num);
  qef_array_in = (double **)callocSafely(sizeof(double *) * octcell_num);
  for (int k = 0; k < octcell_num; k++) {
    vtx_idx_arr[k] = -1;
    vtx_idx_arr_in[k] = -1;
    vtx_idx_arr_refine[k] = -1;
  }
  for (int k = 0; k < dim[0] * dim[1] * dim[2]; k++) {
    grid_idx_arr[k] = -1;
  }
}
...
void Octree::construct_octree(char *rawiv_fname) {
  ...
  err_fp = 0;
  if (err_fp) {
    freadSafely((float *)minmax, sizeof(float), octcell_num * 2, err_fp);
    fclose(err_fp);
  } else {
...
int Octree::cell_comp_in(int oc_id, int level, float pt[12][3],
  ...
  for (e = 0; e < cubeedges[code][0]; e++) {
    ...
    switch (ei->dir) {
    case 0:
      interpRect3Dpts_x(i + ei->di, j + ei->dj, k + ei->dk, f1, f2, iso_val_in,
                        pt[e], norm[e], level);
      break;
    case 1:
      interpRect3Dpts_y(i + ei->di, j + ei->dj, k + ei->dk, f1, f2, iso_val_in,
                        pt[e], norm[e], level);
      break;
    case 2:
      interpRect3Dpts_z(i + ei->di, j + ei->dj, k + ei->dk, f1, f2, iso_val_in,
                        pt[e], norm[e], level);
      break;
    }
  }
...
void Octree::put_qef(int oc_id, double *sigma_ni_2, double *sigma_ni_2_pi,
  ...
  if (qef_array[oc_id] == NULL) {
    qef_array[oc_id] =
        (double *)mallocSafely(sizeof(double) * (3 + 3 + 3 + 3 + 1));
  }
...
void Octree::put_qef_in(int oc_id, double *sigma_ni_2, double *sigma_ni_2_pi,
  ...
  cout << "arand " << qef_array_in[oc_id] << " " << oc_id << endl;
...
float Octree::get_err_grad_test(int oc_id) {
  ...
  val_new[0] =
      getValue(xx * cell_size + cell_size / 2, yy * cell_size, zz * cell_size);
  ...
  val_new[3] =
      getValue(xx * cell_size, yy * cell_size, zz * cell_size + cell_size / 2);
  val_new[4] = getValue(xx * cell_size + cell_size / 2,
                        yy * cell_size + cell_size, zz * cell_size);
  val_new[5] = getValue(xx * cell_size + cell_size, yy * cell_size + cell_size,
                        zz * cell_size + cell_size / 2);
  val_new[6] = getValue(xx * cell_size + cell_size / 2,
                        yy * cell_size + cell_size, zz * cell_size + cell_size);
  val_new[7] = getValue(xx * cell_size, yy * cell_size + cell_size,
                        zz * cell_size + cell_size / 2);
  val_new[8] =
      getValue(xx * cell_size, yy * cell_size + cell_size / 2, zz * cell_size);
    ...
    while (prev_queue.Get(oc_id) >= 0) {
      ...
      if (minmax[oc_id].min > iso_val) {
        continue;
                 ...
                 (is_skipcell(oc_id) == 0 && level < oct_depth - 2) ||
...
int Octree::is_min_edge(int oc_id, int e_id, unsigned int *vtx, int &vtx_num,
  ...
  switch (e_id) {
  case 0:
    if (is_refined(x, y, z - 1, level) || is_refined(x, y - 1, z - 1, level) ||
  ...
  case 6:
    ...
    if (is_refined(x, y + 1, z, level) || is_refined(x, y + 1, z + 1, level) ||
    ...
    break;
  case 7:
    if (is_refined(x - 1, y, z, level) || is_refined(x - 1, y + 1, z, level) ||
        is_refined(x, y + 1, z, level)) {
      return 0;
    }
    temp_vtx[1] = min_vtx(x - 1, y, z, level, geofrm);
...
void Octree::eflag_clear() { memset(ebit, 0, octcell_num * 4 / 8); }
...
void Octree::func_val(Geoframe &geofrm) {
  ...
  float *func_vol;
  func_vol = (float *)mallocSafely(sizeof(float) * dim[0] * dim[1] * dim[2]);
  VolumeLoader *volumeLoader = new VolumeLoader();
  SimpleVolumeData *propData = volumeLoader->loadFile(prop_fname);
  ...
  int sum_5 = 0;
  int sum_1 = 0;
  for (i = 0; i < geofrm.getNumVerts(); i++) {
    ...
    for (j = 0; j < 8; j++) {
      val[j] = func_vol[vtx[j]];
    }
    geofrm.funcs[i][0] =
        (1 - dx) * (1 - dy) * (1 - dz) * val[0] +
        (1 - dx) * (1 - dy) * dz * val[3] + (1 - dx) * dy * (1 - dz) * val[4] +
        dx * (1 - dy) * (1 - dz) * val[1] + (1 - dx) * dy * dz * val[7] +
        dx * (1 - dy) * dz * val[2] + dx * dy * (1 - dz) * val[5] +
        dx * dy * dz * val[6];
...
void Octree::mesh_extract(Geoframe &geofrm, float err_tol) {
  ...
  int func = 1;
  BSplineCoeff =
      (float *)mallocSafely(sizeof(float) * dim[0] * dim[1] * dim[2]);
...
void Octree::quality_improve(Geoframe &geofrm, int improve_method) {
  quality_improve(geofrm, improve_method, 0.01f);
}
...
void Octree::getVertGrad(int i, int j, int k, float g[3]) {
  ...
  {
    GradientAtPoint(BSplineCoeff, (float)i, (float)j, (float)k, dim[0], dim[1],
                    dim[2], g);
    g[0] = g[0] / span[0];
    g[1] = g[1] / span[1];
    g[2] = g[2] / span[2];
...
void Octree::geometric_flow(Geoframe &geofrm) { geometric_flow(geofrm, 0.01f); }

// FIXME: arand, 6-7-11, passing delta_t through as a parameter
//                       but ONLY for tri meshes
void Octree::geometric_flow(Geoframe &geofrm, float delta_t) {
  switch (meshType) {
  case TRI: // triangular mesh
    geometric_flow_tri(geofrm, delta_t);
    break;
  case TET: // tetrahedral mesh
    geometric_flow_tet(geofrm);
    break;
  case QUAD: // quadrilateral  mesh
    geometric_flow_quad(geofrm);
    break;
  case HEX: // hex mesh
    geometric_flow_hex(geofrm);
    break;
  case TRI2: // triangular mesh
    geometric_flow_tri(geofrm);
    break;
  case TET2: // tetraheral mesh
    geometric_flow_tet(geofrm);
    break;
  }
}
...
void Octree::geometric_flow_tri(Geoframe &geofrm) {
  geometric_flow_tri(geofrm, 0.01f);
}

// quality improvement with geometric flow -- tri mesh
void Octree::geometric_flow_tri(Geoframe &geofrm, float delta_t) {
  int i, j, k, v0, v1, v2, itri, index, maxIndex;
  float mv0[3], mv1[3], mv2[3], p[3], sum_0, sum_1[3], area, t;
  int **neighbor;
  //     FILE* output;
  ...
  maxIndex = 100;
  neighbor = 0;
  neighbor = new int *[geofrm.getNumVerts()];
  for (i = 0; i < geofrm.getNumVerts(); i++) {
    neighbor[i] = 0;
  }
  for (i = 0; i < geofrm.getNumVerts(); i++) {
  ...
  // fprintf(output, "%d %d\n", geofrm.getNumVerts(), geofrm.getNumTris());
  for (index = 0; index < maxIndex; index++) {
    ...
    for (i = 0; i < geofrm.getNumVerts(); i++) {
      ...
      for (j = 0; j < 10; j++) {
        ...
        // if(isnan(area)){cout<<"ERROR, stop" <<endl;}
        sum_0 += area;
        for (k = 0; k < 3; k++) {
...
void Octree::geometric_flow_tet(Geoframe &geofrm) {
  ...
  for (i = 0; i < geofrm.getNumTris() / 4; i++) {
  ...
  // fprintf(output, "%d %d\n", geofrm.getNumVerts(), geofrm.getNumTris()/4);
  for (index = 0; index < maxIndex; index++) {
    // fprintf(output, "%d ", index);
    if (index % 20 == 0 && index > 0) {
      // fprintf(output, "\n");
    ...
    for (i = 0; i < geofrm.getNumVerts(); i++) {
      ...
      } else // interior vertex
      {
        ...
        for (j = 0; j < 3; j++) {
          geofrm.verts[i][j] += delta_t * (p[j] - geofrm.verts[i][j]);
          // volume center
        }
      }
    }
...
void Octree::geometric_flow_quad(Geoframe &geofrm) {
  ...
  for (i = 0; i < geofrm.getNumQuads(); i++) {
    v0 = geofrm.quads[i][0];
    v1 = geofrm.quads[i][1];
    v2 = geofrm.quads[i][2];
    v3 = geofrm.quads[i][3];
  ...
  // fprintf(output, "%d %d\n", geofrm.getNumVerts(), geofrm.getNumQuads());
  for (index = 0; index < maxIndex; index++) {
    // fprintf(output, "%d ", index);
    if (index % 20 == 0 && index > 0) {
      // fprintf(output, "\n");
...
void Octree::geometric_flow_hex(Geoframe &geofrm) {
  ...
  for (i = 0; i < geofrm.getNumQuads() / 6; i++) {
  ...
  // fprintf(output, "%d %d\n", geofrm.getNumVerts(), geofrm.getNumQuads()/6);
  for (index = 0; index < maxIndex; index++) {
    // fprintf(output, "%d ", index);
    if (index % 20 == 0 && index > 0) {
      // fprintf(output, "\n");
...
float Octree::area_tri(float v0[3], float v1[3], float v2[3]) {
  ...
  c = 0.0;
  for (i = 0; i < 3; i++) {
    a += (double)(v1[i] - v0[i]) * (v1[i] - v0[i]);
    b += (double)(v2[i] - v1[i]) * (v2[i] - v1[i]);
    c += (double)(v0[i] - v2[i]) * (v0[i] - v2[i]);
  }
  a = (double)sqrt(a);
  b = (double)sqrt(b);
  c = (double)sqrt(c);
...
void Octree::edge_contraction(Geoframe &geofrm) {
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    if (fscanf(input, "%d %d\n", &nv, &ntet) != 2)
      error("Error reading number of vertices and quads!");
    ...
    for (i = 0; i < ntet; i++) {
      ...
      if (aspect > new_edge_ratio && nrep == 0) {
        ...
        //                                          bound_sign[v1],
        //                                          bound_sign[v2],
        //                                          bound_sign[v3], sqrt(e_min),
        //                                          sqrt(e_max), aspect,
        //                                          id_min);
        // fprintf(output, "%f %f %f %f %f %f\n", sqrt(e[0]), sqrt(e[1]),
        // sqrt(e[2]), sqrt(e[3]), sqrt(e[4]), sqrt(e[5]));
        if (id_min == 0) {
          vv0 = v0;
          vv1 = v1;
        } else if (id_min == 1) {
  ...
  input = fopen("input.raw", "r");
  if (fscanf(input, "%d %d\n", &nv, &ntet) != 2)
    error("Could not read the number of vertices and quads!");
  geofrm.setNumVerts(nv);
  geofrm.setNumTris(ntet * 4);
  for (i = 0; i < nv; i++) {
    if (fscanf(input, "%f %f %f %f %f %f %d\n", &vx, &vy, &vz, &nx, &ny, &nz,
               &b_sign) != 7)
      error("Error reading vetex!");
    geofrm.verts[i][0] = vx;
    geofrm.verts[i][1] = vy;
    geofrm.verts[i][2] = vz;
...
void Octree::smoothing_joeliu_volume(Geoframe &geofrm, int sign) {
  ...
  // FILE* output;
  // output = fopen("qimprove_temp", "w");
  nv = geofrm.getNumVerts();
  ntet = geofrm.getNumTris() / 4;
  ...
  // geofrm.bound_sign[av1], geofrm.bound_sign[av2],
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    i = a_vert;
    // fprintf(output, "average %d %f %f %f %d\n", i, geofrm.verts[i][0],
    // geofrm.verts[i][1], geofrm.verts[i][2], geofrm.bound_sign[i]);
    sum_0 = 0.0;
    sum_1 = 0.0;
    sum_2 = 0.0;
    for (j = 0; j < nlist; j++) {
...
void Octree::optimization(Geoframe &geofrm) {
  ...
  maxIndex = 20;
  conditionArr = 0;
  conditionArr = new float[geofrm.getNumVerts()];
  jacobianArr = 0;
  jacobianArr = new float[geofrm.getNumVerts()];
  oddyArr = 0;
  oddyArr = new float[geofrm.getNumVerts()];
  for (i = 0; i < geofrm.getNumVerts(); i++) {
    conditionArr[i] = 1.0f;
    jacobianArr[i] = 1.0f;
    oddyArr[i] = 0.0f;
  ...
  //                       cond_min, temp0/num, cond_max, oddy_min, temp1/num,
  //                       oddy_max, a_vert);
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    // jaco_max,
    //                             oddy_min, oddy_max, cond_min, cond_max,
...
void Octree::quad_adaptive_method1(Geoframe &geofrm, int *oc_id, float err_tol,
                                   unsigned int *vtx) {
...
void Octree::quad_adaptive_method2(Geoframe &geofrm, int *oc_id, float err_tol,
                                   unsigned int *vtx) {
...
void Octree::quad_adaptive_method3(Geoframe &geofrm, int *oc_id, float err_tol,
                                   unsigned int *vtx, int flag_method) {
  int num_id, i, j, vtx_num, xx, yy, zz, level, cell_size, vtx_new_num;
  unsigned int temp_vtx[4], vtx_new[12];
  float x, y, z;
...
void Octree::quad_adaptive_method5(Geoframe &geofrm, int *oc_id, float err_tol,
                                   unsigned int *vtx) {
  int num_id, i, j, vtx_num, xx, yy, zz, level, cell_size, vtx_new_num;
  unsigned int temp_vtx[4], vtx_new[12];
  float x, y, z;
