...
SimpleVolumeData::SimpleVolumeData(unsigned int dim1, unsigned int dim2,
                                   unsigned int dim3) {
  setDefaults();
  m_Dims[0] = dim1;
  m_Dims[1] = dim2;
...
void SimpleVolumeData::parallelAdjustDimsUnmerged() {
  ...
  m_Dims[3] =
      (m_Dims[2] - 1) / size + 1 + (((m_Dims[2] - 1) % size > rank) ? 1 : 0);
  // calculate the offset for non-root slabs
  if (rank != 0)
    for (int i = 0; i < rank; i++) {
      m_ZOff += (m_Dims[2] - 1) / size + 1 +
                (((m_Dims[2] - 1) % size > i) ? 1 : 0) - 1;
    }
  else {
    m_ZOff = 0;
  }
...
void SimpleVolumeData::makeVariablesBigEndian() {
  if (m_Data && isLittleEndian()) {
    for (unsigned int i = 0; i < m_NumberOfVariables; i++) {
      ...
      case USHORT:
        swapByteOrder((unsigned short *)m_Data[i],
                      m_Dims[0] * m_Dims[1] * m_Dims[3]);
        break;
      case ULONG:
        swapByteOrder((unsigned int *)m_Data[i],
                      m_Dims[0] * m_Dims[1] * m_Dims[3]);
        break;
      case FLOAT:
...
char **SimpleVolumeData::getVariableNames() { return m_VarNames; }

double SimpleVolumeData::getDistanceOfVoxel(int i, int j, int k, int width,
                                            int height, int depth) {
  return (sqrt(((i)-width / 2) * ((i)-width / 2) +
               ((k)-depth / 2) * ((k)-depth / 2)));
}

SimpleVolumeData *SimpleVolumeData::createDepthColoredVolume(double *colorMap,
                                                             int colorMapSize) {
  if (!colorMap || (colorMapSize < 1)) {
    return 0;
  }
  if (getNumberOfVariables() < 1) {
    return 0;
  }
  int width = getWidth();
    ...
    for (i = 0; i < width; i++) {
      for (j = 0; j < height; j++) {
        for (k = 0; k < depth; k++) {
          ...
          int index = (int)((dist - minDistance) /
                            (maxDistance - minDistance + 1) * colorMapSize);
    ...
    }
    {
      ...
      for (i = 0; i < colorMapSize; i++) {
        printf("%d = [%lf %lf %lf %lf]\n", i, colorMap[i * 4 + 0],
               colorMap[i * 4 + 1], colorMap[i * 4 + 2], colorMap[i * 4 + 3]);
      }
    }
    // depth color the new volume
    c = 0;
...
bool SimpleVolumeData::getMinMax(unsigned char *data, int width, int height,
                                 int depth, unsigned char *minVal,
                                 unsigned char *maxVal) {
  if (!data || !minVal || !maxVal || (width < 1) || (height < 1) ||
      (depth < 1)) {
    return false;
  }
...
bool SimpleVolumeData::getMinMax(unsigned short *data, int width, int height,
                                 int depth, unsigned short *minVal,
...
bool SimpleVolumeData::getMinMax(unsigned long *data, int width, int height,
                                 int depth, unsigned long *minVal,
...
bool SimpleVolumeData::getMinMax(float *data, int width, int height, int depth,
...
bool SimpleVolumeData::getMinMax(double *data, int width, int height, int depth,
...
bool SimpleVolumeData::getUnsignedCharData(void *data1, int type,
...
/*  The input data is copied as unsigned char into the output array.      */
bool SimpleVolumeData::getNormalizedUnsignedCharData(void *data1, int type,
                                                     unsigned char *data,
  ...
  case SimpleVolumeData::UCHAR: {
    ...
    if (!SimpleVolumeData::getMinMax(inputData, width, height, depth, &minVal,
                                     &maxVal)) {
      return false;
    }
    for (j = 0; j < size; j++) {
...
bool SimpleVolumeData::getUnsignedCharData(void *data1, int type1, void *data2,
                                           int type2, void *data3, int type3,
                                           void *data4, int type4,
                                           unsigned char *data, int width,
                                           int height, int depth) {
...
bool SimpleVolumeData::getMinMax(unsigned int variable, double *minVal,
                                 double *maxVal) {
...
double SimpleVolumeData::getValueAt(unsigned int variable, double x, double y,
                                    double z) {
  if (!inVolume(x, y, z)) {
    return 0;
  }
...
bool SimpleVolumeData::inVolume(double x, double y, double z) {
  if (x < m_Min[0] || x > m_Max[0] || y < m_Min[1] || y > m_Max[1] ||
      z < m_Min[2] || z > m_Max[2]) {
    return false;
  }
  return true;
...
bool SimpleVolumeData::setValueAt(unsigned int variable, Q_ULLONG index,
                                  double val) {
  int w = getWidth();
  int h = getHeight();
  int d = getDepth();
...
bool SimpleVolumeData::addVolume(SimpleVolumeData *sData, double scale,
                                 double sum) {
  if (!sData) {
    return false;
  }
  ...
  if (getNumberOfVariables() != sData->getNumberOfVariables()) {
    return false;
  }
  unsigned int v;
  for (v = 0; v < getNumberOfVariables(); v++) {
    ...
    for (c = 0; c < w1 * h1 * d1; c++) {
      if (!setValueAt(
              v, c, getValueAt(v, c) + sData->getValueAt(v, c) * scale + sum)) {
        return false;
      }
    }
  }
...
bool SimpleVolumeData::getFloatData(float **fVolume) {
  if (getNumberOfVariables() < 4) {
    return getFloatData(fVolume, 0);
  }
  return getFloatData(fVolume, 3);
}

bool SimpleVolumeData::getFloatData(float **fVolume, unsigned int variable) {
  if (!fVolume) {
    return false;
  }
  ...
  data = getData(variable);
  type = getType(variable);
  if (!getMinMax(variable, &minVal, &maxVal)) {
    return false;
  }
  int dim[3];
  dim[0] = getWidth();
  ...
  {
    int i;
    (*fVolume) = new float[dim[0] * dim[1] * dim[2]];
    switch (type) {
    case SimpleVolumeData::UCHAR: {
