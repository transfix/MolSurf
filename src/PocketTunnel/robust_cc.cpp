...
// computation these next functions are used to get the voronoi point if points
// are coplanar. Coplanarity makes CGAL very sad. :-(
...
Point cc_tr_3(const Point &a, const Point &b, const Point &c) {
  ...
  // three points.
  cc =
      CGAL::ORIGIN + ((1. / 3.) * Vector(a[0] + b[0] + c[0], a[1] + b[1] + c[1],
                                         a[2] + b[2] + c[2]));
  return cc;
}

// This function computes the squared circumradius of a triangle in 3D.
// It calls cc_tr_3 to get the circumcenter and then computes the radius.
