...
void convert_pocket_tunnel_to_rawc_geometry(
  ...
  for (; tun_void_cnt < otc;) {
    ...
    if (i >= (int)sorted_cluster_index_vector.size()) {
      cerr << endl
           << "The number of tunnel/voids are less than " << otc << endl;
      break;
    }
    if (tun_void_cnt >= 100) {
  ...
  for (FVI vit = triang.finite_vertices_begin();
    ...
    (*PTV)->m_TriVertColorsTransparent[4 * vit->id] =
        vertex_colors[3 * vit->id];
    (*PTV)->m_TriVertColorsTransparent[4 * vit->id + 1] =
        vertex_colors[3 * vit->id + 1];
    (*PTV)->m_TriVertColorsTransparent[4 * vit->id + 2] =
        vertex_colors[3 * vit->id + 2];
  ...
  /* for debug
          ...
          fout << "COFF" << endl;
          fout << (*PTV)->m_NumTriVerts << " " << (*PTV)->m_NumTris << " 0" <<
     endl; for(i = 0; i < (*PTV)->m_NumTriVerts; i ++)
          {
                  fout << (*PTV)->m_TriVerts[3*i] << " "
                           ...
                           << (*PTV)->m_TriVertColorsTransparent[4*i+2] << " 1"
     << endl;
          }
...
void convert_curated_surf_to_raw_geometry(
  ...
  for (FVI vit = triang.finite_vertices_begin();
       vit != triang.finite_vertices_end(); vit++) {
    (*curated_surf)->m_TriVerts[3 * vit->id] =
        CGAL::to_double(vit->point().x());
    (*curated_surf)->m_TriVerts[3 * vit->id + 1] =
        CGAL::to_double(vit->point().y());
    (*curated_surf)->m_TriVerts[3 * vit->id + 2] =
  ...
  /* //for debug
      ofstream fout;
          ...
          fout << "OFF" << endl;
          fout << (*curated_surf)->m_NumTriVerts << " " <<
     (*curated_surf)->m_NumTris << " 0" << endl; for(int i = 0; i <
     (*curated_surf)->m_NumTriVerts; i ++) fout <<
     (*curated_surf)->m_TriVerts[3*i] << " "
