                           ...
                           const int &opc, const int &otc) // parameters.
{
  ...
  for (int i = 0; i < molsurf->m_NumTriVerts * 3;) {
    float x = molsurf->m_TriVerts[i++], y = molsurf->m_TriVerts[i++],
  ...
  convert_pocket_tunnel_to_rawc_geometry(PTV, triang, cluster_set,
                                         sorted_smax_index_vector, opc, otc);
  convert_curated_surf_to_raw_geometry(curated_surf, triang, cluster_set,
                                       sorted_smax_index_vector, opc, otc);
  return 0;
}
} // namespace PocketTunnel
