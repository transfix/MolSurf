    ...
    for (int id = 0; id < 4; ++id) {
      ...
      if (triang.is_infinite(cit->neighbor(id))) {
        ...
        for (int i = 1; i <= 3; ++i) {
          ...
          new_normal = new_normal / CGAL::sqrt(new_normal.x() * new_normal.x() +
...
void mark_flat_vertices(Triangulation &triang, double ratio, double cocone_phi,
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    if (triang.is_infinite(ch->neighbor(id))) {
      facet_normal = CGAL::cross_product(ch->vertex((id + 1) % 4)->point() -
      ...
      for (int j = 1; j < 4; ++j) {
        ...
        double cos2 =
            (triang.is_infinite(ch->neighbor(id)))
                ? 1.0
                : cosine(ch->neighbor(id)->voronoi() - w->point(), w->normal());
        if (cos1 > cos2) {
          swap(cos1, cos2);
        }
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    for (int i = 1; i < 4; ++i) {
      ...
      double cos2 =
          (triang.is_infinite(ch->neighbor(id)))
              ? 1.0
              : cosine(ch->neighbor(id)->voronoi() - vh->point(), vh->normal());
      if (cos1 > cos2) {
        swap(cos1, cos2);
      }
      // Test for disjoint intervals [cos1, cos2] and [min_cos, max_cos].
...
void safety_check(Triangulation &triang, double sharp_phi) {
  ...
  for (FEI eit = triang.finite_edges_begin(); eit != triang.finite_edges_end();
       ++eit) {
...
void pruning(Triangulation &triang, double sharp_phi) {
  ...
  for (FEI eit = triang.finite_edges_begin(); eit != triang.finite_edges_end();
    ...
    Vertex_handle v = ch->vertex((*eit).second);
    Vertex_handle w = ch->vertex((*eit).third);
    // Process only edges for which both endpoints have an incident umbrella.
...
// reconstructs the surface of an unorganized set of points. modified cocone
...
bool identify_vertices_with_dirty_umbrella(Triangulation &triang) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    // umbrellas then it should be marked as bad. This loop checks if any of the
    // vertex of the surface can be bad
    for (int k = 1; k < 4; k++) {
...
void propagate_in_out_marking(
  ...
  while (!v_stack.empty()) {
    ...
    // Now move around the outside umbrella of vertex untill it hits a cocone
    // facet.
...
void grow_boundary(Triangulation &triang, vector<Facet> &bdy_stack,
  ...
  while (!bdy_stack.empty()) {
    ...
    // here I have to postponne walk to see if any unvisited good (not isolated)
    // point has been reached so that I can mark the in-out for that point now
...
void create_surface(Triangulation &triang, bool postponne_walk_to_do_io_marking,
  ...
  for (ACI cit = triang.all_cells_begin(); cit != triang.all_cells_end();
    ...
    // The cell is not suitable to start. So continue to get a new cell.
    if (!start) {
      continue;
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      start_cell->neighbor(i)->bdy[start_cell->neighbor(i)->index(start_cell)] =
          true;
...
// identified. Then it performs two operations called marking and peeling
