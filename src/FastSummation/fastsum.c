...
double binom(int n, int m) { return fak(n) / fak(m) / fak(n - m); }
 ...
 *  (used in 1D)
 */
double _Complex regkern1(kernel k, double xx, int p, const double *param,
                         double a, double b) {
  int r;
  double _Complex sum = 0.0;
...
double _Complex regkern2(kernel k, double xx, int p, const double *param,
                         double a, double b) {
  int r;
  double _Complex sum = 0.0;
 ...
 */
double _Complex regkern3(kernel k, double xx, int p, const double *param,
                         double a, double b) {
  int r;
  double _Complex sum = 0.0;
...
double _Complex SearchTree(const int d, const int t, const double *x,
                           const double _Complex *alpha, const double *xmin,
                           const double *xmax, const int N, const kernel k,
                           const double *param, const int Ad,
  ...
  if (Min > Median)
    ...
    if (E == d) {
      if (d == 1) {
        ...
        for (l = 0; l < d; l++)
          r += (xmin[l] + a - x[m * d + l]) *
               (xmin[l] + a - x[m * d + l]); /* remember: xmin+a = y */
        r = sqrt(r);
      }
      if (fabs(r) < a) {
        ...
        if (d == 1) {
          if (flags & EXACT_NEARFIELD)
            result -= alpha[m] * regkern1(k, r, p, param, a,
                                          1.0 / 16.0); /* exact value (in 1D) */
          else
            result -= alpha[m] *
                      kubintkern1(r, Add, Ad, a); /* spline approximation */
        } else {
          if (flags & EXACT_NEARFIELD)
            result -= alpha[m] * regkern(k, r, p, param, a,
                                         1.0 / 16.0); /* exact value (in dD)  */
          else
            result -=
                alpha[m] * kubintkern(r, Add, Ad, a); /* spline approximation */
        }
      }
    }
    result += SearchTree(d, (t + 1) % d, x + (m + 1) * d, alpha + (m + 1), xmin,
                         xmax, N - m - 1, k, param, Ad, Add, p, flags) +
              SearchTree(d, (t + 1) % d, x, alpha, xmin, xmax, m, k, param, Ad,
                         Add, p, flags);
    return result;
  }
}
...
void fastsum_init_guru(fastsum_plan *ths, int d, int N_total, int M_total,
  ...
  ths->alpha =
      (double _Complex *)nfft_malloc(N_total * (sizeof(double _Complex)));
  ...
  if (!(ths->flags & EXACT_NEARFIELD)) {
    if (ths->d == 1) {
      ...
      ths->Add = (double _Complex *)nfft_malloc((ths->Ad + 5) *
                                                (sizeof(double _Complex)));
    } else {
      ths->Ad = 2 * (ths->p) * (ths->p);
      ths->Add = (double _Complex *)nfft_malloc((ths->Ad + 3) *
                                                (sizeof(double _Complex)));
    }
  }
  ...
  ths->fft_plan =
      fftw_plan_dft(d, N, ths->b, ths->b, FFTW_FORWARD, FFTW_ESTIMATE);
}
...
void fastsum_precompute(fastsum_plan *ths) {
  ...
  for (j = 0; j < n_total; j++) {
    if (ths->d == 1)
      ths->b[j] = regkern1(ths->k, (double)j / (ths->n) - 0.5, ths->p,
                           ths->kernel_param, ths->eps_I, ths->eps_B) /
                  n_total;
    else {
      k = j;
...
void fastsum_trafo(fastsum_plan *ths) {
  ...
  for (j = 0; j < ths->M_total; j++) {
    ...
    ths->f[j] =
        ths->mv2.f[j] + SearchTree(ths->d, 0, ths->x, ths->alpha, ymin, ymax,
                                   ths->N_total, ths->k, ths->kernel_param,
                                   ths->Ad, ths->Add, ths->p, ths->flags);
    /* ths->f[j] = ths->mv2.f[j]; */
    /* ths->f[j] = SearchTree(ths->d,0, ths->x, ths->alpha, ymin, ymax,
     * ths->N_total, ths->k, ths->kernel_param, ths->Ad, ths->Add, ths->p,
     * ths->flags); */
  }
}
/* \} */
