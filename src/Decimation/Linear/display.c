...
static int face[12][6] = {
    {1, 2, 3, 9, 4, 8},   {2, 3, 0, 9, 5, 11}, {3, 0, 1, 11, 6, 10},
    {0, 1, 2, 10, 7, 8},  {5, 6, 7, 8, 0, 9},  {6, 7, 4, 11, 1, 9},
    {7, 4, 5, 10, 2, 11}, {4, 5, 6, 10, 3, 8}, {7, 10, 3, 0, 9, 4},
    {5, 11, 1, 4, 8, 0},  {2, 11, 6, 3, 8, 7}, {1, 9, 5, 6, 10, 2}};
...
----------------------------------------------------------------------------*/
void PartialxyBernstein2D(weight, m, n, a1, a2, b1, b2, weight1,
                          dir) double weight[]; /* the coefficients of Bernstein
...
int dir;               /* dir = 0--x, 1--y                        */
{
  ...
  if (dir == 0) {
    ...
    for (i = 0; i < m; i++)
      for (j = 0; j < n1; j++)
        weight1[n1 * i + j] =
            x * (weight[n1 * (i + 1) + j] - weight[n1 * i + j]);
    return;
  }
  if (dir == 1) {
    ...
    for (i = 0; i < m1; i++)
      for (j = 0; j < n; j++)
        weight1[n * i + j] =
            y * (weight[n1 * i + (j + 1)] - weight[n1 * i + j]);
    return;
  }
}
...
----------------------------------------------------------------------------*/
void
    PartialBernstein(weight, m, n, q, a1, a2, b1, b2, c1, c2, dxweight,
                     dyweight,
                     dzweight) double weight[]; /* the coefficients of Bernstein
                                                   polynomial degree m,n,q */
...
PartialxyzBernstein -- compute partial derivative about x,or y or z of  a 3D
                     Bernstein polynomial over a cube of degree m,n,q
----------------------------------------------------------------------------*/
void PartialxyzBernstein(weight, m, n, q, a1, a2, b1, b2, c1, c2, weight1,
                         dir) double weight[]; /* the coefficients of Bernstein
                                                  polynomial degree m,n,q */
int m, n, q;                   /* the degree of the  Bernstein polynomial */
double a1, a2, b1, b2, c1, c2; /* a1 <= x <= a2; b1 <= y <= b2;
...
int dir; /* dir = 0--x, 1--y, 2--z                  */
{
  ...
  if (dir == 0) {
    ...
    for (i = 0; i < m; i++)
      for (j = 0; j < n1; j++)
        for (k = 0; k < q1; k++)
          weight1[n1 * q1 * i + q1 * j + k] =
              x * (weight[n1 * q1 * (i + 1) + q1 * j + k] -
  ...
  if (dir == 1) {
    ...
    for (i = 0; i < m1; i++)
      for (j = 0; j < n; j++)
        for (k = 0; k < q1; k++)
          weight1[n * q1 * i + q1 * j + k] =
              y * (weight[n1 * q1 * i + q1 * (j + 1) + k] -
                   weight[n1 * q1 * i + q1 * j + k]);
    return;
  }
  if (dir == 2) {
    ...
    for (i = 0; i < m1; i++)
      for (j = 0; j < n1; j++)
        for (k = 0; k < q; k++)
          weight1[n1 * q * i + q * j + k] =
              z * (weight[n1 * q1 * i + q1 * j + k + 1] -
                   weight[n1 * q1 * i + q1 * j + k]);
  }
}
/*---------------------------------------------------------------------------
...
----------------------------------------------------------------------------*/
void Intersection12Edge(
    weight, m, a1, a2, b1, b2, c1, c2, intersect, numberin,
    flag) double weight[];     /* the coefficients of Bernstein polynomial
...
int *flag;                     /* condition a,b,c in paper are fail       */
{
  ...
  */
  ...
  Intersection12Edge1(weight1, m, a1, a2, b1, b2, c1, c2, intersect, numberin,
                      flag);
  ...
  *numberin = PointsNum(intersect);
  ...
  if (intersect[0][0] + intersect[8][0] + intersect[4][0] + intersect[9][0] >
      2.0) {
    *flag = 0;
    return;
  }
  if (intersect[2][0] + intersect[6][0] + intersect[10][0] + intersect[11][0] >
      2.0) {
    *flag = 0;
    return;
  }
  if (intersect[0][0] + intersect[1][0] + intersect[2][0] + intersect[3][0] >
      2.0) {
    *flag = 0;
    return;
  }
  if (intersect[4][0] + intersect[5][0] + intersect[6][0] + intersect[7][0] >
      2.0) {
    *flag = 0;
    return;
  }
  if (intersect[1][0] + intersect[5][0] + intersect[9][0] + intersect[11][0] >
      2.0) {
    *flag = 0;
    return;
  }
  if (intersect[3][0] + intersect[7][0] + intersect[8][0] + intersect[10][0] >
      2.0) {
    *flag = 0;
    return;
  }
...
----------------------------------------------------------------------------*/
void Intersection4Edge(
    weight, m, a1, a2, b1, b2, intersect, numberin,
    flag) double weight[]; /* the coefficients of Bernstein polynomial
                              degree (2,2,2) or  (3,3,3)              */
int m;                     /* degree m = 2 or m = 3                   */
double a1, a2, b1, b2;     /* a1 <= x <= a2; b1 <= y <= b2;           */
...
----------------------------------------------------------------------------*/
void FindRoots23Edges(edges, i, intersect, flag, degree) double edges[12][4];
int i, *flag;
...
----------------------------------------------------------------------------*/
void FindRoots2D123Edges(edges, i, intersect, flag, degree) double edges[4][4];
int i, *flag;
double intersect[4][2];
int degree;
{
...
----------------------------------------------------------------------------*/
void DisplayBerns123(
    weight, a1, a2, b1, b2, c1, c2, m, epsilon, scaler, threshold,
    polygons) double weight[]; /* the coefficients of Bernstein polynomial
...
Polygons *polygons;            /* polygons chain  created                 */
{
  ...
  m1 = (m + 1) * (m + 1) * (m + 1);
  for (i = 0; i < m1; i++)
    weight2[i] = weight[i] - threshold + 0.000314159261828;
  ;
  cube[0] = a1;
  cube[1] = a2;
  cube[2] = b1;
  ...
  cube[4] = c1;
  cube[5] = c2;
  PartialBernstein(weight2, m, m, m, a1, a2, b1, b2, c1, c2, dxweight, dyweight,
                   dzweight);
  DisplayBerns123in(weight2, dxweight, dyweight, dzweight, a1, a2, b1, b2, c1,
                    c2, m, epsilon, cube, scaler, polygons, &endpoly);
}
...
PolyLink polygons;             /* polygons chain  created                 */
{
  ...
  PartialBernstein(weight2, m, m, m, a1, a2, b1, b2, c1, c2, dxweight, dyweight,
                   dzweight);
  DisplayBerns123in(weight2, dxweight, dyweight, dzweight, a1, a2, b1, b2, c1,
                    c2, m, epsilon, cube, scaler, &polygons->head,
                    &polygons->tail);
}
...
DisplayBerns123in -- display a implicitly defined surface befined by
                     ...
                     subdivision
----------------------------------------------------------------------------*/
void DisplayBerns123in(
    weight, dxweight, dyweight, dzweight, a1, a2, b1, b2, c1, c2, m, epsilon,
    cube, scaler, polygons,
    endpoly) double weight[]; /* the coefficients of Bernstein polynomial
                                 degree (1,1,1) or (2,2,2) or  (3,3,3)   */
double dxweight[], dyweight[], dzweight[];
/* the coefficients of derivatives about x,
   y and z                                 */
...
Polygons *polygons, *endpoly;
{
  ...
  */
  ...
  if ((a2 - a1 >= b2 - b1) && (a2 - a1 >= c2 - c1)) {
    ...
    aa2 = (a1 + a2) / 2.0;
    a1a1 = aa2;
  } else if ((b2 - b1 >= a2 - a1) && (b2 - b1 >= c2 - c1)) {
    dir = 2;
    bb2 = (b1 + b2) / 2.0;
    b1b1 = bb2;
  } else if ((c2 - c1 >= a2 - a1) && (c2 - c1 >= b2 - b1)) {
    dir = 3;
    cc2 = (c1 + c2) / 2.0;
    c1c1 = cc2;
  }
  ...
  if ((a2 - a1 > epsilon) || (b2 - b1 > epsilon) || (c2 - c1 > epsilon)) {
    ...
    DisplayBerns123in(weight2, dxweight, dyweight, dzweight, a1a1, a1a2, b1b1,
                      b1b2, c1c1, c1c2, m, epsilon, cube, scaler, polygons,
                      endpoly);
  } else {
    Intersection12Edge(weight, m, a1, a2, b1, b2, c1, c2, intersect, &numberin,
                       &flag);
    /*
    ...
    */
    if (flag == 0) {
      Partition(weight, dir, m, 0.5, weight1, weight2);
      DisplayBerns123in(weight1, dxweight, dyweight, dzweight, aa1, aa2, bb1,
                        bb2, cc1, cc2, m, epsilon, cube, scaler, polygons,
                        endpoly);
      DisplayBerns123in(weight2, dxweight, dyweight, dzweight, a1a1, a1a2, b1b1,
                        b1b2, c1c1, c1c2, m, epsilon, cube, scaler, polygons,
                        endpoly);
    ...
    if (flag == 1 && numberin >= 3) {
      DisplayPolygons(intersect, dxweight, dyweight, dzweight, m, a1, a2, b1,
                      b2, c1, c2, cube, scaler, polygons, endpoly);
    }
  }
}
...
----------------------------------------------------------------------------*/
void DisplayPolygons(intersect, dxweight, dyweight, dzweight, m, a1, a2, b1, b2,
                     c1, c2, cube, scaler, polygons,
                     endpoly) double intersect[12][2];
double dxweight[], dyweight[], dzweight[];
int m;
double a1, a2, b1, b2, c1, c2; /* a1 <= x <= a2; b1 <= y <= b2;
                                  c1 <= z <= c2                           */
  ...
  */
  ...
  while (start != run) {
    ...
    */
    ...
    x = 0.0;
    for (i = 0; i < 3; i++)
      x = x + normals[sum][i] * normals[sum][i];
    x = sqrt(x);
    for (i = 0; i < 3; i++)
      normals[sum][i] = normals[sum][i] / x;
    sum = sum + 1;
  }
  ...
  if (PointsNum(intersect) >= 3)
    DisplayPolygons(intersect, dxweight, dyweight, dzweight, m, a1, a2, b1, b2,
                    c1, c2, cube, scaler, polygons, endpoly);
}
/*---------------------------------------------------------------------------
DisplayPolygons2D -- display polygons
----------------------------------------------------------------------------*/
void DisplayPolygons2D(intersect, m, a1, a2, b1, b2, cube, zvalue, scaler,
                       polygons, endpoly) double intersect[4][2];
int m;
double a1, a2, b1, b2; /* a1 <= x <= a2; b1 <= y <= b2;            */
...
Polygons *polygons, *endpoly;
{
  ...
  */
  ...
  start = -1;
  for (i = 0; i < 4; i++) {
    if (intersect[i][0] == 1.0 && start == -1)
      start = i;
    if (intersect[i][0] == 1.0 && start > -1)
      end = i;
  }
  if (start == 0) {
    x = intersect[start][1] * (a2 - a1) + a1;
    y = b1;
    z = zvalue;
  }
  if (start == 1) {
    x = a2;
    y = intersect[start][1] * (b2 - b1) + b1;
    z = zvalue;
  }
  if (start == 2) {
    x = intersect[start][1] * (a2 - a1) + a1;
  ...
  if (end == 0) {
    x1 = intersect[end][1] * (a2 - a1) + a1;
    y1 = b1;
    z1 = zvalue;
  }
  if (end == 1) {
    x1 = a2;
    y1 = intersect[end][1] * (b2 - b1) + b1;
    z1 = zvalue;
  }
  if (end == 2) {
    x1 = intersect[end][1] * (a2 - a1) + a1;
...
----------------------------------------------------------------------------*/
void DisplayBerns2D123(
    weight, a1, a2, b1, b2, m, epsilon, zz, scaler, threshold,
    polygons) double weight[]; /* the coefficients of Bernstein polynomial
...
----------------------------------------------------------------------------*/
void DisplayBerns2D123in(
    weight, a1, a2, b1, b2, m, epsilon, cube, zvalue, scaler, polygons,
    endpoly) double weight[]; /* the coefficients of Bernstein polynomial
                                 degree  (1,1) or  (2,2) or  (3,3)       */
double a1, a2, b1, b2;        /* a1 <= x <= a2; b1 <= y <= b2;           */
int m;                        /* degree m = 1 or m = 2, or m = 3         */
...
Polygons *polygons, *endpoly;
{
  ...
  if ((a2 - a1 > epsilon) || (b2 - b1 > epsilon)) {
    ...
    DisplayBerns2D123in(weight2, a1a1, a1a2, b1b1, b1b2, m, epsilon, cube,
                        zvalue, scaler, polygons, endpoly);
  } else {
...
----------------------------------------------------------------------------*/
void GotPoint(intersect, a1, a2, b1, b2, c1, c2, i, point, point1,
              scaler) double intersect[12][2];
...
double point[], point1[], scaler[];
{
  ...
  if (i == 0) {
    x = intersect[i][1] * (a2 - a1) + a1;
    y = b1;
    z = c1;
  }
  if (i == 1) {
    x = a2;
    y = intersect[i][1] * (b2 - b1) + b1;
    z = c1;
  }
  if (i == 2) {
    x = intersect[i][1] * (a2 - a1) + a1;
...
----------------------------------------------------------------------------*/
void Partition(weight, dir, m, c, weight1, weight2) double weight[], weight1[],
    weight2[];
int dir, m;
double c;
{
    ...
    for (j = 0; j < m1; j++)
      for (k = 0; k < m1; k++) {
        if (dir == 1) {
          ...
          for (l = 0; l <= i; l++) {
            sum =
                sum + weight[m2 * l + j * m1 + k] * BernsteinBaseValue(i, l, c);
          }
        ...
        if (dir == 2) {
          ...
          for (l = 0; l <= j; l++) {
            sum =
                sum + weight[m2 * i + l * m1 + k] * BernsteinBaseValue(j, l, c);
          }
          weight1[m2 * i + j * m1 + k] = sum;
          sum = 0.0;
          for (l = 0; l <= m - j; l++) {
        ...
        if (dir == 3) {
          ...
          for (l = 0; l <= k; l++) {
            sum =
                sum + weight[m2 * i + j * m1 + l] * BernsteinBaseValue(k, l, c);
          }
          weight1[m2 * i + j * m1 + k] = sum;
          sum = 0.0;
          for (l = 0; l <= m - k; l++) {
...
----------------------------------------------------------------------------*/
void Partition2D(weight, dir, m, c, weight1, weight2) double weight[],
    weight1[], weight2[];
int dir, m;
double c;
{
  ...
  for (i = 0; i < m1; i++)
    for (j = 0; j < m1; j++) {
      if (dir == 1) {
        ...
        for (l = 0; l <= m - i; l++) {
          sum =
              sum + weight[m1 * (i + l) + j] * BernsteinBaseValue(m - i, l, c);
        }
        weight2[m1 * i + j] = sum;
      }
      if (dir == 2) {
        ...
        for (l = 0; l <= m - j; l++) {
          sum =
              sum + weight[m1 * i + (l + j)] * BernsteinBaseValue(m - j, l, c);
        }
        weight2[m1 * i + j] = sum;
      }
    }
...
Display2DFunction --linearize the surface z = f(x,y), here f is in BB-form
                    ...
                    BB-form.
----------------------------------------------------------------------------*/
void Display2DFunction(
    a1, a2, b1, b2, m, n, weight, scaler, detx, dety,
    polygons) double weight[]; /* the coefficients of Bernstein polynomial*/
double a1, a2, b1, b2;         /* a1 <= x <= a2; b1 <= y <= b2;           */
...
Polygons *polygons;            /* polygons chain  created                 */
{
  ...
  for (k = 0; k < mx; k++)     /* k loop     */
    for (j = 0; j < my; j++) { /* j loop     */
      ...
      vertices[0][2] =
          scaler[0] * BernsteinValue2D(weight, x, y, m, n) + scaler[3];
...
----------------------------------------------------------------------------*/
Polygons CreatPolygons(n, vertices, normals)
int n;            /* the number of vertices, n > 0           */
Vertex *vertices; /* the vertices of the polygon being
                                         created */
Normal
    *normals; /* the normals of the polygon if n = 1 or 2,
...
----------------------------------------------------------------------------*/
void FreePolygons(polygons) Polygons polygons; /* the given polygons */
{
