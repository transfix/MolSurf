...
void ArandDecimation::arandNeighbors(int v, set<int> &neighbors) {
  for (set<int>::iterator iter = vert2tri[v].begin(); iter != vert2tri[v].end();
       iter++) {
    int tri = *iter;
...
void ArandDecimation::arandNeighbors(int v, vector<int> &neighbors) {
  ...
  do {
    ...
    success = false;
    for (set<int>::iterator myit = vert2tri[v].begin();
         myit != vert2tri[v].end(); myit++) {
      tri = *myit;
      ...
      } else if ((triangulation[tri].c == current &&
                  triangulation[tri].a == v) ||
                 (triangulation[tri].a == current &&
                  triangulation[tri].c == v)) {
      ...
      } else if ((triangulation[tri].b == current &&
                  triangulation[tri].a == v) ||
                 (triangulation[tri].a == current &&
                  triangulation[tri].b == v)) {
...
void ArandDecimation::arandRetriangulate(int v, vector<arandTri> &newTri) {
  ...
  // ?
  Vector_3 normal = (vertices[neighbors[0]] - vertices[v]) ^
                    (vertices[neighbors[1]] - vertices[v]);
  int sometri = *(vert2tri[v].begin());
  Vector_3 orientationCheck =
      (vertices[triangulation[sometri].c] -
       vertices[triangulation[sometri].a]) ^
  ...
  while (neighbors.size() > 3) {
    ...
    for (int i = 0; i < directions.size(); i++) {
      ...
      Vector_3 myNormal =
          (vertices[neighbors[iplus]] - vertices[neighbors[i]]) ^
          (vertices[neighbors[iplus2]] - vertices[neighbors[i]]);
...
void ArandDecimation::arandDecimate(Geoframe &geo, int nIt, float percentage,
                                    float min_angle, float max_variation) {
  ...
  for (int iIt = 0; iIt < nIt && target < curVerts; iIt++) {
    ...
    if (min_angle < 30) {
      min_angle_cur =
          30.0 - (double)(iIt + 1) * (30.0 - min_angle) / (double)nIt;
    } else {
    ...
    for (int i = 0; i < nV; i++) {
      ...
      if (canDecimate[i] == 0) {
        ...
        for (set<int>::iterator sIt = myNeighbors.begin();
             sIt != myNeighbors.end(); sIt++) {
          if (acos(normals[i] * normals[*sIt]) * 180.0 / PI > max_var_cur) {
            canDecimate[i] = 0;
          }
        }
        // 1. Check if the new triangles have small angles
        for (int iTri = 0; iTri < newTri.size() && canDecimate[i] == 1;
             iTri++) {
    ...
    for (int i = 0; i < nV; i++) {
      if (canDecimate[i] == 1) {
        ...
        for (set<int>::iterator sIt = vert2tri[i].begin();
             sIt != vert2tri[i].end(); sIt++) {
          int a, b, c;
...
void ArandDecimation::printRaw(string fname) {
  ...
  // cout << "Printing Triangulation: " << numVertices << " vertices, " <<
  // numTriangles << " triangles" << endl;
  fout << numVertices << " " << numTriangles << endl;
  for (int i = 0; i < vertices.size(); i++) {
    if (canDecimate[i] >= 0) {
      fout << vertices[i].xyz[0] << " " << vertices[i].xyz[1] << " "
           << vertices[i].xyz[2];
      if (fname[fname.size() - 1] == 'n') {
        fout << " " << normals[i].xyz[0] << " " << normals[i].xyz[1] << " "
             << normals[i].xyz[2];
      }
      fout << endl;
    }
  ...
  for (int i = 0; i < triangulation.size(); i++) {
    if (triExists[i]) {
      fout << vertMap[triangulation[i].a] << " " << vertMap[triangulation[i].c]
