...
Triangle tri1;
{
  ...
  if ((p1[0] <= a) && (p2[0] <= a) && (p3[0] <= a) && (p1[2] >= b) &&
      (p2[2] >= b) && (p3[2] >= b)) {
    return (0);
  } else {
    for (i = 0; i < 3; i++) {
...
void TransferFile_raw() {
  ...
  for (i = 0; i < numbtris; i++) {
    for (l = 0; l < 3; l++) {
      ...
      for (j = 0; j < numbpts; j++) {
        k = 6 * j;
        dis = fabs(points[k] - x) + fabs(points[k + 1] - y) +
              fabs(points[k + 2] - z);
        if (dis < 0.00001) {
          eqal = 1;
          index[l] = j;
  ...
  for (i = 0; i < numbpts; i++) {
      fprintf(fppts2, "%lf %lf %lf %lf %lf %lf\n", points[6*i], points[6*i+1],
  points[6*i+2], points[6*i+3], points[6*i+4], points[6*i+5]);
  }
  */
  ...
  */
  ...
  for (i = 0; i < numbpts; i++) {
    ...
    fprintf(fppts2, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", x, y,
            z, x + det * nx, y + det * ny, z + det * nz, nx, ny, nz, nx, ny,
            nz);
  }
