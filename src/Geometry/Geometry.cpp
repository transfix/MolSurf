...
*/
// Represents a molecule's surface
// This class is central to MolSurf and TexMol
...
Geometry::Geometry() {
  ZeroMembers();
  InitializeColors();
  m_UsingBuffers = false;
  m_BuffersInitialized = false;
  m_Extensions = 0;
  m_SortInitialized = false;
  m_DepthOrderedIndices = 0;
}

Geometry::~Geometry() {
  if (m_DepthOrderedIndices) {
    delete[] m_DepthOrderedIndices;
  }
...
void Geometry::CopyTris(const Geometry &copy) {
  ...
  for (c = 0; c < m_NumTris * 3 * 3;
       c++) // for every coordinate of every vertex of every flat Tri
  {
  ...
  m_Use2DTriangleTexture = copy.m_Use2DTriangleTexture;
  m_Texture2D = copy.m_Texture2D;
...
void Geometry::CopyTriVertColors(const Geometry &copy) {
  unsigned int c;
  if (copy.m_TriVertColorsTransparent) {
    AllocateTriVertColors();
    // for (c=0; c<m_NumTriVerts*3; c++) {
    //    m_TriVertColors[c] = copy.m_TriVertColors[c];
    // }
    for (c = 0; c < m_NumTriVerts * 4; c++) {
      m_TriVertColorsTransparent[c] = copy.m_TriVertColorsTransparent[c];
    }
  } else {
    // m_TriVertColors = 0;
    m_TriVertColorsTransparent = 0;
...
void Geometry::ClearPointColors() {
  if (m_PointColorsArray) {
    delete[] m_PointColorsArray;
    m_PointColorsArray = 0;
  }
}
...
void Geometry::ClearTriVertColors() {
  delete[] m_TriVertColorsTransparent;
  m_TriVertColorsTransparent = 0;
...
void Geometry::AllocateTris(unsigned int NumTriVerts, unsigned int NumTris) {
  ...
  m_bExtentsReady = false;
  // for depth sorting
  m_DepthOrderedIndices = new GLuint[m_NumTris * 3];
}
...
void Geometry::AllocateTriTexCoords2D() {
  ...
  unsigned int c;
  for (c = 0; c < m_NumTriVerts; c++) {
    m_TriVertTexCoords2D[c * 2 + 0] =
        (m_TriVerts[c * 3 + 0] - m_Min[0]) / (m_Max[0] - m_Min[0]);
    m_TriVertTexCoords2D[c * 2 + 1] =
        (m_TriVerts[c * 3 + 1] - m_Min[1]) / (m_Max[1] - m_Min[1]);
  }
}

void Geometry::AllocateTriVertColors() {
  ClearTriVertColors();
  m_TriVertColorsTransparent = new float[m_NumTriVerts * 4];
  unsigned int c;
  for (c = 0; c < m_NumTriVerts * 4; c++) {
    m_TriVertColorsTransparent[c] = 0.0f;
  }
}
...
static void setVector(float *dest, const float *v) {
  dest[0] = v[0];
  dest[1] = v[1];
  dest[2] = v[2];
}
...
void Geometry::CalculateQuadSmoothNormals() {
  ...
  for (c = 0; c < m_NumQuadVerts; c++) {
    setVector(m_QuadVertNormals + c * 3, zero);
  }
  // for each Quadangle
...
void Geometry::CalculateTriSmoothNormals() {
  ...
  float zero[3] = {0.0f, 0.0f, 0.0f};
  for (c = 0; c < m_NumTriVerts; c++) {
    setVector(m_TriVertNormals + c * 3, zero);
  }
  // for each triangle
...
void Geometry::CalculateQuadFlatNormals() {
  ...
  for (c = 0; c < m_NumQuads; c++) // for every quad
  {
    ...
    add(normal1, normal2);
    normalize(normal1);
    setVector(m_QuadFlatVerts + c * 12 + 0 * 3,
              m_QuadVerts + v0 * 3); // set the verts
    setVector(m_QuadFlatVerts + c * 12 + 1 * 3, m_QuadVerts + v1 * 3);
    setVector(m_QuadFlatVerts + c * 12 + 2 * 3, m_QuadVerts + v2 * 3);
    setVector(m_QuadFlatVerts + c * 12 + 3 * 3, m_QuadVerts + v3 * 3);
    if (m_QuadVertTexCoords) {
      setVector(m_QuadFlatTexCoords + c * 12 + 0 * 3,
                m_QuadVertTexCoords + v0 * 3); // set the tex coords
      setVector(m_QuadFlatTexCoords + c * 12 + 1 * 3,
                m_QuadVertTexCoords + v1 * 3);
      setVector(m_QuadFlatTexCoords + c * 12 + 2 * 3,
                m_QuadVertTexCoords + v2 * 3);
      setVector(m_QuadFlatTexCoords + c * 12 + 3 * 3,
                m_QuadVertTexCoords + v3 * 3);
    }
    setVector(m_QuadFlatNormals + c * 12 + 0 * 3, normal1); // set the normals
    setVector(m_QuadFlatNormals + c * 12 + 1 * 3, normal1);
    setVector(m_QuadFlatNormals + c * 12 + 2 * 3, normal1);
    setVector(m_QuadFlatNormals + c * 12 + 3 * 3, normal1);
  }
...
void Geometry::CalculateTriFlatNormals() {
  ...
  for (c = 0; c < m_NumTris; c++) // for every triangle
  {
    ...
    CalculateTriangleNormal(normal, v0, v1, v2);
    normalize(normal);
    setVector(m_TriFlatVerts + c * 9 + 0 * 3,
              m_TriVerts + v0 * 3); // set the verts
    setVector(m_TriFlatVerts + c * 9 + 1 * 3, m_TriVerts + v1 * 3);
    setVector(m_TriFlatVerts + c * 9 + 2 * 3, m_TriVerts + v2 * 3);
    if (m_TriVertTexCoords) {
      setVector(m_TriFlatTexCoords + c * 9 + 0 * 3,
                m_TriVertTexCoords + v0 * 3); // set the tex coords
      setVector(m_TriFlatTexCoords + c * 9 + 1 * 3,
                m_TriVertTexCoords + v1 * 3);
      setVector(m_TriFlatTexCoords + c * 9 + 2 * 3,
                m_TriVertTexCoords + v2 * 3);
    }
    setVector(m_TriFlatNormals + c * 9 + 0 * 3, normal); // set the normals
    setVector(m_TriFlatNormals + c * 9 + 1 * 3, normal);
    setVector(m_TriFlatNormals + c * 9 + 2 * 3, normal);
  }
...
void Geometry::mergeComponents(int *componentIndex, int v1, int v2, int n) {
  ...
  if (componentIndex[v2] == -1) {
    ...
    int j;
    for (j = 0; j < n; j++) {
      if (componentIndex[j] == componentIndex[v2]) {
        componentIndex[j] = componentIndex[v1];
      }
    }
  }
}

bool Geometry::getNextComponentSize(int *numVerts, int *numTris,
                                    int *nextComponentIndex,
                                    int *componentIndex) {
  // find the first index != -1. Find the number of vertices and triangles in
  // it. Return the index also
  if (!numVerts || !numTris || !nextComponentIndex || !componentIndex) {
    return false;
  }
  {
    int i;
...
bool Geometry::addComponent(Geometry *component, int nextComponentIndex,
                            int *componentIndex) {
  if (!component) {
    return false;
  }
...
void Geometry::resetUsedComponentIndex(int nextComponentIndex,
                                       int *componentIndex) {
...
bool Geometry::separateComponents(vector<Geometry *> *components) {
  ...
  for (i = 0; i < m_NumTris; i++) {
    ...
    if (componentIndex[v1] == -1 && componentIndex[v2] == -1 &&
        componentIndex[v3] == -1) {
      componentIndex[v1] = componentIndex[v2] = componentIndex[v3] =
          tempComponentIndex;
      tempComponentIndex++;
    }
  }
  ...
  while (true) {
    ...
    if (!getNextComponentSize(&numVerts, &numTris, &nextComponentIndex,
                              componentIndex)) {
      break;
    }
    if (numVerts == 0 || numTris == 0 || nextComponentIndex == -1) {
      break;
    }
    Geometry *component = new Geometry();
    component->AllocateTris(numVerts, numTris);
    if (m_TriVertColorsTransparent) {
...
Geometry *Geometry::merge(Geometry *geometry) {
  ...
  mergedGeometry->AllocateTris(m_NumTriVerts + geometry->m_NumTriVerts,
                               m_NumTris + geometry->m_NumTris);
  ...
  if (m_TriVerts && geometry->m_TriVerts) {
    ...
    for (i = m_NumTriVerts; i < m_NumTriVerts + geometry->m_NumTriVerts; i++) {
      mergedGeometry->m_TriVerts[3 * i + 0] =
          geometry->m_TriVerts[3 * (i - m_NumTriVerts) + 0];
      mergedGeometry->m_TriVerts[3 * i + 1] =
          geometry->m_TriVerts[3 * (i - m_NumTriVerts) + 1];
      mergedGeometry->m_TriVerts[3 * i + 2] =
          geometry->m_TriVerts[3 * (i - m_NumTriVerts) + 2];
    }
  }
  if (m_Tris && geometry->m_Tris) {
    ...
    for (i = m_NumTris; i < m_NumTris + geometry->m_NumTris; i++) {
      mergedGeometry->m_Tris[3 * i + 0] =
          m_NumTriVerts + geometry->m_Tris[3 * (i - m_NumTris) + 0];
      mergedGeometry->m_Tris[3 * i + 1] =
          m_NumTriVerts + geometry->m_Tris[3 * (i - m_NumTris) + 1];
      mergedGeometry->m_Tris[3 * i + 2] =
          m_NumTriVerts + geometry->m_Tris[3 * (i - m_NumTris) + 2];
    }
  }
  if (m_TriVertNormals && geometry->m_TriVertNormals) {
    ...
    for (i = m_NumTriVerts; i < m_NumTriVerts + geometry->m_NumTriVerts; i++) {
      mergedGeometry->m_TriVertNormals[3 * i + 0] =
          geometry->m_TriVertNormals[3 * (i - m_NumTriVerts) + 0];
      mergedGeometry->m_TriVertNormals[3 * i + 1] =
          geometry->m_TriVertNormals[3 * (i - m_NumTriVerts) + 1];
      mergedGeometry->m_TriVertNormals[3 * i + 2] =
          geometry->m_TriVertNormals[3 * (i - m_NumTriVerts) + 2];
    }
  }
  if (m_TriVertColorsTransparent && geometry->m_TriVertColorsTransparent) {
    for (i = 0; i < m_NumTriVerts; i++) {
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 0] =
          m_TriVertColorsTransparent[4 * i + 0];
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 1] =
          m_TriVertColorsTransparent[4 * i + 1];
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 2] =
          m_TriVertColorsTransparent[4 * i + 2];
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 3] =
          m_TriVertColorsTransparent[4 * i + 3];
    }
    for (i = m_NumTriVerts; i < m_NumTriVerts + geometry->m_NumTriVerts; i++) {
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 0] =
          geometry->m_TriVertColorsTransparent[4 * (i - m_NumTriVerts) + 0];
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 1] =
          geometry->m_TriVertColorsTransparent[4 * (i - m_NumTriVerts) + 1];
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 2] =
          geometry->m_TriVertColorsTransparent[4 * (i - m_NumTriVerts) + 2];
      mergedGeometry->m_TriVertColorsTransparent[4 * i + 3] =
          geometry->m_TriVertColorsTransparent[4 * (i - m_NumTriVerts) + 3];
    }
  }
  if (m_TriMeanCurv && geometry->m_TriMeanCurv) {
    ...
    for (i = m_NumTriVerts; i < m_NumTriVerts + geometry->m_NumTriVerts; i++) {
      mergedGeometry->m_TriMeanCurv[i] =
          geometry->m_TriMeanCurv[(i - m_NumTriVerts)];
    }
  }
  if (m_TriGaussianCurv && geometry->m_TriGaussianCurv) {
    ...
    for (i = m_NumTriVerts; i < m_NumTriVerts + geometry->m_NumTriVerts; i++) {
      mergedGeometry->m_TriGaussianCurv[i] =
          geometry->m_TriGaussianCurv[(i - m_NumTriVerts)];
    }
  }
  if (m_K1 && geometry->m_K1) {
    ...
    for (i = m_NumTriVerts * 3;
         i < (m_NumTriVerts + geometry->m_NumTriVerts) * 3; i++) {
...
void Geometry::getExtents(double *minx, double *miny, double *minz,
  ...
  if (m_NumPoints) {
    ...
    *minz = *maxz = m_Points[2];
    for (i = 1; i < m_NumPoints; i++) {
      if (m_Points[3 * i + 0] < *minx) {
        *minx = m_Points[3 * i + 0];
      }
      if (m_Points[3 * i + 1] < *miny) {
        *miny = m_Points[3 * i + 1];
      }
      if (m_Points[3 * i + 2] < *minz) {
        *minz = m_Points[3 * i + 2];
      }
      if (m_Points[3 * i + 0] > *maxx) {
        *maxx = m_Points[3 * i + 0];
      }
      if (m_Points[3 * i + 1] > *maxy) {
        *maxy = m_Points[3 * i + 1];
      }
      if (m_Points[3 * i + 2] > *maxz) {
        *maxz = m_Points[3 * i + 2];
      }
    }
  }
  if (m_NumLineVerts) {
    ...
    *minz = *maxz = m_LineVerts[2];
    for (i = 1; i < m_NumLineVerts; i++) {
      if (m_LineVerts[3 * i + 0] < *minx) {
        *minx = m_LineVerts[3 * i + 0];
      }
      if (m_LineVerts[3 * i + 1] < *miny) {
        *miny = m_LineVerts[3 * i + 1];
      }
      if (m_LineVerts[3 * i + 2] < *minz) {
        *minz = m_LineVerts[3 * i + 2];
      }
      if (m_LineVerts[3 * i + 0] > *maxx) {
        *maxx = m_LineVerts[3 * i + 0];
      }
      if (m_LineVerts[3 * i + 1] > *maxy) {
        *maxy = m_LineVerts[3 * i + 1];
      }
      if (m_LineVerts[3 * i + 2] > *maxz) {
        *maxz = m_LineVerts[3 * i + 2];
      }
    }
  }
  if (m_NumTriVerts) {
    ...
    *minz = *maxz = m_TriVerts[2];
    for (i = 1; i < m_NumTriVerts; i++) {
      if (m_TriVerts[3 * i + 0] < *minx) {
        *minx = m_TriVerts[3 * i + 0];
      }
      if (m_TriVerts[3 * i + 1] < *miny) {
        *miny = m_TriVerts[3 * i + 1];
      }
      if (m_TriVerts[3 * i + 2] < *minz) {
        *minz = m_TriVerts[3 * i + 2];
      }
      if (m_TriVerts[3 * i + 0] > *maxx) {
        *maxx = m_TriVerts[3 * i + 0];
      }
      if (m_TriVerts[3 * i + 1] > *maxy) {
        *maxy = m_TriVerts[3 * i + 1];
      }
      if (m_TriVerts[3 * i + 2] > *maxz) {
        *maxz = m_TriVerts[3 * i + 2];
      }
    }
  }
}
...
void Geometry::print() {
  ...
  for (i = 0; i < m_NumPoints; i++) {
    printf("%u [%f %f %f]\n", i, m_Points[3 * i + 0], m_Points[3 * i + 1],
           m_Points[3 * i + 2]);
  }
  ...
  for (i = 0; i < m_NumLines; i++) {
    ...
    printf("%u [%u %u]: [%f %f %f] - [%f %f %f]\n", i, idx1, idx2,
           m_LineVerts[3 * idx1 + 0], m_LineVerts[3 * idx1 + 1],
           m_LineVerts[3 * idx1 + 2], m_LineVerts[3 * idx2 + 0],
           m_LineVerts[3 * idx2 + 1], m_LineVerts[3 * idx2 + 2]);
  }
  ...
  for (i = 0; i < m_NumTris; i++) {
    ...
    printf("%u [%u %u %u]: [%f %f %f] - [%f %f %f] - [%f %f %f]\n", i, idx1,
           idx2, idx3, m_TriVerts[3 * idx1 + 0], m_TriVerts[3 * idx1 + 1],
           m_TriVerts[3 * idx1 + 2], m_TriVerts[3 * idx2 + 0],
           m_TriVerts[3 * idx2 + 1], m_TriVerts[3 * idx2 + 2],
           m_TriVerts[3 * idx3 + 0], m_TriVerts[3 * idx3 + 1],
           m_TriVerts[3 * idx3 + 2]);
  }
}
...
bool Geometry::useUniquePointColors() { return m_UsePointColors; }

bool Geometry::useUniqueLineColors() { return m_UseLineColors; }

bool Geometry::useUniqueTriangleColors() { return m_UseTriangleColors; }

bool Geometry::use2DTriangleTexture() { return m_Use2DTriangleTexture; }

bool Geometry::useUniqueWireframeColors() { return m_UseWireframeColors; }
...
void Geometry::computeTriangleMeshDerivatives() {
  if (m_TriangleDerivativesValid) {
    return;
  }
  allocateTriangleDerivates();
  MeshDerivatives *meshDerivatives = new MeshDerivatives(
      m_NumTris, m_NumTriVerts, m_Tris, m_TriVerts, m_TriVertNormals,
      m_TriMeanCurv, m_TriGaussianCurv, m_K1, m_K2);
  meshDerivatives->computeDerivatives();
  m_TriangleDerivativesValid = true;
  delete meshDerivatives;
}

void Geometry::computeDerivatives() { computeTriangleMeshDerivatives(); }
...
bool Geometry::printTriangleDerivatives(FILE *fp) {
  ...
  if (!m_TriangleDerivativesValid) {
    return false; // we wont bother trying to compute them here
  }
  if (!m_TriVertNormals || !m_TriMeanCurv || !m_TriGaussianCurv || !m_K1 ||
      !m_K2) {
    return false;
  }
  int i;
  ...
  fprintf(fp, "Normals:\n");
  for (i = 0; i < m_NumTriVerts; i++) {
    fprintf(fp, "%8.3f %8.3f %8.3f\n", m_TriVertNormals[3 * i + 0],
            m_TriVertNormals[3 * i + 1], m_TriVertNormals[3 * i + 2]);
  }
...
void Geometry::setTriangleColorsByNormals() {
  computeDerivatives();
  if (!m_TriangleDerivativesValid) {
    return; // we wont bother trying to compute them here
  }
  AllocateTriVertColors();
  int i;
  // for( i=0; i<m_NumTriVerts*3; i++ )
  ...
  // }
  for (i = 0; i < m_NumTriVerts; i++) {
    m_TriVertColorsTransparent[i * 4 + 0] =
        m_TriVertNormals[i * 3 + 0] / 2.0 + 0.5;
    m_TriVertColorsTransparent[i * 4 + 1] =
        m_TriVertNormals[i * 3 + 1] / 2.0 + 0.5;
    m_TriVertColorsTransparent[i * 4 + 2] =
        m_TriVertNormals[i * 3 + 2] / 2.0 + 0.5;
  }
}
...
void Geometry::setTriangleColorsByMeanCurv() {
  ...
  for (i = 0; i < m_NumTriVerts; i++) {
    ...
    m_TriVertColorsTransparent[4 * i + 0] = r;
    m_TriVertColorsTransparent[4 * i + 1] = g;
    m_TriVertColorsTransparent[4 * i + 2] = b;
  }
}
...
bool Geometry::getPointInTriangle(float x1, float y1, float z1, float x2,
                                  float y2, float z2, float x3, float y3,
                                  float z3, float *xinterp, float *yinterp,
                                  float *zinterp) {
  float v1x = x2 - x1;
  float v1y = y2 - y1;
  float v1z = z2 - z1;
...
bool Geometry::set2DTexture(int width, int height, unsigned char *bits,
                            int numBytesPerEntry) {
  if (!bits || (width < 1) || (height < 1)) {
    return false;
  }
  delete m_Texture2D;
  m_Texture2D = new Texture2D(width, height, bits, numBytesPerEntry);
  return true;
}

bool Geometry::enable2DTriangleTexture(bool enable) {
  if (!m_Texture2D) {
    return false; // what else!
  }
  if (!m_Texture2D->enable(enable)) {
    return false;
  }
  if (enable && (!m_TriVertTexCoords2D)) {
    AllocateTriTexCoords2D();
  }
  m_Use2DTriangleTexture = enable;
  return true;
}

double dist2(float p1[3], float p2[3]) {
  return (p1[0] - p2[0]) * (p1[0] - p2[0]) + (p1[1] - p2[1]) * (p1[1] - p2[1]) +
         (p1[2] - p2[2]) * (p1[2] - p2[2]);
}

double Geometry::getHausdorffDistance(float p11[3], float p12[3], float p13[3],
                                      float p21[3], float p22[3],
                                      float p23[3]) {
  // for now return some crap.
  double dmin1, dmin2, dmin3, d;
  dmin1 = dist2(p11, p21);
  d = dist2(p11, p22);
  if (d < dmin1) {
    dmin1 = d;
...
void Geometry::deallocateBufferObjects() {
  if (m_UsingBuffers) {
    m_Extensions->glDeleteBuffersARB(1, &m_VertexObject);
    m_Extensions->glDeleteBuffersARB(1, &m_NormalObject);
    m_Extensions->glDeleteBuffersARB(1, &m_ColorObject);
  }
}

void Geometry::allocateBufferObjects() {
  if (!m_Extensions) {
  ...
  if (!m_Extensions->initExtensions("GL_VERSION_1_2 "
    ...
    fprintf(stderr,
            "ERROR: Geometry needs VBOs/OpenGL 1.5 to work properly.\n");
    m_UsingBuffers = false;
  } else {
    m_Extensions->glGenBuffersARB(1, &m_VertexObject);
    m_Extensions->glGenBuffersARB(1, &m_NormalObject);
    m_Extensions->glGenBuffersARB(1, &m_ColorObject);
    m_UsingBuffers = true;
  }
}

void Geometry::prepareBuffersTris() {
  if (!m_BuffersInitialized) {
    allocateBufferObjects();
    if (m_UsingBuffers) {
      m_Extensions->glBindBufferARB(GL_ARRAY_BUFFER_ARB, m_VertexObject);
      m_Extensions->glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                                    m_NumTriVerts * 3 * sizeof(float),
                                    m_TriVerts, GL_STATIC_DRAW_ARB);
      m_Extensions->glBindBufferARB(GL_ARRAY_BUFFER_ARB, m_NormalObject);
      m_Extensions->glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                                    m_NumTriVerts * 3 * sizeof(float),
                                    m_TriVertNormals, GL_STATIC_DRAW_ARB);
      m_Extensions->glBindBufferARB(GL_ARRAY_BUFFER_ARB, m_ColorObject);
      m_Extensions->glBufferDataARB(
          GL_ARRAY_BUFFER_ARB, m_NumTriVerts * 4 * sizeof(float),
          m_TriVertColorsTransparent, GL_STATIC_DRAW_ARB);
      // so as not to confuse any future GL state
      unbindBuffer();
    }
...
void Geometry::bindVertexBuffer() {
  ...
  if (m_UsingBuffers) {
    ...
    glVertexPointer(3, GL_FLOAT, 0, m_TriVerts);
  }
}
...
void Geometry::bindNormalBuffer() {
  ...
  if (m_UsingBuffers) {
    ...
    glNormalPointer(GL_FLOAT, 0, m_TriVertNormals);
  }
}

void Geometry::bindColorBuffer(bool colorsChanged) {
  prepareBuffersTris();
  if (m_UsingBuffers) {
    m_Extensions->glBindBufferARB(GL_ARRAY_BUFFER_ARB, m_ColorObject);
    // reload w/ new alpha values
    if (colorsChanged) {
      m_Extensions->glBufferDataARB(
          GL_ARRAY_BUFFER_ARB, m_NumTriVerts * 4 * sizeof(float),
          m_TriVertColorsTransparent, GL_DYNAMIC_DRAW_ARB);
    }
    glColorPointer(4, GL_FLOAT, 0, 0);
    unbindBuffer();
  } else {
    glColorPointer(4, GL_FLOAT, 0, m_TriVertColorsTransparent);
  }
}
...
std::vector<CCVOpenGLMath::Vector>
    triangleCentersNDC; // normalized depth coordinates

CCVOpenGLMath::Vector Geometry::triangleIndexToCenter(int triangleIndex) {
  const int triangleVertex1Index = m_Tris[triangleIndex * 3 + 0];
  const int triangleVertex2Index = m_Tris[triangleIndex * 3 + 1];
  const int triangleVertex3Index = m_Tris[triangleIndex * 3 + 2];
  CCVOpenGLMath::Vector triangleVertex1(
      m_TriVerts[triangleVertex1Index * 3 + 0],
      m_TriVerts[triangleVertex1Index * 3 + 1],
      m_TriVerts[triangleVertex1Index * 3 + 2], 1);
  CCVOpenGLMath::Vector triangleVertex2(
      m_TriVerts[triangleVertex2Index * 3 + 0],
      m_TriVerts[triangleVertex2Index * 3 + 1],
      m_TriVerts[triangleVertex2Index * 3 + 2], 1);
  CCVOpenGLMath::Vector triangleVertex3(
      m_TriVerts[triangleVertex3Index * 3 + 0],
      m_TriVerts[triangleVertex3Index * 3 + 1],
      m_TriVerts[triangleVertex3Index * 3 + 2], 1);
  CCVOpenGLMath::Vector returnValue =
      (triangleVertex1 + triangleVertex2 + triangleVertex3) * (1.0f / 3);
  returnValue[3] = 1.0f; // sigh...
  return returnValue;
}

// for std::sort()
bool triangleDepthCompareFunctor(int lhs, int rhs) {
  CCVOpenGLMath::Vector lhsVectorProjected = triangleCentersNDC[lhs];
  CCVOpenGLMath::Vector rhsVectorProjected = triangleCentersNDC[rhs];
  // return ordering (back to front) based on depth
  return (lhsVectorProjected[2] > rhsVectorProjected[2]);
}

void Geometry::sortGeometryTris() {
  // NOTE: if this is a transparent render we must to back-to-front rendering.
  // simply doing front/back culling isn't sufficient because there are
  // self-shading dents in the surface.  so we do a full depth sort on the
  // triangles.
  // FIXME: slow as hell
  // calculate the normalized device coordinates of the center of each triangle
  // construct the MVP matrix
  GLfloat model[16];
  glGetFloatv(GL_MODELVIEW_MATRIX, model);
  CCVOpenGLMath::Matrix modelviewMatrix(
      model[0], model[4], model[8], model[12], model[1], model[5], model[9],
      model[13], model[2], model[6], model[10], model[14], model[3], model[7],
      model[11], model[15]);
  GLfloat proj[16];
  glGetFloatv(GL_PROJECTION_MATRIX, proj);
  CCVOpenGLMath::Matrix projectionMatrix(proj[0], proj[4], proj[8], proj[12],
                                         proj[1], proj[5], proj[9], proj[13],
                                         proj[2], proj[6], proj[10], proj[14],
                                         proj[3], proj[7], proj[11], proj[15]);
  CCVOpenGLMath::Matrix MVP = projectionMatrix * modelviewMatrix;
  triangleCentersNDC.resize(m_NumTris);
  // calculate centers for all triangles
  for (int i = 0; i < m_NumTris; i++) {
    CCVOpenGLMath::Vector center = triangleIndexToCenter(i);
    CCVOpenGLMath::Vector centerProjected = MVP * center;
    triangleCentersNDC[i] = centerProjected * (1.0f / centerProjected[3]);
  }
  // arbitrary "triangle index"
  // reset this at beginning OR if we edit the geometry
  if (!m_SortInitialized) {
    m_DepthOrderedTriangles.resize(m_NumTris);
    // initialize to {1, 2, 3,...}
    for (int i = 0; i < m_DepthOrderedTriangles.size(); i++) {
      m_DepthOrderedTriangles[i] = i;
    }
    // first time, use quicksort
    std::sort(m_DepthOrderedTriangles.begin(), m_DepthOrderedTriangles.end(),
              triangleDepthCompareFunctor);
    m_SortInitialized = true;
  } else {
    // iterative, largely sorted before rotation (?)
    std::sort(m_DepthOrderedTriangles.begin(), m_DepthOrderedTriangles.end(),
              triangleDepthCompareFunctor);
  }
  // 2: create m_DepthOrderedIndices
  for (int i = 0; i < m_DepthOrderedTriangles.size(); i++) {
    m_DepthOrderedIndices[3 * i + 0] =
        m_Tris[m_DepthOrderedTriangles[i] * 3 + 0];
    m_DepthOrderedIndices[3 * i + 1] =
        m_Tris[m_DepthOrderedTriangles[i] * 3 + 1];
    m_DepthOrderedIndices[3 * i + 2] =
        m_Tris[m_DepthOrderedTriangles[i] * 3 + 2];
  }
}

// Craig: Really doesn't belong here, but currently the situation is
// unreformable
//  should look like:
//  Point a; Point b;
//  (a-b).norm();
double Geometry::distVtxToPoint(int vertex, float *point) {
  double x = m_TriVerts[3 * vertex + 0] - point[0];
  double y = m_TriVerts[3 * vertex + 1] - point[1];
  double z = m_TriVerts[3 * vertex + 2] - point[2];
  return sqrt(x * x + y * y + z * z);
}

// Return the area of the molecule within a bounding sphere
// craig: I moved this here from SurfaceData
double Geometry::getArea(float *point, double radius) {
  double area = 0;
  ...
  for (int i = 0; i < m_NumTris; i++) {
    ...
    int v2 = m_Tris[3 * i + 1];
    int v3 = m_Tris[3 * i + 2];
    area += CCVOpenGLMath::AreaVolumes::getTriangleArea(
        m_TriVerts[3 * v1 + 0], m_TriVerts[3 * v1 + 1], m_TriVerts[3 * v1 + 2],
        m_TriVerts[3 * v2 + 0], m_TriVerts[3 * v2 + 1], m_TriVerts[3 * v2 + 2],
        m_TriVerts[3 * v3 + 0], m_TriVerts[3 * v3 + 1], m_TriVerts[3 * v3 + 2]);
  }
  return area;
}
...
void Geometry::CrossProduct(double *v1, double *v2, double *v) {
  v[0] = v1[1] * v2[2] - v1[2] * v2[1];
  v[1] = v1[2] * v2[0] - v1[0] * v2[2];
  v[2] = v1[0] * v2[1] - v1[1] * v2[0];
}

double Geometry::DotProduct(double *v1, double *v2) {
  return v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2];
}

double Geometry::getVolume() {
  ...
  for (int i = 0; i < m_NumTris; i++) {
    ...
    V1[0] = m_TriVerts[3 * v1 + 0];
    V1[1] = m_TriVerts[3 * v1 + 1];
    V1[2] = m_TriVerts[3 * v1 + 2];
    V2[0] = m_TriVerts[3 * v2 + 0];
    V2[1] = m_TriVerts[3 * v2 + 1];
    V2[2] = m_TriVerts[3 * v2 + 2];
    V3[0] = m_TriVerts[3 * v3 + 0];
    V3[1] = m_TriVerts[3 * v3 + 1];
    V3[2] = m_TriVerts[3 * v3 + 2];
