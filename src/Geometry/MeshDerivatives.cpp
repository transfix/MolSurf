...
MeshDerivatives::MeshDerivatives(unsigned int numTris, unsigned int numTriVerts,
                                 unsigned int *tris, float *triVerts,
                                 float *triVertNormals, float *triMeanCurv,
                                 float *triGaussianCurv, float *k1, float *k2) {
  m_NumTris = numTris;
...
bool MeshDerivatives::computeFaceSet() {
  if ((m_NumTris <= 0) || !m_Tris || !m_FaceSet) {
    return false;
  }
...
bool MeshDerivatives::findVerts(unsigned int triIndex, unsigned int vertIndex,
                                unsigned int *vleft, unsigned int *vright) {
  if (!vleft || !vright) {
    return false;
  }
  if (triIndex > m_NumTris) {
    return false;
  }
...
double MeshDerivatives::voronoiArea(unsigned int vertIndex, unsigned int vleft,
                                    unsigned int vright, double angleVLeft,
                                    double angleVRight) {
...
bool MeshDerivatives::isObtuseTriangle(double angle1, double angle2,
                                       double angle3) {
  if (angle1 < 0 || angle1 > PI / 2.0) {
    return true;
  }
  if (angle2 < 0 || angle2 > PI / 2.0) {
    return true;
  }
  if (angle3 < 0 || angle3 > PI / 2.0) {
    return true;
  }
  return false;
}

double MeshDerivatives::getTriangleArea(unsigned int v1, unsigned int v2,
                                        unsigned int v3) {
  double x1 = m_TriVerts[3 * v1 + 0];
  double y1 = m_TriVerts[3 * v1 + 1];
  double z1 = m_TriVerts[3 * v1 + 2];
...
double MeshDerivatives::areaOfInfinitesimalTriangle(int triangleIdx,
  ...
  if (isObtuseTriangle(angleVertIndex, angleVLeft, angleVRight)) {
    ...
    if (angleVertIndex >= 0 && angleVertIndex <= PI / 2.0) {
      fprintf(fpAMixed,
              "%10.7lf T/4: %10.7lf <%10.7lf, %10.7lf, %10.7lf = %10.7lf>\n",
              triArea, triArea / 4.0, angleVertIndex * f, angleVLeft * f,
              angleVRight * f,
              angleVertIndex * f + angleVLeft * f + angleVRight * f);
      return triArea / 4.0;
    } else {
      fprintf(fpAMixed,
              "%10.7lf T/2: %10.7lf <%10.7lf, %10.7lf, %10.7lf = %10.7lf>\n",
              triArea, triArea / 2.0, angleVertIndex * f, angleVLeft * f,
              angleVRight * f,
              angleVertIndex * f + angleVLeft * f + angleVRight * f);
      return triArea / 2.0;
    }
  ...
  fprintf(fpAMixed,
          "%10.7lf v : %10.7lf <%10.7lf, %10.7lf, %10.7lf = %10.7lf>\n",
          triArea, vArea, angleVertIndex * f, angleVLeft * f, angleVRight * f,
          angleVertIndex * f + angleVLeft * f + angleVRight * f);
  return vArea;
}
...
double MeshDerivatives::computeArea(unsigned int vertIndex) {
  ...
  for (i = 0; i < numtris; i++) {
    ...
    area += areaOfInfinitesimalTriangle(m_FaceSet[vertIndex][i], vertIndex,
                                        vleft, vright);
  }
  return area;
}
...
double MeshDerivatives::computeThetaSum(unsigned int vertIndex) {
  if (m_FaceSet[vertIndex].size() < 1) {
    return 0; // wierd!
  }
  // loop through all faces. Find which of 3 vertices this is. Pick up that
  // angle and add to sum
  double thetaSum = 0;
  int i;
...
void MeshDerivatives::unionVertex(unsigned int v,
                                  vector<unsigned int> *neighboringVertices) {
  int j;
  for (j = 0; j < neighboringVertices->size(); j++) {
    if (neighboringVertices->at(j) == v) {
      return;
    }
  }
...
void MeshDerivatives::getNeighbors(unsigned int vertIndex,
                                   vector<unsigned int> *neighboringVertices) {
  int i;
  for (i = 0; i < m_FaceSet[vertIndex].size(); i++) {
...
void MeshDerivatives::getNeighborsAngles(unsigned int x_i, unsigned int x_j,
...
bool MeshDerivatives::getLaplaceBeltramiOperator(double *Kx, double *Ky,
                                                 double *Kz,
                                                 unsigned int vertIndex,
                                                 double A_mixed) {
  if (!Kx || !Ky || !Kz) {
  ...
  for (int i = 0; i < neighboringVertices.size(); i++) {
    // find the other 2 vertices and the respective angles. Add to K
    // if we can only get one other neighbor, do some funky approximation
    // SKVINAY
    double alpha, beta;
    int count = 0;
    getNeighborsAngles(vertIndex, neighboringVertices[i], &alpha, &beta,
                       &count);
    if (count == 0) {
      continue;
    }
...
bool MeshDerivatives::computeDerivatives(unsigned int vertIndex) {
  ...
  // for each vertex neighbor, compute alphas, betas, xj-xi, and update K(x) -
  // laplace beltrami operator
  double Kx = 0, Ky = 0, Kz = 0;
  if (!getLaplaceBeltramiOperator(&Kx, &Ky, &Kz, vertIndex, A_mixed)) {
    return false;
  }
...
bool MeshDerivatives::computeDerivatives() {
  ...
  if ((m_NumTris <= 0) || (m_NumTriVerts <= 2) || !m_Tris || !m_TriVerts ||
      !m_TriVertNormals || !m_TriMeanCurv || !m_TriGaussianCurv || !m_K1 ||
      !m_K2 || !m_FaceSet || !m_Angles) {
    return false;
  }
  ...
  // compute n, H, G, k1, k2 for each vertex. (right now im not implementing the
  // minimization for k1, k2 - SKVINAY)
  for (int i = 0; i < m_NumTriVerts; i++) {
