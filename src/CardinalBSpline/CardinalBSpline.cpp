...
bool CardinalBSpline::computeFunction(std::vector<PDBParser::Atom *> atomList,
                                      float *data, unsigned int *dim,
  ...
  for (int i = 0; i < size; i++) {
    ...
    radius += probeRadius;
    CardinalBSpline_One_Atom(position, radius, order, dim, minExtent, dinteval,
                             data);
  }
}

bool CardinalBSpline::getAtomListAndExtent(
    GroupOfAtoms *molecule, std::vector<PDBParser::Atom *> &atomList,
    float *minExtent, float *maxExtent) {
...
SimpleVolumeData *
CardinalBSpline::getCardinalBSplineSurface(GroupOfAtoms *molecule,
...
void CardinalBSpline::CardinalBSpline_One_Atom(float *position, float radius,
                                               int order, unsigned int *dim,
                                               float *minExtent,
                                               float *dinteval, float *data) {
  float funct;
  float x, y, z;
  int nx, ny, nz;
  int space[3];
    ...
    for (int j = ny - space[1] - tmp; j < ny + space[1] + tmp; j++) {
      ...
      for (int k = nz - space[2] - tmp; k < nz + space[2] + tmp; k++) {
        if ((k >= 0) && (k < dim[2])) {
          ...
          funct = CardinalBBasis(order, x, radius) *
                  CardinalBBasis(order, y, radius) *
                  CardinalBBasis(order, z, radius);
          if (funct <= -0.0) {
            funct *= -1.0;
          }
