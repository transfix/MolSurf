...
bool SDFLibrary::setParameters(int Size, int isNormalFlip, float *mins,
                               float *maxs) {
...
float *SDFLibrary::computeSDF(int nverts, float *verts, int ntris, int *tris) {
  ...
  // Return the SDF
  numb =
      (SDFLibrary::size + 1) * (SDFLibrary::size + 1) * (SDFLibrary::size + 1);
...
SimpleVolumeData *SDFLibrary::getSDF(Geometry *geometry, int size) {
  ...
  float *values =
      SDFLibrary::computeSDF(geometry->m_NumTriVerts, geometry->m_TriVerts,
                             geometry->m_NumTris, (int *)(geometry->m_Tris));
  if (!values) {
    return 0;
  }
