...
void process_triangle(int i) {
  ...
  distances[i] = (double)(-1 * ((nx * vertices[v1].x) + (ny * vertices[v1].y) +
                                (nz * vertices[v1].z)));
  surface[i].type = -1;
}
  ...
  for (int i = 0; i < total_triangles; i++) {
    ...
    if (vertices[surface[i].v1].trisUsed >= MAX_TRIS_PER_VERT) {
      printf("more than %d triangles share this vertex... %d for vert=%d\n",
             MAX_TRIS_PER_VERT, vertices[surface[i].v1].trisUsed,
             surface[i].v1);
      flag = 1;
    }
    if (vertices[surface[i].v2].trisUsed >= MAX_TRIS_PER_VERT) {
      printf("more than %d triangles share this vertex... %d for vert=%d\n",
             MAX_TRIS_PER_VERT, vertices[surface[i].v2].trisUsed,
             surface[i].v2);
      flag = 1;
    }
    if (vertices[surface[i].v3].trisUsed >= MAX_TRIS_PER_VERT) {
      printf("more than %d triangles share this vertex... %d for vert=%d\n",
             MAX_TRIS_PER_VERT, vertices[surface[i].v3].trisUsed,
             surface[i].v3);
      flag = 1;
    }
    // If any of these above statements are printed, then please increase the
    // MAX_TRIS_PER_VERT definition in head.h file and try.
    if (flag == 1) {
      printf("Please try changing the MAX_TRIS_PER_VERT variable in <head.h> "
             "file and rerun\n");
      exit(0);
    }
  }
...
bool setOctree_depth() {
  ...
  case (1024):
    ...
    break;
  default:
    printf("This version can only deal with Volumes of sizes 16, 32, 64, 128, "
           "256, 512 or 1024\n");
    return false;
  }
  return true;
