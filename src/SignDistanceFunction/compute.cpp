...
// (-31, -32, -33 or 31, 32 33), then, find the distance between the two points
...
// compute the distance of each Triangle in the cell (ci,cj,ck) from the Vertex
// vert.
int each_cell(int ci, int cj, int ck, int vi, int vj, int vk) {
  ...
  while (currNode != NULL) {
    ...
    val = (double)point_2_plane(currrentTri, xCoord(vi), yCoord(vj), zCoord(vk),
                                &temp);
  ...
  // if (values[vert].value >= (MAX_DIST) || values[vert].value <=
  // (-1*MAX_DIST))
  ...
  for (int i = 0; i <= size; i++) {
    for (int j = 0; j <= size; j++) {
      for (int k = 0; k <= size; k++) {
        int sgn = klc_assign(i, j, k);
        int ind = index2vert(i, j, k);
