...
ConDataset *newDatasetUnstr(int datatype, int meshtype, int nvars, int ntime,
                            int nverts, int ncells, double *verts, u_int *cells,
...
ConDataset *loadDataset(int datatype, int meshtype, int nvars, int ntime,
  ...
  if (vnfd) {
    ...
    for (i = 0; i < nvars; i++) {
      ...
      CVCUtility::fgetsSafely(dataset->vnames[i], 80, vnfd);
...
SliceData *getSlice(ConDataset *dataset, int variable, int timestep, char axis,
                    u_int index) {
