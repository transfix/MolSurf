        ...
        if (verbose)
          if (same != 3)
            printf("cell %d (%d %d %d %d) not adj to %d (%d %d %d %d)\n", i,
                   ...
                   cells[adj][0], cells[adj][1], cells[adj][2], cells[adj][3]);
      }
    }
  }
  preprocessData(data);
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    ...
    tetSurfIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]), getVert(v[3]),
                    ...
                    getValue(v[3]), fx, val, len, getMin(), getMax(), 1.0);
  }
  return (val);
}
...
float *Datavol::compGradient(u_int &len, float **funx) {
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    ...
    tetSurfIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]), getVert(v[3]),
                    getValue(v[0]), getValue(v[1]), getValue(v[2]),
                    getValue(v[3]), fx, val, len, getMin(), getMax(),
                    (float)fabs(scaling));
  }
  return (val);
}
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    ...
    tetVolIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]), getVert(v[3]),
                   getValue(v[0]), getValue(v[1]), getValue(v[2]),
                   getValue(v[3]), fx, val, cum, len, getMin(), getMax(), 1.0);
  }
