...
float *Dataslc::compLength(u_int &len, float **funx) {
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    v = getCellVerts(c);
    triSurfIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]), getValue(v[0]),
                    getValue(v[1]), getValue(v[2]), fx, val, len, getMin(),
                    getMax(), 1.0);
  }
  return (val);
}
...
float *Dataslc::compArea(u_int &len, float **funx) {
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    v = getCellVerts(c);
    triVolIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]), getValue(v[0]),
                   getValue(v[1]), getValue(v[2]), fx, val, cum, len, getMin(),
                   getMax(), 1.0);
  }
