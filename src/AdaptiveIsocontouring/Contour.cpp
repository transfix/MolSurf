...
void Contour::initialize(int fine_level_, int coarse_level_, float *vd_coarse_,
                         float *min, float *max, LittleGeoframe *geo_) {
...
void Contour::initialize(int fine_level, int coarse_level, float *vol,
                         float *min, float *max, LittleGeoframe &geo) {
  initialize(fine_level, coarse_level, vol, min, max, &geo);
}

void Contour::initialize(int fine_level_, int coarse_level_, double *vol,
                         LittleGeoframe &geo_) {
  fine_level = fine_level_;
  coarse_level = coarse_level_;
  geo = &geo_;
...
void Contour::set_isovalue_outer(float iso) { iso_val_outer = iso; }

void Contour::set_isovalue_inner(float iso) { iso_val_inner = iso; }
...
void Contour::put_bishoulder() {
  ...
  float all_mid[3] = {in_min[0] + dim[0] / 2, in_min[1] + dim[1] / 2,
                      in_min[2] + dim[2] / 2};
  float tmp[8];
  float bi_vert_tmp[3];
...
void Contour::polygonize() {
  ...
  cout << "called Contour::polygonize, geo_size: " << geo->getNumVerts()
       << " isovalue: " << iso_val_outer << endl;
  int c_size = 1 << coarse_level;
  int c_osize = c_size + 1;
  int ind1, ind2;
  unsigned int quad[4];
    ...
    for (int z = 1; z < c_osize - 1; z++) {
      for (int x = 0; x < c_size; x++) {
        ...
        if (check_iso_range(iso_val_outer, vd_coarse[ind1], vd_coarse[ind2])) {
          ...
          geo->addQuad(quad, 4);
        } else if (check_iso_range(iso_val_outer, vd_coarse[ind2],
                                   vd_coarse[ind1])) {
          quad[0] = x + (y - 1) * c_size + (z - 1) * c_size * c_size;
...
void Contour::update_quad_render() {
  ...
  for (int j = 0; j < geo->getNumQuads(); j++) {
    ...
    for (int k = 0; k < 4; k++) {
      ...
      int loc[3] = {vert % c_size, vert / c_size % c_size,
                    vert / c_size / c_size};
...
void Contour::initialize_redbox(int resol, float *b_min, float *b_max,
                                float *sum_in) {
  vd_fine = sum_in;
  ...
  for (int j = 0; j < 3; j++) {
    ...
    box_rel_min[j] =
        (int)floor((double)(b_min[j] - in_min[j]) /
                   dim[j]); // the coarse index where the red_box begins
    box_rel_max[j] =
        (int)floor((double)(b_max[j] - in_min[j]) /
                   dim[j]); // the coarse index where the red_box ends
    if (box_rel_min[j] < 0) {
      box_rel_min[j] = 0;
    ...
    for (int z = 1; z < c_osize - 1; z++) {
      for (int x = 0; x < c_size; x++) {
        ...
        if (check_iso_range(iso_val_inner, vd_fine[ind1], vd_fine[ind2])) {
          ...
          quad[3] += x + (y)*c_size + (z - 1) * c_size * c_size;
          geo->addQuad(quad, 4);
        } else if (check_iso_range(iso_val_inner, vd_fine[ind2],
                                   vd_fine[ind1])) {
          quad[0] += x + (y - 1) * c_size + (z - 1) * c_size * c_size;
          quad[1] += x + (y)*c_size + (z - 1) * c_size * c_size;
          quad[2] += x + (y)*c_size + (z)*c_size * c_size;
