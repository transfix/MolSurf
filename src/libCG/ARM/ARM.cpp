...
void ARM::getAtomList(std::vector<PDBParser::Atom *> &atomList,
                      const char *fname) {
...
void ARM::getAtomListOfBead(int index,
...
void ARM::getOptimizedCG(std::vector<PDBParser::Atom *> &cg, int size,
                         int iteration, char *outfilename) {
  ...
  for (int i = 0; i < armHierarchy.size(); i++) {
    ...
    printf("before Optimized: %f %f %f %f\n", arm[1][id]->m_Position[0],
           arm[1][id]->m_Position[1], arm[1][id]->m_Position[2],
           arm[1][id]->getRadius());
    bead->setPosition(optm->adata[0].center[0], optm->adata[0].center[1],
                      optm->adata[0].center[2]);
    ...
    for (int k = 0; k < atomList_bead.size(); k++) {
      ...
      charge +=
          atomList_bead[k]->getCharge() *
          exp(blob *
              (((atomList_bead[k]->m_Position[0] - arm[1][id]->m_Position[0]) *
                    (atomList_bead[k]->m_Position[0] -
                     arm[1][id]->m_Position[0]) +
                (atomList_bead[k]->m_Position[1] - arm[1][id]->m_Position[1]) *
                    (atomList_bead[k]->m_Position[1] -
                     arm[1][id]->m_Position[1]) +
                (atomList_bead[k]->m_Position[2] - arm[1][id]->m_Position[2]) *
                    (atomList_bead[k]->m_Position[2] -
                     arm[1][id]->m_Position[2])) /
                   (atomList_bead[k]->getRadius() *
                    atomList_bead[k]->getRadius()) -
               1.0));
...
void ARM::printAtomList(std::vector<PDBParser::Atom *> &atomList,
                        char *outfilename) {
  FILE *fp = fopen(outfilename, "w");
  for (int i = 0; i < atomList.size(); i++) {
    fprintf(fp, "ATOM%7d  C   %3s %c%4d    %8.3f%8.3f%8.3f%8.4f%7.4f\n", i + 1,
            atomList[i]->getResidueName(), atomList[i]->getChain(),
            atomList[i]->getResidueNum(), atomList[i]->m_Position[0],
            atomList[i]->m_Position[1], atomList[i]->m_Position[2],
            atomList[i]->getCharge(), atomList[i]->getRadius());
  }
  fclose(fp);
}
