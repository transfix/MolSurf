...
void CGOptim::radiusOptimizeFunction(double *p, double *x, int m, int n,
                                     void *data) {
...
void CGOptim::centerOptimizeFunction(double *p, double *x, int m, int n,
                                     void *data) {
  FunctionData *fdata = (FunctionData *)data;
  Optimizer *opt = new Optimizer();
  opt->adata[0].center[0] = fdata->adata[0].center[0];
...
void CGOptim::radiuscenterOptimizeFunction(double *p, double *x, int m, int n,
                                           void *data) {
  FunctionData *fdata = (FunctionData *)data;
  Optimizer *opt = new Optimizer();
  opt->adata[0].center[0] = fdata->adata[0].center[0];
...
void CGOptim::chargeOptimizeFunction(double *p, double *x, int m, int n,
                                     void *data) {
  FunctionData *fdata = (FunctionData *)data;
  Optimizer *opt = new Optimizer();
...
void CGOptim::chargeOptimizeFunction2(double *p, double *x, int m, int n,
                                      void *data) {
  FunctionData *fdata = (FunctionData *)data;
  Optimizer *opt = new Optimizer();
  opt->AE = fdata->AE;
...
void centerOptimizeFunction(double *c, double *x, int mm, int n, void *data)
{
  ...
  for(j=0; j<n; j++)
  {
    ...
    for(i=0; i<m; i++)
    {
      ...
      x[j] += evalDensity(m_blobby, center, samplePoints[j], dptr[i].radius);//
- blurValue[j];
    }
    x[j] -= blurValue[j];
  }
...
void radiuscenterOptimizeFunction(double *c, double *x, int m, int n, void
*data)
{
  register int i,j;
  double tmp = 0.0;
