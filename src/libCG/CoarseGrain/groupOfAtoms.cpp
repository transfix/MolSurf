...
void GroupOfAtoms::getBornRadii(Surface *surface) {
  ...
  */
  ...
  for (i = 0; i < surface->numbtris; i++) {
    for (k = 0; k < nn; k++) {
      coeff[i * nn + k] = dotProduct(surface->m_Faces[i]->node[k].Point,
                                     surface->m_Faces[i]->node[k].Normal);
  ...
  for (i = 0; i < numOfAtoms; i++) {
    A4 = (sum[i] - m_Atoms[i]->position[0] * sumx[i] -
          m_Atoms[i]->position[1] * sumy[i] -
          m_Atoms[i]->position[2] * sumz[i]) /
         (4 * pi);
  ...
  for (i = 0; i < numOfAtoms; i++) {
    ...
    (A4 != 0) ? m_Atoms[i]->Born_radius = 1.0 / A4
  ...
  for (i = 0; i < numOfAtoms; i++) {
    ...
    A7 = (sum[i] - m_Atoms[i]->position[0] * sumx[i] -
          m_Atoms[i]->position[1] * sumy[i] -
          m_Atoms[i]->position[2] * sumz[i]) /
         (16 * pi);
    //            printf("%f %f\n", A4, A7);
    ...
    if (A4 + A7 > 0.0000001) {
      m_Atoms[i]->Born_radius =
          1.0 / ((1.0 - 1.0 / sqrt(2.0)) * A4 + A7); // this is GBSW model
    } else {
      m_Atoms[i]->Born_radius = 100000.0;
    }
  ...
  for (i = 0; i < surface->numbtris; i++) {
    for (k = 0; k < nn; k++) {
      x[3 * (i * nn + k)] =
          (surface->m_Faces[i]->node[k].Point[0] - surface->center[0]) /
          (2 * abs_max);
      x[3 * (i * nn + k) + 1] =
          (surface->m_Faces[i]->node[k].Point[1] - surface->center[1]) /
          (2 * abs_max);
      x[3 * (i * nn + k) + 2] =
          (surface->m_Faces[i]->node[k].Point[2] - surface->center[2]) /
          (2 * abs_max);
      if (fabs(x[3 * (i * nn + k)]) > 0.25 ||
          fabs(x[3 * (i * nn + k) + 1]) > 0.25 ||
          fabs(x[3 * (i * nn + k) + 2]) > 0.25) {
  ...
  for (i = 0; i < numOfAtoms; i++) {
    ...
    y[3 * i + 1] =
        (m_Atoms[i]->position[1] - surface->center[1]) / (2 * abs_max);
    y[3 * i + 2] =
        (m_Atoms[i]->position[2] - surface->center[2]) / (2 * abs_max);
    if (fabs(y[3 * i]) > 0.25 | fabs(y[3 * i + 1]) > 0.25 |
        fabs(y[3 * i + 2]) > 0.25) {
  ...
  for (i = 0; i < surface->numbtris; i++) {
    for (k = 0; k < nn; k++) {
      ...
      coeffx[i * nn + k] = surface->m_Faces[i]->node[k].Normal[0];
      coeffx[i * nn + k] *=
          surface->m_Faces[i]->weight[k] / pow(2 * abs_max, 4);
      coeffy[i * nn + k] = surface->m_Faces[i]->node[k].Normal[1];
      coeffy[i * nn + k] *=
          surface->m_Faces[i]->weight[k] / pow(2 * abs_max, 4);
      coeffz[i * nn + k] = surface->m_Faces[i]->node[k].Normal[2];
  ...
  param /= (1 * abs_max);
  m_fastsum = new FastSummation(3, N, M, n, m, p, "one_over_quartic", &param,
                                eps_I, eps_B);
  m_fastsum->threadedFastSum(x, y, coeff, coeffx, coeffy, coeffz, sum, sumx,
                             sumy, sumz);
  delete m_fastsum;
  /*
          printf("compare\n");
  ...
  */
  ...
  /*
          ...
          for (i = 0; i < surface->numbtris; i++)
          {
                  ...
                  {
                          x[3*(i*nn+k)  ] =
  surface->m_Faces[i]->node[k].Point[0]; x[3*(i*nn+k)+1] =
  surface->m_Faces[i]->node[k].Point[1]; x[3*(i*nn+k)+2] =
  surface->m_Faces[i]->node[k].Point[2];
                  }
          }
          ...
          for (i = 0; i < surface->numbtris; i++)
          {
                  ...
                  {
                          coeff[i*nn+k] =
  dotProduct(surface->m_Faces[i]->node[k].Point,
  surface->m_Faces[i]->node[k].Normal); coeff[i*nn+k] *=
  surface->m_Faces[i]->weight[k];

                          coeffx[i*nn+k] =
  surface->m_Faces[i]->node[k].Normal[0]; coeffx[i*nn+k] *=
  surface->m_Faces[i]->weight[k];

                          coeffy[i*nn+k] =
  surface->m_Faces[i]->node[k].Normal[1]; coeffy[i*nn+k] *=
  surface->m_Faces[i]->weight[k];

                          coeffz[i*nn+k] =
  surface->m_Faces[i]->node[k].Normal[2]; coeffz[i*nn+k] *=
  surface->m_Faces[i]->weight[k];
                  }
          }
          ...
          for (i = 0; i < numOfAtoms; i++)
          {
                  t_sum[i] = 0;     t_sumx[i] = 0;    t_sumy[i] = 0; t_sumz[i] =
  0; for (k = 0; k < surface->numbtris*nn; k++)
                  {
                          double denom = (x[3*k  ] - y[3*i  ])*(x[3*k  ] - y[3*i
  ]) + (x[3*k+1] - y[3*i+1])*(x[3*k+1] - y[3*i+1]) + (x[3*k+2] -
  y[3*i+2])*(x[3*k+2] - y[3*i+2]);
                          //if (denom > 5*5)      continue;
                          denom *= denom;
                          t_sum[i]  += coeff[k] /
  (denom*denom*denom*sqrt(denom)); t_sumx[i] += coeffx[k] /
  (denom*denom*denom*sqrt(denom)); t_sumy[i] += coeffy[k] /
  (denom*denom*denom*sqrt(denom)); t_sumz[i] += coeffz[k] /
  (denom*denom*denom*sqrt(denom));
                  }
          }
  #endif // _TRIVIAL_
  */
  ...
  param /= (1 * abs_max);
  m_fastsum =
      new FastSummation(3, N, M, n, m, p, (char *)"one_over_quartic",
                        (char *)"one_over_seventh", &param, eps_I, eps_B);
  m_fastsum->threadedFastSumCorrection(x, y, coeff, coeffx, coeffy, coeffz, sum,
                                       sumx, sumy, sumz, c_sum, c_sumx, c_sumy,
                                       c_sumz);
  delete m_fastsum;
  //     for (i = 0; i < numOfAtoms; i++)
  ...
  for (i = 0; i < numOfAtoms; i++) {
    A4 = (sum[i] - m_Atoms[i]->position[0] * sumx[i] -
          m_Atoms[i]->position[1] * sumy[i] -
          m_Atoms[i]->position[2] * sumz[i]) /
         (4 * pi * pow(2 * abs_max, 4));
    (A4 > 0.0) ? m_Atoms[i]->Born_radius = 1.0 / A4
               : m_Atoms[i]->Born_radius = 100000.0;
    if (A4 < 0.0) {
      printf("negative\n");
    }
  ...
  /*
          for (i = 0; i < numOfAtoms; i++)
          {
                  c_sum[i] /= pow(2*abs_max, 3);  c_sumx[i] /= pow(2*abs_max,
     3); c_sumy[i] /= pow(2*abs_max, 3); c_sumz[i] /= pow(2*abs_max, 3);
          }
  */
  for (i = 0; i < numOfAtoms; i++) {
    A4 = 1.0 / m_Atoms[i]->Born_radius;
    A7 = (c_sum[i] - m_Atoms[i]->position[0] * c_sumx[i] -
          m_Atoms[i]->position[1] * c_sumy[i] -
          m_Atoms[i]->position[2] * c_sumz[i]) /
         (16 * pi * pow(2 * abs_max, 7));
    //            printf("%f %f\n", A4, sqrt(sqrt(A7)));
    if (A7 < 0.0) {
      A7 = 0.0;
    }
       ...
       /*
               ...
               for (a = 0; a < 20; a++)
               {
                       ...
                       for (i = 0; i < surface->numbtris; i++)
                       {
                               for (k = 0; k < nn; k++)
                               {
                                       xy[0] = surface->m_Faces[i]->node[k].Point[0]
          - m_Atoms[a]->position[0];      xy[1] =
          surface->m_Faces[i]->node[k].Point[1] -      m_Atoms[a]->position[1];      xy[2]
          = surface->m_Faces[i]->node[k].Point[2] -      m_Atoms[a]->position[2];      den
          = dotProduct(xy, xy);      if (sqrt(den) > 4.0)      continue;      den =
          den*den;      num = dotProduct(xy,      surface->m_Faces[i]->node[k].Normal);
          num *=      surface->m_Faces[i]->weight[k];      testsum[a] += num / den;
                               }
                       }
                       printf("%f, %f\n",
          sum[a]-m_Atoms[a]->position[0]*sumx[a]-m_Atoms[a]->position[1]*sumy[a]-m_Atoms[a]->position[2]*sumz[a],
          testsum[a]);
               }
       */
       /*
               ...
               m_fastsum = new FastSummation(3, N, M, n, m, p, "one_over_seventh",
          &param, eps_I, eps_B);      for (i = 0; i < surface->numbtris; i++)
               {
                       for (k = 0; k < nn; k++)
                       {
               ...
               m_fastsum->threadedFastSum(x, y, coeff, coeffx, coeffy, coeffz, sum,
          sumx, sumy, sumz);
     
               for (i = 0; i < numOfAtoms; i++)
               {
                       A7 =
          (sum[i]-m_Atoms[i]->position[0]*sumx[i]-m_Atoms[i]->position[1]*sumy[i]-m_Atoms[i]->position[2]*sumz[i])/(16*pi);
                       if(A7 < 0.0) A7 = 0.0;
                       A7 = sqrt(A7); A7 = sqrt(A7); // wenqi added on 12/17
               }
               delete m_fastsum;
               ...
               for (i = 0; i < numOfAtoms; i++)
               {
                       A4 = m_Atoms[i]->Born_radius;
                       if( A4 + A7 > 0.0000001)
                               m_Atoms[i]->Born_radius = 1.0 / ( (1.0-1.0/sqrt(2.0))
          * A4 + A7 ); // this is GBSW model      else      m_Atoms[i]->Born_radius =
          100000.0;      if (m_Atoms[i]->Born_radius < 0.0)      printf("%d %f\n", i,
          m_Atoms[i]->Born_radius);
               }
       */
       /*
       ...
       /////////////////////////////////////////////////////////////////////////
               ...
               for (i = 0; i < surface->numbtris; i++)
               {
                       ...
                       {
                               coeff[i*nn+k] =
       surface->m_Faces[i]->node[k].Normal[0];      coeff[i*nn+k] *=
       surface->m_Faces[i]->weight[k] / pow(4*abs_max, 4);
                       }
               }
               ...
               for (i = 0; i < surface->numbtris; i++)
               {
                       ...
                       {
                               coeff[i*nn+k] =
       surface->m_Faces[i]->node[k].Normal[1];      coeff[i*nn+k] *=
       surface->m_Faces[i]->weight[k] / pow(4*abs_max, 4);
                       }
               }
               m_fastsum->fastSum(x, y, coeff, sum2);
               ...
               for (i = 0; i < surface->numbtris; i++)
               {
                       ...
                       {
                               coeff[i*nn+k] =
       surface->m_Faces[i]->node[k].Normal[2];      coeff[i*nn+k] *=
       surface->m_Faces[i]->weight[k] / pow(4*abs_max, 4);
                       }
               }
               m_fastsum->fastSum(x, y, coeff, sum2);
...
bool GroupOfAtoms::overlap(int i, int j) {
  ...
  dij = (xi[0] - xj[0]) * (xi[0] - xj[0]) + (xi[1] - xj[1]) * (xi[1] - xj[1]) +
        (xi[2] - xj[2]) * (xi[2] - xj[2]);
  if (dij < (ri + rj) * (ri + rj)) {
    return true;
  } else {
