...
void CoarseGrain::CoarseGrainChargeGB(GroupOfAtoms *groupOfBeads) {
...
void CoarseGrain::CoarseGrainChargeGB2(GroupOfAtoms *groupOfBeads) {
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++) {
      double rij2 = (X[i] - X[j]) * (X[i] - X[j]) +
                    (Y[i] - Y[j]) * (Y[i] - Y[j]) +
                    (Z[i] - Z[j]) * (Z[i] - Z[j]);
      err1 += groupOfBeads->m_Atoms[i]->charge *
              groupOfBeads->m_Atoms[j]->charge /
              sqrt(rij2 + R[i] * R[j] * exp(-1.0 * rij2 / (4.0 * R[i] * R[j])));
    }
  ...
  for (i = 0; i < N; i++)
    for (j = i + 1; j < N; j++) {
      double rij2 = (X[i] - X[j]) * (X[i] - X[j]) +
                    (Y[i] - Y[j]) * (Y[i] - Y[j]) +
                    (Z[i] - Z[j]) * (Z[i] - Z[j]);
      err2 += groupOfBeads->m_Atoms[i]->charge *
              groupOfBeads->m_Atoms[j]->charge / sqrt(rij2);
    }
  printf("AE Col = %f CE Col = %f\n", ecol, err2);
}

double CoarseGrain::getBeadSelfEnergy(GroupOfAtoms *groupOfBeads, int index) {
  int i, j, atomi, atomj;
