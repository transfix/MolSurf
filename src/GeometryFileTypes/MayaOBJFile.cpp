...
// ignoring normals cause its tough to parse and TexMol computes decent normals
// anyway SKVINAY a very basic read, only for triangles!!
      ...
      if (strlen(line) > 3) {
        if (line[0] == 'f' && line[1] == ' ') {
          ...
          if (vPresent && !nPresent && !tPresent) {
            ...
            if (sscanf(line, "f %d/%d %d/%d %d/%d", &idx_1, &t_idx_1, &idx_2,
                       &t_idx_2, &idx_3, &t_idx_3) == 6) {
              vindices.push_back(idx_1 - 1);
              vindices.push_back(idx_2 - 1);
              vindices.push_back(idx_3 - 1);
            ...
            if (sscanf(line, "f %d//%d %d//%d %d//%d", &idx_1, &n_idx_1, &idx_2,
                       &n_idx_2, &idx_3, &n_idx_3) == 6) {
              vindices.push_back(idx_1 - 1);
              vindices.push_back(idx_2 - 1);
              vindices.push_back(idx_3 - 1);
              numTris++;
            ...
            if (sscanf(line, "f %d/%d/%d %d/%d/%d %d/%d/%d", &idx_1, &t_idx_1,
                       &n_idx_1, &idx_2, &t_idx_2, &n_idx_2, &idx_3, &t_idx_3,
                       &t_idx_3) == 9) {
              vindices.push_back(idx_1 - 1);
              vindices.push_back(idx_2 - 1);
              vindices.push_back(idx_3 - 1);
...
bool MayaOBJFile::checkType(const string &fileName) { return false; }

bool MayaOBJFile::saveFile(const Geometry *geometry, const string &fileName) {
  if (!geometry) {
    return false;
  }
  if ((geometry->m_NumTriVerts <= 0) || !geometry->m_TriVerts ||
      (geometry->m_NumTris <= 0) || !geometry->m_Tris) {
    return false;
  }
  FILE *fp;
  // open the file
