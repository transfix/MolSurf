...
bool RawcFile::checkType(const string &fileName) {
  ...
  for (c = 0; c < numverts; c++) {
    ...
    // position and normal
    if (6 != fscanf(fp, "%f %f %f %f %f %f", &(f1), &(f2), &(f3), &(f4), &(f5),
                    &(f6))) {
  ...
  for (c = 0; c < numtris; c++) {
    ...
    if (u1 == (unsigned int)numverts || u2 == (unsigned int)numverts ||
        u3 == (unsigned int)numverts) {
      maxSeen = true;
    }
    // cant have both!
    if (maxSeen && zeroSeen) {
    ...
    if (geometry->m_TriVertColorsTransparent) {
      ...
      if (0 >=
          fprintf(fp, "%f %f %f %f %f %f\n", (geometry->m_TriVerts[c * 3 + 0]),
                  (geometry->m_TriVerts[c * 3 + 1]),
                  (geometry->m_TriVerts[c * 3 + 2]), (1.0), (1.0), (1.0))) {
        printf("Error writing out vert # %d\n", c);
        fclose(fp);
        return false;
      }
