...
Geometry *RawncFile::loadFile(const string &fileName) {
  ...
  if ((vertPositions.size() == 0) ||
      ((pointIndices.size() == 0) && (lineIndices.size() == 0) &&
       (triangleIndices.size() == 0))) {
    return 0;
  }
  fclose(fp);
  ...
  if (triangleIndices.size()) {
    ...
    for (i = 0; i < vertNormals.size(); i++) {
      geometry->m_TriVertNormals[i] = vertNormals[i];
    }
...
bool RawncFile::checkType(const string &fileName) { return false; }
...
bool RawncFile::saveMolCenters(const Geometry *geometry) {
  ...
  for (c = 0; c < geometry->m_NumTris; c++) {
    ...
    if (r_val < 0.6) {
      ...
      Geometry::getPointInTriangle(x1, y1, z1, x2, y2, z2, x3, y3, z3, &xinterp,
                                   &yinterp, &zinterp);
      float nx, ny, nz;
  ...
  for (c = 0; c < geometry->m_NumTriVerts; c++) {
    if (geometry->m_TriVertColorsTransparent) {
      if (0 >= fprintf(fp, "%f %f %f %f %f %f %f %f %f\n",
                       ...
                       (geometry->m_TriVertColorsTransparent[c * 3 + 0]),
                       (geometry->m_TriVertColorsTransparent[c * 3 + 1]),
                       (geometry->m_TriVertColorsTransparent[c * 3 + 2]))) {
      ...
      if (0 >= fprintf(fp, "%f %f %f %f %f %f %f %f %f\n",
                       ...
                       (geometry->m_TriVertNormals[c * 3 + 0]),
                       (geometry->m_TriVertNormals[c * 3 + 1]),
                       (geometry->m_TriVertNormals[c * 3 + 2]), (1.0), (1.0),
                       (1.0))) {
        // qDebug("Error writing out vert # %d", c);
        printf("Error writing out vert # %d\n", c);
        fclose(fp);
