...
int ScoreComparator::compareUsingWeightedSum(const Score *S1,
                                             const Score *S2) const {
  if (verbose)
    std::cout << "Using weighted sum" << std::endl;
  ...
  for (int i = 0; i < numScores; i++) {
    if (!W1->getWeight(i, w1))
      return 0;
    if (!W2->getWeight(i, w2))
      return 0;
    if (!S1->getScore(i, s1))
      return 0;
    if (!S2->getScore(i, s2))
  ...
  if (verbose)
    std::cout << totalscore1 << " " << totalscore2 << "\n ";
...
int ScoreComparator::compareUsingMajority(const Score *S1,
                                          const Score *S2) const {
  if (verbose)
    std::cout << "Using majority" << std::endl;
...
int ScoreComparator::compareUsingLexicographicOrder(const Score *S1,
                                                    const Score *S2) const {
  if (verbose)
    std::cout << "Using lexicography" << std::endl;
...
int ScoreComparator::compareUsingGroupWeightedSum(const Score *S1,
                                                  const Score *S2) const {
  if (verbose)
    std::cout << "Using grouped weighted sum" << std::endl;
