...
MultiBodyGraph::MultiBodyGraph(ScoreWeight* nw, ScoreWeight* ew, ScoreComparator* nc, ScoreComparator* ec, bool v)
{
	verbose = v;
	numNodes = 0;
	numEdges = 0;
...
{
	MultiBodyNode* node = new MultiBodyNode(numNodes, nodeFileName, nodeWeights, verbose);
...
{
	MultiBodyEdge* edge = new MultiBodyEdge(src, tgt, numEdges, xformFileName, edgeWeights, edgeComp, verbose);
...
void MultiBodyGraph::sortNodes()
{
	...
	std::sort (nodestemp.begin(), nodestemp.end(), compareNodes(nodeComp,verbose));
...
{
	if(verbose) std::cout<<"Sorting "<<numEdges<< " edges\n";
	...
	std::sort (edgestemp.begin(), edgestemp.end(), compareMultiEdges(new compareSimpleEdges(edgeComp, false), false));
	...
	if(verbose)
	{
		for(int i=0; i<edges.size(); i++)
		{
			...
			std::cout<<"Rank "<< i <<" MultiEdge = " << index << ".   "<< chr[edges[index]->getSourceNodeIndex()] << " - " << chr[edges[index]->getDestinationNodeIndex()] << " with best simple edge index "<< edges[index]->getIndexOfNextBestSimpleEdge() << std::endl;
