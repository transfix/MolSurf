...
Assembler::~Assembler() { delete graph; }

bool Assembler::makeGraph(string nodeDataFileName, string edgeDataFileName,
  ...
  if (mode == WEIGHTEDSUM || mode == MAJORITY || mode == LEXICOGRAPHIC) {
    int numWeights;
    if (fscanf(fp, "%d\n", &numWeights) != 1) {
      std::cout << "Error reading nodeScoreSettings file. Failed to read "
                   "numWeights\n";
      return false;
    }
    ...
    for (int i = 0; i < numWeights; i++) {
      ...
      if (fscanf(fp, "%lf", &w) != 1) {
        std::cout
            << "Error reading nodeScoreSettings file. Failed to read weight\n";
        return false;
      }
  ...
  if (mode == WEIGHTEDSUM || mode == MAJORITY || mode == LEXICOGRAPHIC) {
    int numWeights;
    if (fscanf(fp, "%d\n", &numWeights) != 1) {
      std::cout << "Error reading edgeScoreSettings file. Failed to read "
                   "numWeights\n";
      return false;
    }
    ...
    for (int i = 0; i < numWeights; i++) {
      ...
      if (fscanf(fp, "%lf", &w) != 1) {
        std::cout
            << "Error reading edgeScoreSettings file. Failed to read weight\n";
        return false;
      }
  ...
  graph =
      new MultiBodyGraph(nodeWeights, edgeWeights, nodeComp, edgeComp, verbose);
  ...
  for (int i = 0; i < numNodes; i++) {
    if (fgets(str, 300, fp) != NULL) {
      ...
      std::cout << "Error reading nodedata file. Failed to read nfo file name "
                << i << "\n";
      return false;
    }
  }
  ...
  for (int i = 0; i < numMultiEdges; i++) {
    if (fscanf(fp, "%d %d ", &source, &target) != 2) {
      std::cout
          << "Error reading edgedata file. Failed to read source and target\n";
      return false;
    }
    ...
    if (fgets(str, 300, fp) != NULL) {
      ...
      std::cout << "Error reading edgedata file. Failed to read nfo file name "
                << i << "\n";
      return false;
    }
  }
