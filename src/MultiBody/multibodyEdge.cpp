...
MultiBodyEdge::MultiBodyEdge(int src, int dest, int id, std::string xformFileName, ScoreWeight* sw, ScoreComparator* ec, bool v)
{
	verbose = v;
	edgeId = id;
	source = src;
	...
	if(verbose) std::cout<<"reading xforms and scores from "<<xformFileName<<"\n";
...
bool MultiBodyEdge::readXformFile(string xformFileName)
{
	...
	{
		if(fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &t00, &t01, &t02, &t03, &t10, &t11, &t12, &t13, &t20, &t21, &t22, &t23, &t30, &t31, &t32, &t33) != 16)
		{
			std::cout<<"Error reading xform file. Failed to read transformation "<< i <<"\n";
			return false;
		}
		CCVOpenGLMath::Matrix T(t00, t01, t02, t03, t10, t11, t12, t13, t20, t21, t22, t23, t30, t31, t32, t33);
		...
		for(int j=0; j<numScores; j++)
		{
			...
			{
				std::cout<<"Error reading xform file. Failed to read score "<< i <<" " << j <<"\n";
				return false;
			}
			scores.push_back(sc);
		}
...
void MultiBodyEdge::addSimpleEdge(CCVOpenGLMath::Matrix mtx, vector<double>& scores)
{
	SimpleEdge* simpedge = new SimpleEdge(numSimpleEdges, source, destination, edgeWeights, scores, mtx, verbose);
...
bool MultiBodyEdge::getTransformation(int simpleEdgeIndex, CCVOpenGLMath::Matrix& T) const
{
...
bool MultiBodyEdge::getScore(int simpleEdgeIndex, int scoreIndex, double& score) const
{
	int numSimpleEdges = simpleEdges.size();
...
void MultiBodyEdge::sortSimpleEdges()
{
	...
	std::sort (edgestemp.begin(), edgestemp.end(), compareSimpleEdges(edgeComp,false));
