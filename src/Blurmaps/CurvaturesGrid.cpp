...
CurvaturesGrid::CurvaturesGrid() { grid = 0; }
...
void CurvaturesGrid::getMinMax(GroupOfAtoms *molecule, int numberOfPoints,
                               float *points) {
  int i;
...
void CurvaturesGrid::getIndices(double x, double y, double z, int *gridx,
                                int *gridy, int *gridz) {
...
int CurvaturesGrid::getIndex(double x, double y, double z) {
  ...
  return gridz * m_NumberOfGridDivisions * m_NumberOfGridDivisions +
         gridy * m_NumberOfGridDivisions + gridx;
}

void CurvaturesGrid::fillAtomsRecursively(GroupOfAtoms *molecule,
                                          double overlapExtent, int radiusType,
                                          unsigned int level) {
  unsigned int i;
  ...
  for (i = 0; i < molecule->m_Atoms.size(); i++) {
    ...
    getIndices(x - overlapExtent, y - overlapExtent, z - overlapExtent,
    ...
    for (j = mingridz; j <= maxgridz; j++) {
      for (k = mingridy; k <= maxgridy; k++) {
        for (l = mingridx; l <= maxgridx; l++) {
          int index = j * m_NumberOfGridDivisions * m_NumberOfGridDivisions +
                      k * m_NumberOfGridDivisions + l;
          Tuple t(x, y, z, rad);
...
void CurvaturesGrid::createGrid(GroupOfAtoms *molecule, int numberOfPoints,
                                float *points, double blobbyness,
                                int numberOfGridDivisions,
                                double maxFunctionError, int radiusType,
                                unsigned int level) {
  delete grid;
  grid = 0;
  ...
  grid = new CurvaturesGridVoxel[numberOfGridDivisions * numberOfGridDivisions *
  ...
  double overlapExtent =
      (log(maxFunctionError) + m_Blobbyness) * maxAtomRadius / m_Blobbyness;
  fillAtomsRecursively(molecule, overlapExtent, radiusType, level);
}

bool CurvaturesGrid::getCurvatures(double *HandK, double *normals,
                                   double *k1Vec, double *k2Vec,
                                   GroupOfAtoms *molecule, int numberOfPoints,
                                   float *points, double blobbyness,
                                   int numberOfGridDivisions,
                                   double maxFunctionError, int radiusType,
                                   unsigned int level) {
  createGrid(molecule, numberOfPoints, points, blobbyness,
  ...
  for (i = 0; i < numberOfPoints; i++) {
    ...
    for (k = 0; k < numAtoms; k++) {
      ...
      evalCurvature(&phiX, &phiY, &phiZ, &phiXX, &phiYY, &phiZZ, &phiXY, &phiXZ,
                    &phiYZ, ax, ay, az, rad, x, y, z, i);
    }
    ...
    // mean curvature
    HandK[i * 2 + 0] =
        (phiX * phiX * (phiYY + phiZZ) + phiY * phiY * (phiXX + phiZZ) +
         phiZ * phiZ * (phiXX + phiYY) - 2 * phiX * phiY * phiXY -
         2 * phiY * phiZ * phiYZ - 2 * phiX * phiZ * phiXZ) /
    ...
    if (fabs(k1 - k2) > 1e-10) {
      // construct curvature matrix. Find eigenvectors corresponding to k1 and
      // k2.
      double c11, c12, c13;
      double c21, c22, c23;
      double c31, c32, c33;
      double g3 = g * g * g;
      c11 = (phiXX * g - phiX * (phiX * phiXX + phiY * phiXY + phiZ * phiXZ)) /
            g3;
      c12 = (phiXY * g - phiX * (phiX * phiXY + phiY * phiYY + phiZ * phiYZ)) /
            g3;
      c13 = (phiXZ * g - phiX * (phiX * phiXZ + phiY * phiYZ + phiZ * phiZZ)) /
            g3;
      c21 = (phiXY * g - phiY * (phiX * phiXX + phiY * phiXY + phiZ * phiXZ)) /
            g3;
      c22 = (phiYY * g - phiY * (phiX * phiXY + phiY * phiYY + phiZ * phiYZ)) /
            g3;
      c23 = (phiYZ * g - phiY * (phiX * phiXZ + phiY * phiYZ + phiZ * phiZZ)) /
            g3;
      c31 = (phiXZ * g - phiZ * (phiX * phiXX + phiY * phiXY + phiZ * phiXZ)) /
            g3;
      c32 = (phiYZ * g - phiZ * (phiX * phiXY + phiY * phiYY + phiZ * phiYZ)) /
            g3;
      c33 = (phiZZ * g - phiZ * (phiX * phiXZ + phiY * phiYZ + phiZ * phiZZ)) /
            g3;
      if (CCVOpenGLMath::LinearAlgebra::solveEigenSystem(
              c11, c12, c13, c21, c22, c23, c31, c32, c33, tk1Vec, tk2Vec, k1,
              k2)) {
        double d;
        d = sqrt(tk1Vec[0] * tk1Vec[0] + tk1Vec[1] * tk1Vec[1] +
                 tk1Vec[2] * tk1Vec[2]);
        tk1Vec[0] /= d;
        tk1Vec[1] /= d;
        tk1Vec[2] /= d;
        d = sqrt(tk2Vec[0] * tk2Vec[0] + tk2Vec[1] * tk2Vec[1] +
                 tk2Vec[2] * tk2Vec[2]);
        tk2Vec[0] /= d;
        tk2Vec[1] /= d;
        tk2Vec[2] /= d;
    ...
    if (((i + 1) % 20) == 0 || (i + 1) == numberOfPoints) {
      printf("Evaluating functions %2.2f%% done (%012d)\r",
             100.0 * (i + 1) / (float)(numberOfPoints), i + 1);
    }
  }
  printf("\n");
  return true;
...
inline void
CurvaturesGrid::evalCurvature(double *phiX, double *phiY, double *phiZ,
                              double *phiXX, double *phiYY, double *phiZZ,
                              double *phiXY, double *phiXZ, double *phiYZ,
                              double xc, double yc, double zc, double rad,
  ...
  /*     if( vertexNumber == 329 )
          {
                  ...
                  fprintf( fp, "phi = %lf, *phiX = %lf, point=[%lf %lf %lf],
     center=[%lf %lf %lf]\n", phi, (*phiX), x, y, z, xc, yc, zc ); fclose( fp);
          }*/
}

bool CurvaturesGrid::write(int numberOfPoints, double *HandK, double *normals,
                           double *k1Vec, double *k2Vec, const char *filename) {
  ...
  fprintf(fp, "# Then, search for one of the following: Mean, Gaussian, "
              "Normal, K1Vec, K2Vec\n");
  fprintf(fp, "# Any of the above should be followed by the corresponding "
              "values with 1, 1, 3, 3, 3 in each line\n");
  fprintf(fp, "#\n");
  ...
  fprintf(fp, "K1Vec\n");
  for (i = 0; i < numberOfPoints; i++) {
    fprintf(fp, "%lf %lf %lf\n", k1Vec[i * 3 + 0], k1Vec[i * 3 + 1],
            k1Vec[i * 3 + 2]);
  }
  fprintf(fp, "K2Vec\n");
  for (i = 0; i < numberOfPoints; i++) {
    fprintf(fp, "%lf %lf %lf\n", k2Vec[i * 3 + 0], k2Vec[i * 3 + 1],
            k2Vec[i * 3 + 2]);
  }
  fclose(fp);
  return true;
