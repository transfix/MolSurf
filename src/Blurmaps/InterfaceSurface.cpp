...
bool InterfaceSurface::detect(double &area1, double &area2) {
  ...
  if (!interfaceFile1) {
    cout << "could not open surface file " << interIndexFileName1.c_str()
         << endl;
    return false;
  }
  ...
  if (!interfaceFile2) {
    cout << "could not open surface file " << interIndexFileName2.c_str()
         << endl;
  ...
  if (!highlightedSurfaceFile1) {
    cout << "could not open surface file "
         << highlightedSurfaceFileName1.c_str() << endl;
    return false;
  }
  ...
  if (!highlightedSurfaceFile2) {
    cout << "could not open surface file "
         << highlightedSurfaceFileName2.c_str() << endl;
  ...
  /*      interfaceFile = fopen(interfaceFileName.c_str(), "rt");
          ...
          {
                  cout<<"could not open surface file "<<
     interfaceFileName.c_str() << endl; fclose(highlightedSurfaceFile1);
                  fclose(highlightedSurfaceFile2);
                  return false;
          }*/

  /* Identifying interface points on the first surface and writing to
   * highlightedSurfaceFile1 */

  fprintf(highlightedSurfaceFile1, "%d\n", numVert1);
  fprintf(highlightedSurfaceFile1, "%d\n", numTriang1);

  for (int i = 0; i < numVert1; i++) {
    fprintf(highlightedSurfaceFile1, "%lf %lf %lf %lf %lf %lf", xArray1[i],
            yArray1[i], zArray1[i], nxArray1[i], nyArray1[i], nzArray1[i]);

    DPG::Point *p = new DPG::Point(xArray1[i], yArray1[i], zArray1[i]);

    if (pg2->pointsWithinRange(
            p, interfaceWidth)) // point is on the interface. (re)color it
    {
      interface1[i] = true;
  ...
  for (int i = 0; i < numTriang1;
       i++) // copy over the triangles and compute the area
  {
  ...
  /* Identifying interface points on the second surface and writing to
   * highlightedSurfaceFile2 */

  fprintf(highlightedSurfaceFile2, "%d\n", numVert2);
  fprintf(highlightedSurfaceFile2, "%d\n", numTriang2);

  for (int i = 0; i < numVert2; i++) {
    fprintf(highlightedSurfaceFile2, "%lf %lf %lf %lf %lf %lf", xArray2[i],
            yArray2[i], zArray2[i], nxArray2[i], nyArray2[i], nzArray2[i]);

    DPG::Point *p = new DPG::Point(xArray2[i], yArray2[i], zArray2[i]);

    if (pg1->pointsWithinRange(
            p, interfaceWidth)) // point is on the interface. (re)color it
    {
      interface2[i] = true;
